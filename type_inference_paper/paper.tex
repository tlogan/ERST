...
\begin{figure*}[h]
  ...
  \begin{flalign*}
    &\boxed{\text{build}^{+|-}(\mathrm{A}, \Delta, \mathrm{A}, \Delta, \tau) = \tau}&
  \end{flalign*}
  \[\begin{array}{rclr}
    \\
    \text{build}^{+|-}(\epsilon, \epsilon, \epsilon, \epsilon, \tau) 
    &=& 
    \tau
    \\
    \text{build}^{+|-}(\epsilon, \epsilon, \mathrm{A}_i, \Delta_i, \tau) 
    &=& 
    \text{inner}(+|-)\code{[}\mathrm{A}_i\code{ }\Delta_i\code{]}\tau
    \\
    \text{build}^{+|-}(\mathrm{A}_o, \Delta_o, \epsilon, \epsilon, \tau) 
    &=& 
    \text{outer}(+|-)\code{[}\mathrm{A}_o\code{ }\Delta_o\code{]}\tau
    \\
    \text{build}^{+|-}(\mathrm{A}_o, \Delta_o, \mathrm{A}_i, \Delta_i, \tau) 
    &=& 
  ...
  \begin{mathpar}
    \inferrule[Main] {
      ...
      \mathrm{A}, \mathrm{A}, \Delta \entails \tau \equiv^{+|-} \tau
    }
  \end{mathpar}
\caption{HELPERS}
\label{fig:helpers}

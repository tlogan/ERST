...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given Z}\ (Proof typing)
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \Gamma \entails \code{fix($e$)} \hastype \code{ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\Tau_{rel})$]]$\alpha_5$} \given Z_0
    }
  \end{mathpar}
\end{definition}
...
\begin{theorem}(Proof typing soundness)
  ...
  \item \Z $\B{case } 
    ...
    Z_0 \looparrowright Z_1
    \I
    \tau' \subtypes \code{$\alpha_{h^+}$->$\alpha_l$->$\alpha_r$} \given Z_1
  $ 
  \item \Z $
  ...
  \item \Z\Z $
    \tau' \subtypes 
    \code{BOT->ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\Tau)$]]$\alpha_5$} 
    \given Z_0 
  ...
  \item \Z\Z $
    \Omega \satisfies e' \hastype \code{BOT->$\tau$}
  $ by theorem \ref{theorem:model_subtyping_elimination} 
  ...
  \item \Z\Z $
    \Omega \satisfies \code{fix($e'$)} \hastype \tau 
...
\begin{theorem}(Fixpoint duality soundness redo)
  \label{theorem:fixpoint_duality_soundness_redo}
  \TODO{redo fixpoint typing rule to require that body of fix has type $\alpha_{h^+}$->$\tau$}

  \TODO{this will simplify the interpretation of $\alpha_{h^+}$ and avoid intermediate variables}
  \begin{mathpar}
    \inferrule { 
      \tau \subtypes \code{$\alpha_{h^+}$->$\alpha_l$->$\alpha_r$} \given M, \Delta
      \\\\
      \text{FTV}(\Gamma) \entails \alpha_{h^+} \cdot Z \cdot \code{$\alpha_l$->$\alpha_r$} \fallingdotseq \alpha_{h^-} \cdot \Tau
      \\\\
      \forall M'\ \Delta' .\ \left< M', \Delta' \right> \in Z 
      \implies M \preceq M' \land \Delta \preceq \Delta' 
    } {
      \tau
      \subtypes
      \code{BOT->ALL[$\alpha_l'$]$\alpha_l'$->EXI[$\alpha_r'$.($\alpha_l'$,$\alpha_r'$)<:FIX[$\alpha_{h^-}$.|$(\Tau)$]]$\alpha_r'$} 
      \given M, \Delta
    } 
  \end{mathpar}
  Proof: 
...
\begin{theorem}(Fixpoint duality reduction)
  \label{theorem:fixpoint_duality_reduction}
  \begin{mathpar}
    \inferrule { 
      \tau' \subtypes \code{$\alpha_{h^+}$->$\alpha_l$->$\alpha_r$} \given M, \Delta
      \\\\
      N \entails \alpha_{h^+} \cdot Z \cdot \code{$\alpha_l$->$\alpha_r$} \fallingdotseq \alpha_{h^-} \cdot \Tau
    } {
      \exists \alpha\ \tau .\ \tau' = \code{$\alpha$->$\tau$}
      \land
      \alpha_{h^+} \subtypes \alpha \given M, \Delta
      \land
      \tau \subtypes \code{$\alpha_l$->$\alpha_r$} \given M, \Delta
    } 
  \end{mathpar}
  Proof: 
\end{theorem}

\begin{theorem}(Fixpoint duality soundness inductable)
  \label{theorem:fixpoint_duality_soundness_inductable}
  \TODO{figure out statement that breaks down $\tau$ and $\Tau$ in parallel}
  \begin{mathpar}
    \inferrule { 
      \tau \subtypes \code{$\alpha_l$->$\alpha_r$} \given M_1, \Delta_1 
      \\\\
      \forall M'\ \Delta' .\ \left< M', \Delta' \right> \in Z 
      \implies M \preceq M' \land \Delta \preceq \Delta' 
      \\\\
...
\begin{theorem}(Model typing implication independence)
  ...
  \begin{mathpar}
    \inferrule { 
      ...
      \Omega, \Gamma \satisfies \code{fix($e$)} \hastype \tau_r
    } 
  \end{mathpar}
  Proof: 
  \item \TODO{...} 

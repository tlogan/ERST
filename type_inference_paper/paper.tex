...
\begin{definition}
\boxed{N \entails \alpha \circlearrowleft Z \cdot \code{$\alpha$->$\alpha$} \fallingdotseq \alpha \circlearrowright \Tau}
\ (Fixpoint duality)
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      M, \Delta, \Delta \entails \Tau_r \subtypes \alpha_2
      \\
      \code{(\&$(\Tau_l)$,|$(\Tau_r)$)} = \tau_p
      \\\\
      \text{FTV}(\tau_p) = \Alpha_p 
      \\
      ...
      \fallingdotseq 
      \alpha_3 \circlearrowright \Tau\ \tau
    }
  \end{mathpar}
\end{definition}
...
\begin{theorem}(Proof typing soundness)
  ...
  $
  \item \Z $
    \tau = \code{ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\Tau_{rel})$]]$\alpha_5$}
  $
  \item \Z $
    Z = Z_0
  $

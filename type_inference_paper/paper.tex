...
\begin{lemma}(Elimination of implication of model typing)
  ...
  \item \Z $\textbf{case }
    ...
    \I
    e_1[\Sigma] \rightsquigarrow e_1'
    \I
    \Omega, \Gamma \satisfies e_1' \hastype \tau_l
  $
  \item \Z\Z $\Omega, \Gamma \satisfies v_0\code{(}e_1'\code{)} \hastype \tau_r$ by induction hypothesis
  \item \Z\Z $v_0\code{(}e_1[\Sigma]\code{)} \rightsquigarrow v_0\code{(}e_1'\code{)}$
  \item \Z\Z ($v_0\code{(}e_1\code{)})[\Sigma] \rightsquigarrow v_0\code{(}e_1'\code{)}$
  \item \Z\Z $\Omega, \Gamma \satisfies v_0\code{(}e_1\code{)} \hastype \tau_r$ 
  ...
  \item \Z $\Omega, \Gamma \satisfies e_0\code{(}e_1\code{)} \hastype \tau_r$ by induction on $\Omega, \Gamma \satisfies e_0 \hastype \tau_l \code{->} \tau_r$ and $\Omega, \Gamma \satisfies e_1 \hastype \tau_l$ 

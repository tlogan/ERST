...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \Omega}\ Proof Typing
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \forall \vec{\alpha}'\ \Delta' .\ 
       (\vec{\alpha}', \Delta', \alpha_l \obj{->} \alpha_r) \in \vec{\pi}_\nu
      \implies
      \tau \subtypes \alpha_l\obj{->}\alpha_r \given  \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
      \\\\
...
\begin{definition}\boxed{\tau \subtypes \tau \given \Omega}\ Proof Subtyping
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \TODO{update to sub the new constraint into previous constraints, matching polarity}
      \\\\
      \alpha \notin \vec{\alpha}
      \\\\
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\eta}}\ Function Lifting
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \forall \vec{\alpha}'\ \Delta'\ \tau_r .\ (\vec{\alpha}', \Delta', \tau_l \obj{->} \tau_r) \in \vec{\pi}' 
      \implies
...
\begin{definition}
\boxed{\vec{\alpha} \entails \text{GFP}[\alpha]\vec{\pi} \fallingdotseq \text{LFP}[\alpha]\vec{\pi}}
\ Fixed Point Duality 
...
\Gamma, ... \satisfies (e_l,e_r) : \alpha_l \obj{*} \alpha_r
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \text{LFP}[\alpha_{\mu}] \vec{\pi}_\mu
      \\\\
      \vec{\alpha_c} \entails \Delta_\nu \entails \alpha_l \subtypes (\Delta', \tau_l)
      \\
      \vec{\alpha_c} \entails \Delta' \entails \alpha_r \supertypes (\Delta'', \tau_r)
      \\
      [\alpha_\nu]\Delta'' \fallingdotseq [\alpha_\mu]\Delta_\mu
      \\\\
      \TODO{add removal of intermediates to match implementation}
    } {
      \vec{\alpha}_f \entails 
      \text{GFP}[\alpha_{\nu}]\vec{\pi}_\nu\ \left< \vec{\alpha}_c,\Delta_\nu, \alpha_l \obj{->} \alpha_r \right> 
      \fallingdotseq 
      \text{LFP}[\alpha_{\mu}] \vec{\pi}_\mu\ \left< \vec{\alpha}_c,\Delta_\mu, \tau_l \obj{*} \tau_r \right>
    }
  \end{mathpar}
\end{definition}
...
\begin{definition}\boxed{
      \vec{\alpha} \entails (\Delta, \alpha) \subtypes (\Delta, \tau)
...
\begin{definition}\boxed{
  \vec{\alpha} \entails (\Delta, \alpha) \supertypes (\Delta, \tau)

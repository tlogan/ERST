...
\begin{theorem}(Soundness of typing)
  ...
  \item \I $\textbf{let } \Omega\ \Gamma' \ \tau' \textbf{ s.t. } \Omega, \Gamma' \satisfies e \hastype \tau'$ by \D{Lemma \ref{lemma:soundness_proof_typing}}
  \item \I $e \textbf{ sound} $ by \D{Lemma \ref{lemma:soundness_model_typing}}
  \item $\square$
\end{theorem}
...
\begin{lemma}(Soundness of proof typing)
  ...
  Proof ... 
\end{lemma}
\TODO{Cretin's corresponding theorem is on p. 134}
...
\begin{lemma}(Soundness of model typing)
  ...
  \end{mathpar}
  Proof:
  \item \TODO{the induction hypothesis includes the generalized assumption, e.g. $\forall e' .\ e' < e \implies Q(e')$ if inducting on $e$ or $\forall e' .\ e' < e \implies P(e) \implies Q(e)$ if inducting on predicate $P$} 
  \item $\textbf{assume }
        \Omega, \Sigma \satisfies \Gamma 
        \Z \Omega, \Gamma \satisfies e \hastype \tau
  $
  \item \Z $\textbf{case } e = \code{@}$
  \item \Z\Z $\textbf{let } v \textbf{ s.t. } \code{@} = v$
  \item \Z\Z $e[\Sigma] = v$
  \item \Z\Z $e[\Sigma] \textbf{ sound}$


  \item \Z $\textbf{case } e = \code{\ttilde}l\ e'$
  \item \Z\Z $e'[\Sigma] \textbf{ sound}$ by induction hypothesis


  \item \Z\Z $\tau = \code{\ttilde}l\ \tau'$ by inversion on $\Omega, \Gamma \satisfies e \hastype \tau$
  \item \Z\Z $\Omega, \Gamma \satisfies e' \hastype \tau'$ by inversion on $\Omega, \Gamma \satisfies e \hastype \tau$
  ...
  \item \Z\Z\Z $e[\Sigma] \textbf{ sound}$

  \item \Z\Z $\textbf{case } e'[\Sigma] \rightsquigarrow e'' \Z e'' \textbf{ sound}$
  \item \Z\Z\Z $\code{\ttilde}l\ e'[\Sigma] \rightsquigarrow \code{\ttilde}l\ e''$
  \item \Z\Z\Z $\code{\ttilde}l\ e'' \textbf{ sound}$
  \item \Z\Z\Z $\code{\ttilde}l\ e'[\Sigma] \textbf{ sound}$
  \item \Z\Z\Z $(\code{\ttilde}l\ e')[\Sigma] \textbf{ sound}$
  \item \Z\Z\Z $e[\Sigma] \textbf{ sound}$

  \item \Z\Z $e \textbf{ sound}$ by cases on $e' \textbf{ sound}$

  \item \Z \TODO{remaining introduction cases}

  \item \Z $\textbf{case } e = x$
  \item \Z\Z $x \hastype \tau \in \Gamma$ by inversion on $\Omega, \Gamma \satisfies e \hastype \tau$
  \item \Z\Z $x \slash v \in \Sigma$ by inversion on $\Omega, \Sigma \satisfies \Gamma$
  \item \Z\Z $x[\Sigma] = v$
  \item \Z\Z $e[\Sigma] = v$
  \item \Z\Z $e[\Sigma] \textbf{ sound}$

  \item \Z $\textbf{case } e = e_0\code{(} e_1 \code{)}$
  \item \Z\Z $e_0[\Sigma] \textbf{ sound} \I e_1[\Sigma] \textbf{ sound}$ by induction hypothesis
  \item \Z\Z $\textbf{let } e' \textbf{ s.t. } e_0\code{(} e_1 \code{)} \rightsquigarrow e'$ 

  \item \Z\Z $\textbf{case } e_0[\Sigma] = F \I e_1[\Sigma] = v$ 
  \item \Z\Z\Z \TODO{easy}
  \item \Z\Z\Z $e_0[\Sigma]\code{(} e_1[\Sigma] \code{)} \textbf{ sound}$ 

  \item \Z\Z $\textbf{case } e_0[\Sigma] = F \I e_1[\Sigma] \rightsquigarrow e_1'$ 
  \item \Z\Z\Z \TODO{...}
  \item \Z\Z\Z $e_0[\Sigma]\code{(} e_1[\Sigma] \code{)} \textbf{ sound}$ 

  \item \Z\Z $\textbf{case } e_0[\Sigma] \rightsquigarrow e_0'$ 
  \item \Z\Z\Z \TODO{think carefully}
  \item \Z\Z\Z $e_0[\Sigma]\code{(} e_1[\Sigma] \code{)} \textbf{ sound}$ 

  % \item \Z\Z $\textbf{case } e_0[\Sigma] \rightsquigarrow e_0' \I e_1[\Sigma] = v$ 
  % \item \Z\Z\Z \TODO{think carefully}
  % \item \Z\Z\Z $e_0[\Sigma]\code{(} e_1[\Sigma] \code{)} \textbf{ sound}$ 

  % \item \Z\Z $\textbf{case } e_0[\Sigma] \rightsquigarrow e_0' \I e_1[\Sigma] \rightsquigarrow e_1'$ 
  % \item \Z\Z\Z \TODO{think carefully}
  % \item \Z\Z\Z $e_0[\Sigma]\code{(} e_1[\Sigma] \code{)} \textbf{ sound}$ 

  \item \Z\Z $e[\Sigma] \textbf{ sound}$ by cases on $e_0\code{(} e_1 \code{)} \rightsquigarrow e'$
  \item \Z\Z \TODO{what does Cretin do? see Lemma 91, p. 129}
  \item \Z\Z \TODO{CANNOT apply induction to reduction in small step, since small step result is not strictly smaller, i.e. a sequence of small steps is possibly infinite.}
    % \inferrule { 
  ...
  \item \Z $e \textbf{ sound}$ by induction on $e$
  \item $\square$
  ...
  \item \TODO{
    ...
    further analysis of the tails (opacity)".
  }
  \item \TODO{
    Kozen demonstrates a legal proof by induction on infinite streamts too
  }
\end{lemma}
...
\end{lemma}
  ...
  \TODO{Cretin's Definition 88 has a typo. semantic judgment |= should be defined with an existential substituion mapping}
  \TODO{Definition 88 typo is due to lack of flipping of quantifier when moving from outside to inside the premise}

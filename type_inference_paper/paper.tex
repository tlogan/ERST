...
\begin{figure*}[h]
  ...
  \begin{mathpar}
    ...
    \inferrule[Recursion] {
      ...
      \\
      \alpha_0, \alpha_1, \alpha_2\ \#\ \Gamma, \vec{W}
      \\
      \vec{W}_0 \looparrowright \vec{W}_1
      \\
      \tau \subtypes \code{$\alpha_0$->$\alpha_1$->$\alpha_2$} \given \vec{W}_1
      \\\\
      \text{FTV}(\Gamma) \entails \alpha_0 \circlearrowleft \vec{W}_1 \cdot \code{$\alpha_1$->$\alpha_2$} \rightsquigarrow \tau_{rel}
    } {
      \Gamma \entails \code{fix($e$)} \hastype \code{ALL[$\alpha_3$]$\alpha_3$->EXI[$\alpha_4$;($\alpha_3$,$\alpha_4$)<:$\tau_{rel}$]$\alpha_4$} \given \vec{W}_0
    }
...
\begin{figure*}[h]
  ...
  \begin{flalign*}
    &\boxed{\vec{z} \entails \alpha \circlearrowleft \vec{W} \cdot \code{$\alpha$->$\alpha$} \rightsquigarrow \tau}&
  \end{flalign*}
  % NOTE: read "z, \- a0 Circle W  . a1 -> a2 --> T" as 
  % - forall x y, (FIX[a] intersect vec{W}: a1 -> a2) <: (x -> y) iff (x, y) <: T
  \begin{mathpar}
    \inferrule[TODO] {
    } {
      \vec{z} \entails \alpha \circlearrowleft \vec{W} \cdot \code{$\alpha$->$\alpha$} \rightsquigarrow \tau
    }
  \end{mathpar}

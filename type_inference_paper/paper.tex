...
\begin{figure*}[h]
  \[\begin{array}[t]{r@{}l}
    ...
    \\
    \nonterm{\delta} & \is \cdot \tau <: \tau 
    \\
    \nonterm{\mathrm{B}} & \is \epsilon \sep \mathrm{B}\ \beta
    \\
    \nonterm{\beta} & \is \cdot \alpha
    \\
    \nonterm{N} & \is \epsilon \sep N\ n
    \\
    \nonterm{n} & \is \cdot \alpha
    \\
...
\begin{figure*}[h]
  \[\begin{array}[t]{r@{}l}
    ...
    \\
    \nonterm{w} & \is \cdot u \triangleright u
    \\
    \\
  ...
  \end{array}\]
  \label{fig:model_syntax}
  \caption{Model Syntax}
\end{figure*}
...
\begin{mathpar}
  ...
  %   \text{unify}(p,v) = \Sigma 
  % } {
  %   \code{(case $p$=>$e$)($v$)} \rightsquigarrow e[\Sigma]
  % }
...
\begin{figure*}[h]
  ...
  \begin{flalign*}
    &\boxed{\Omega \entails u \uhastype \tau}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Var] { 
      \cdot \alpha \slash \tau \in \Omega
      \\
      \Omega \entails u \uhastype \tau
    } {
      \Omega \entails u \uhastype \alpha 
    } 

    \inferrule[Unit] { 
    } {
      \Omega \entails \code{@} \uhastype \code{@}
    } 

    \inferrule[Tag] { 
      \Omega \entails u \uhastype \tau
    } {
      \Omega \entails \code{\textasciitilde{}$l$ $u$} \uhastype \code{\textasciitilde{}$l$ $\tau$}
    } 
    ...
    \inferrule[Record] { 
      ...
      \Omega \entails H \uhastype \code{$l$->$\tau$}
    } 

    \inferrule[Implication] { 
      \cdot u_l \triangleright u_r \in W 
      \\
      \Omega \entails u_l \uhastype \tau_l
      \\
      \Omega \entails u_r \uhastype \tau_r
      \\
      \forall u_r' .\ \cdot u_l \triangleright u_r \in W \rightarrow u_r' = u_r 
    } {
      \Omega \entails W \uhastype \code{$\tau_l$->$\tau_r$}
    } 

    \inferrule[Union-L] { 
      \Omega \entails u \uhastype \code{$\tau_l$}
    } {
      \Omega \entails u \uhastype \code{$\tau_l$|$\tau_r$}
    } 

    \inferrule[Union-R] { 
      \Omega \entails u \uhastype \code{$\tau_r$}
    } {
      \Omega \entails u \uhastype \code{$\tau_l$|$\tau_r$}
    } 
    ...
    \inferrule[Diff] { 
      \Omega \entails u \uhastype \code{$\tau_l$}
      \\
      \neg\ (\Omega \entails u \uhastype \code{$\tau_r$})
    } {
      \Omega \entails u \uhastype \code{$\tau_l$\textbackslash$\tau_r$}
    } 

    \inferrule[IndUnion] { 
      \Omega \sqcup \Omega' \entails Q
      \\
      \Omega \sqcup \Omega' \entails u \uhastype \tau
    } {
      \Omega \entails u \uhastype \code{EXI[$\mathrm{A}$ $Q$]$\tau$}
    } 

    \inferrule[IndInter] { 
      \forall \Omega' .\ \Omega \sqcup \Omega' \entails Q \rightarrow 
      \Omega \sqcup \Omega' \entails u \uhastype \tau
    } {
      \Omega \entails u \uhastype \code{ALL[$\mathrm{A}$ $Q$]$\tau$}
    } 

    \inferrule[Fixpoint] { 
      \Omega \cdot \alpha \slash \code{FIX[$\alpha$*$\tau$]} \entails u \uhastype \tau
    } {
      \Omega \entails u \uhastype \code{FIX[$\alpha$*$\tau$]}
    } 
  \end{mathpar}
  \begin{flalign*}
    &\boxed{\Omega \entails Q}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] { 
    } {
      \Omega \entails \epsilon
    } 

    \inferrule[Step] { 
      \Omega \entails Q 
      \\
      \forall u .\ \Omega \entails u \uhastype \tau_l \rightarrow u \uhastype \tau_r
    } {
      \Omega \entails Q \code{*$\tau_l$<:$\tau_r$}  
    } 
  \end{mathpar}
\caption{Type semantics and subtype semantics}

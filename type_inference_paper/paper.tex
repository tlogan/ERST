...
\begin{definition} \boxed{Z \looparrowright Z}
  \label{definition:multiverse_ordering}
  \begin{mathpar}
    \inferrule {
...
\begin{definition}\boxed{\tau \subtypes \tau \given M, \Delta}\ (Proof typing)
...
\begin{definition} \boxed{\tau \subtypes \tau \given M, \Delta}
  \begin{mathpar}
...
\begin{definition}\boxed{\tau \subtypes \tau \given Z}
  \begin{mathpar}
    \inferrule {
      \forall  M\ \Delta .\  
...
\begin{definition}\boxed{\code{|}(\mathrm{T}) = \tau}
  \[\begin{array}{rclr}
    \code{|}(\epsilon)
    &=& 
    \code{BOT}
...
\begin{definition}\boxed{M, \Delta, \Delta \entails \mathrm{T} \ll \alpha}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{M, \Delta, \Delta \entails \alpha \ll \mathrm{T}}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{M, \Delta, \mathrm{A} \entails \Omega}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{N, M \entails \Pi \equiv \mathrm{T}}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{N \entails \alpha \circlearrowleft Z \cdot \code{$\alpha$->$\alpha$} \fallingdotseq \alpha \circlearrowright \mathrm{T}}
...
\begin{definition}\boxed{M, \Delta \entails \mathrm{T} \subtypes \alpha}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{\text{quantify}^{+|-}(\mathrm{A}, \Delta, \mathrm{A}, \Delta, \tau) = \tau}
  \[\begin{array}{rclr}
...
\begin{definition}\boxed{\mathrm{A}, \mathrm{A}, \Delta \entails \tau \equiv^{+|-} \tau}
  \begin{mathpar}
    \inferrule {
...
\begin{definition}\boxed{\mathrm{A}, \Delta \entails \Delta \wr \Delta}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{e\ \textbf{wellformed}}
...
\begin{theorem}(Typing Soundness)
  ...
  \item \I $\textbf{let } \Omega\ \Gamma' \ \tau' \textbf{ s.t. } \Omega, \Gamma' \satisfies e \hastype \tau'$ by \D{Lemma \ref{theorem:proof_typing_soundness}}
  \item \I $\Omega, \Sigma \satisfies \Gamma'$ by ...
  \item \I $e[\Sigma] \textbf{ wellformed} $ by theorem \ref{theorem:model_typing_soundness}
  \item \I $e[\Sigma] = e$ by ...
  \item \I $e \textbf{ wellformed} $ by substitution 
  \item $\square$
\end{theorem}
...
\begin{theorem}(Proof typing soundness)
  ...
  \end{mathpar}
  Proof: 
  \item $\textbf{assume } \Gamma \entails e \hastype \tau \given Z$ 
  \item \Z $\textbf{induct on } \Gamma \entails e \hastype \tau \given Z$ 

  \item \Z $\textbf{case } e = \code{@} \I \tau = \code{@}$ 
  \item \Z\Z $\textbf{for } \Omega$
  \item \Z\Z\Z $\Omega, \Gamma \satisfies \code{@} \hastype \code{@}$
  \item \Z\Z\Z $\Omega, \Gamma \satisfies e \hastype \code{@}$ by substitution
  \item \Z\Z\Z $\exists \Omega\ \Gamma'\ \tau' .\ \Omega, \Gamma' \satisfies e \hastype \tau'$ by witness
  \item \Z\Z $\forall \Omega .\ \exists \Omega\ \Gamma'\ \tau' .\ \Omega, \Gamma' \satisfies e \hastype \tau'$ by generalization 
  \item \Z\Z $\exists \Omega\ \Gamma'\ \tau' .\ \Omega, \Gamma' \satisfies e \hastype \tau'$ by independence 

  \item \Z $\textbf{case } e = x \I \code{$x$:$\tau$} \in \Gamma$ 
  \item \Z $\textbf{wrt } x$ 
  \item \Z\Z $\textbf{for } \Omega $
  \item \Z\Z\Z $\Omega, \Gamma \satisfies x \hastype \tau$ 
  \item \Z\Z\Z $\exists \Omega, \tau' \Omega, \Gamma \satisfies x \hastype \tau'$ by witness
  \item \Z\Z $\forall \Omega .\ \exists \Omega, \tau' \Omega, \Gamma \satisfies x \hastype \tau'$ by generalization 
  \item \Z\Z $\exists \Omega, \tau' \Omega, \Gamma \satisfies x \hastype \tau'$ by independence 



  \item \Z $\textbf{case } e = \code{\ttilde $l$ $e'$} \I \tau = \code{\ttilde $l$ $\tau'$}
    \I \Gamma \entails e' \hastype \tau' \given Z
  $ 
  \item \Z $\textbf{hypo } 
      \Gamma \entails e' \hastype \tau' \given Z  
      \implies
      \exists \Omega\ \Gamma' \ \tau'' .\ \Omega, \Gamma' \satisfies e' \hastype \tau''
  $ 
  \item \Z $\textbf{wrt } e' \ \tau'$ 
  \item \Z\Z $
    \exists \Omega\ \Gamma' \ \tau'' .\ \Omega, \Gamma' \satisfies e' \hastype \tau''
  $ by application
  \item \Z\Z $
    \textbf{let } \Omega\ \Gamma' \ \tau'' \textbf{ s.t. } \Omega, \Gamma' \satisfies e' \hastype \tau''
  $
  \item \Z\Z $
    \Omega, \Gamma' \satisfies \code{\ttilde $l$ $e'$} \hastype \code{\ttilde $l$ $\tau''$}
  $ by definition
  \item \Z\Z $
    \Omega, \Gamma' \satisfies e \hastype \code{\ttilde $l$ $\tau''$} 
  $ by substitution
  \item \Z\Z $
    \exists \Omega\ \Gamma' \ \tau''' .\ \Gamma' \satisfies e \hastype \tau'''
  $ by witness
  ...
  \item \Z $\textbf{case } e = \code{$e'$.$l$} \I \tau = \alpha \I Z = Z_1
    \I
    \Gamma \entails e' \hastype \tau_0 \given Z_0
    \I
    \alpha\ \#\ \Gamma, Z_0
  ...
  \item \Z $\textbf{hypo } 
    ...
    \exists \Omega \ \Gamma' \tau' .\  \Gamma' \satisfies e' \hastype \tau'
  $ 
  \item \Z $\textbf{wrt } e' \ l \ \alpha \ \tau_0 \ Z_0 \ Z_1$ 
  \item \Z\Z $
    \Omega \satisfies \tau_0  \subtypes \code{$l$->$\alpha$}
  $ by theorem \ref{} \TODO{subtyping soundness}
  \item \Z\Z $
    \forall e \ \Gamma .\ \satisfies e \hastype  \tau_0  \implies \satisfies e \hastype \code{$l$->$\alpha$}
  $ by theorem \ref{} \TODO{model subtyping to typing}
  ...
  \item \TODO{... details ...}
  \item \Z\Z $
    \Gamma' \satisfies \code{$e'$.$l$} \hastype \tau''
  $ 
  \item \Z\Z $
    \exists \Omega\ \Gamma' \ \tau''' .\ \Gamma' \satisfies e \hastype \tau'''
  $ by witness
  ...
  \item \Z \TODO{remaining elimination caseis} 
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %   }
  % \end{mathpar}
  \item $\exists \Omega\ \Gamma' \ \tau' .\ \Omega, \Gamma' \satisfies e \hastype \tau'$
    by induction
  \item \TODO{Cretin's corresponding theorem is Theorem 101 on p. 134}
  \item \TODO{See how Cretin proves this without using subject reduction}
  \item $\square$
\end{theorem}
...
\begin{theorem}(Model typing subsumption)
  ...
  \end{mathpar}
  Proof: 
  \item $\textbf{assume } \Omega, \Gamma \satisfies e \hastype \tau_l 
  \I \Omega, \Gamma \satisfies \tau_l <: \tau_r
  $
  \item \Z $\textbf{invert on}\ \Omega, \Gamma \satisfies \tau_l <: \tau_r$
  \item \Z $\textbf{case}\ \forall e' .\ \Omega, \Gamma \satisfies e' : \tau_l \implies \Omega, \Gamma \satisfies e' : \tau_r$
...
\begin{theorem}(Model typing implication elimination)
  ...
  \item \Z $\textbf{induct on } \Omega, \Gamma \satisfies e_0 \hastype \tau_l \code{->} \tau_r$
  \item \Z $\textbf{case }
    e_0 = \code{($F$*$p$=>$e$)} 
    \I
  ...
  $
  \item \Z $
    \textbf{hypo} \  \Omega, \Gamma \satisfies F \hastype \tau_l \code{->} \tau_r \implies \Omega, \Gamma \satisfies \code{$F$($e_1$)} \hastype \tau_r
  $
  \item \Z $
    \textbf{wrt} \ F \ p \ e \ \Gamma' \  
  $

  \item \Z\Z $\textbf{let } \Sigma \textbf{ s.t. } \Omega, \Sigma \satisfies \Gamma $ by theorem \ref{theorem:model_typing_assignability} 
  \item \Z\Z $e_0[\Sigma] \textbf{ wellformed}$ by theorem \ref{theorem:model_typing_soundness}
  \item \Z\Z $\code{($F$*$p$=>$e$)}[\Sigma] \textbf{ wellformed}$ by substitution 
  \item \Z\Z $\textbf{let } v_0 \textbf{ s.t. } v_0 = \code{($F$*$p$=>$e$)}[\Sigma]$ 
    by theorem \ref{theorem:wellformed_function_valuation} 
  \item \Z\Z $e_1[\Sigma] \textbf{ wellformed}$ by theorem \ref{theorem:model_typing_soundness}



  \item \Z\Z $\textbf{induct on}\ e_1[\Sigma] \textbf{ wellformed}$

  \item \Z\Z $\textbf{case } e_1[\Sigma] = v_1 \ \textbf{for} \ v_1$  
  ...
  \item \Z\Z\Z $\textbf{invert on}\ F = \epsilon \lor \Omega, \Gamma \satisfies F \hastype \tau_l \code{->} \tau_r$
  \item \Z\Z\Z $\textbf{case } F = \epsilon$ 
  ...
  \item \Z\Z\Z\Z $\textbf{let}\ \Sigma' \textbf{ s.t. } p \equiv v_1 \given \Sigma'$ 
    by theorem \ref{theorem:model_typing_pattern_matching} 
  \item \Z\Z\Z\Z $\textbf{for } e'$ 
  ...
  \item \Z\Z\Z $\textbf{case } \Omega, \Gamma \satisfies F \hastype \tau_l \code{->} \tau_r$ 
  \item \Z\Z\Z\Z $\Omega, \Gamma \satisfies \code{$F$($e_1$)} \hastype \tau_r$ by application 
  \item \Z\Z\Z\Z $\textbf{let } e' \textbf{ s.t. } (\code{$F$($e_1$}))[\Sigma] \rightsquigarrow e' \land \Omega, \Gamma \satisfies e' \hastype \tau_r$ by theorem \ref{theorem:model_typing_function_progress}
  ...
  \item \Z\Z $\textbf{case}\ e_1[\Sigma] \rightsquigarrow e_1' \I e_1'\ \textbf{wellformed}$
  \item \Z\Z $\textbf{hypo}\ e_1'\ \textbf{wellformed} \implies \Omega, \Gamma \satisfies \code{($F$*$p$=>$e$)($e_1'$)} \hastype \tau_r$
  \item \Z\Z $\textbf{wrt}\ e_1' $
  \item \Z\Z\Z $\Omega, \Gamma \satisfies \code{($F$*$p$=>$e$)($e_1'$)} \hastype \tau_r$ by application
  \item \Z\Z\Z $\code{($F$*$p$=>$e$)$[\Sigma]$($e_1[\Sigma]$)} \rightsquigarrow \code{($F$*$p$=>$e$)$[\Sigma]$($e_1'$)}$ by definition 
  \item \Z\Z\Z $\forall x .\ x \notin \textbf{FV}(e_1')$ by theorem \ref{theorem:reduction_closed} 
...
\begin{theorem}(Well-formed function valuation)
  ...
  \end{mathpar}
  Proof:
  \item $\textbf{assume } F \textbf{ wellformed}$ 
  \item \Z $\textbf{invert on } F \textbf{ wellformed}$
  \item \Z $\textbf{case } v = F$
  \item \Z\Z $v = F$ by identity
  \item \Z $\textbf{case } F \rightsquigarrow e$
  \item \Z $\textbf{wrt } e$
...
\begin{theorem}(Model typing function progress)
  ...
  \end{mathpar}
  Proof:
  \item $\textbf{assume } \Omega, \Gamma \satisfies \code{$F$($e$)} \hastype \tau_r$
  \item \Z $\textbf{invert on } \Omega, \Gamma \satisfies \code{$F$($e$)} \hastype \tau_r$
  \item \Z $\textbf{case } ...$
  \item \Z \TODO{vacuous cases} 
  \item \Z $\textbf{case } \code{$F$($e$)} \rightsquigarrow e' \I \Omega, \Gamma \satisfies e' \hastype \tau_r$
  \item \Z $\textbf{wrt } e'$
...
%   \end{mathpar}
%   Proof:
%   \item $\textbf{for } p\ e\ F\ v\ \Sigma\ \textbf{assume }
%       \code{*$p$=>$e$} \in F 
%       \I
%       p \equiv v \given \Sigma 
%       \I
...
%   \item \Z\Z $\textbf{for } e'$
%   \item \Z\Z\Z $\textbf{case}\ F'\code{($v$)} \rightsquigarrow e'$
...
%     \code{*$p$=>$e$} \in F' 
%   $
%   \item \Z\Z $\textbf{let } e' \textbf{ s.t. } F'\code{(}v\code{)} \rightsquigarrow e'$ by induction hypothesis
...
\begin{theorem}(Model typing soundness)
  ...
  $
  \item \Z $\textbf{case } e = \code{@}$
  \item \Z\Z $\textbf{let } v \textbf{ s.t. } \code{@} = v$
  \item \Z\Z $e[\Sigma] = v$
  \item \Z\Z $e[\Sigma] \textbf{ wellformed}$


  \item \Z $\textbf{case } e = \code{\ttilde}l\ e' 
  \I \tau = \code{\ttilde}l\ \tau'
  \I \Omega, \Gamma \satisfies e' \hastype \tau'
  $
  \item \Z\Z $e' \textbf{ wellformed}$ by induction hypothesis

  \item \Z\Z $\textbf{case } e'[\Sigma] = v$
  \item \Z\Z\Z $\textbf{let } v' \textbf{ s.t. } \code{\ttilde}l\ v = v'$
  \item \Z\Z\Z $\code{\ttilde}l\ e'[\Sigma] = v'$
  \item \Z\Z\Z $(\code{\ttilde}l\ e')[\Sigma] = v'$
  \item \Z\Z\Z $e[\Sigma] = v'$
  \item \Z\Z\Z $e[\Sigma] \textbf{ wellformed}$

  \item \Z\Z $\textbf{case } e'[\Sigma] \rightsquigarrow e'' \I e'' \textbf{ wellformed}$
  \item \Z\Z\Z $\code{\ttilde}l\ e'[\Sigma] \rightsquigarrow \code{\ttilde}l\ e''$
  \item \Z\Z\Z $\code{\ttilde}l\ e'' \textbf{ wellformed}$
  \item \Z\Z\Z $\code{\ttilde}l\ e'[\Sigma] \textbf{ wellformed}$
  \item \Z\Z\Z $(\code{\ttilde}l\ e')[\Sigma] \textbf{ wellformed}$
  \item \Z\Z\Z $e[\Sigma] \textbf{ wellformed}$

  \item \Z\Z $e[\Sigma] \textbf{ wellformed}$ by cases on $e' \textbf{ wellformed}$

  \item \Z \TODO{remaining introduction cases}

  \item \Z $\textbf{case } e = x
  \I x \hastype \tau \in \Gamma
  \I x \slash v \in \Sigma
  $
  \item \Z\Z $x[\Sigma] = v$
  \item \Z\Z $e[\Sigma] = v$
  \item \Z\Z $e[\Sigma] \textbf{ wellformed}$

  \item \Z $\textbf{case } e = e_0\code{(} e_1 \code{)}$ and all other elimination cases 
  \item \Z\Z $\textbf{let } e' \textbf{ s.t. } e[\Sigma] \rightsquigarrow e' \I \Omega, \Gamma \satisfies e' \hastype \tau$ by inversion on $\Omega, \Gamma \satisfies e \hastype \tau$
  \item \Z\Z $e'[\Sigma] \textbf{ wellformed}$ by induction hypothesis
  \item \Z\Z $e[\Sigma] \textbf{ wellformed}$

  \item \Z $e[\Sigma] \textbf{ wellformed}$ by induction on $\Omega, \Gamma \satisfies e \hastype \tau$
  \item $\square$
  \item \TODO{Cretin's corresponding theorem is by definition of pretypes on p. 125}
\end{theorem}

\noindent
\textbf{NOTE: } The induction hypothesis includes the generalized assumption, e.g. $\forall e' .\ e' < e \implies Q(e')$ if inducting on $e$ or $\forall e' .\ (P(e') \implies P(e)), P(e') \implies Q(e')$ if inducting on predicate $P$

\noindent
\textbf{NOTE: } we induct on $\Omega, \Gamma \satisfies e \hastype \tau$ instead of $e$, as the predicate acts as a guard/ordering in lieu of a decreasing e.
This allows us to use the induction hypothesis on the reduction step result in the elimination case.

\noindent
\textbf{NOTE: } Kozen says, "Intuitively, one can appeal to the coinductive hypothesis as long as there
...
\begin{definition}\boxed{\Omega, \Gamma \satisfies e \hastype \tau}\ (Model Typing)
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      ...
      \Omega, \Gamma \satisfies e \hastype \tau 
      \\\\
      \forall e' .\ \code{*$l$=>$e'$} \in R \implies e' = e
    } {
      \Omega, \Gamma \satisfies R \hastype \code{$l$->$\tau$}
    } 
    ...
    \inferrule { 
      ...
      \Omega \satisfies e \hastype \code{EXI[$\mathrm{A}$ $Q$]$\tau$}
    } 
    ...
    \inferrule { 
      ...
      \Omega, \Gamma \satisfies e \hastype \code{ALL[$\mathrm{A}$ $Q$]$\tau$}
    } 

...
\begin{theorem}(Model typing record elimination)
  ...
  \TODO{update according to updated model typing rule}
  \begin{mathpar}
    \inferrule { 
...
\begin{theorem}(Proof subtyping soundness)
  ...
  \end{mathpar}
  Proof: 
  \item \TODO{May need to redo subtyping definition to avoid mutual recursion with multi-subtyping} 
  \item $\B{assume } 
    \tau_l \subtypes \tau_r \given M, \Delta 
  $
    ...
    $
    \item \Z $\B{hypo }
      \tau_l \subtypes \code{($l$->$\tau_{rl}$)\&($l$->$\tau_{rr}$)}
      \given M, \Delta
      \implies
      \exists \Omega .\ \Omega \satisfies \tau_l \subtypes \code{($l$->$\tau_{rl}$)\&($l$->$\tau_{rr}$)}
    $ 
    \item \Z $\B{wrt } l\ \tau_{rl}\ \tau_{rr}$ 
      \item \Z\Z $\B{let } \Omega \B{ s.t. } \Omega \satisfies \tau_l \subtypes \code{($l$->$\tau_{rl}$)\&($l$->$\tau_{rr}$)}$
      by application and declaration
      \item \Z\Z $\B{for } e\ \Gamma \B{ assume } \Omega, \Gamma \satisfies e \hastype \tau_l$ 
        \item \Z\Z\Z $\Omega, \Gamma \satisfies e \hastype \code{($l$->$\tau_{rl}$)\&($l$->$\tau_{rr}$)}$
        by theorem \ref{theorem:model_typing_subsumption} 
        \item \Z\Z\Z $\Omega, \Gamma \satisfies e \hastype \code{$l$->$\tau_{rl}$}$ by theorem \ref{theorem:model_typing_intersection_elimination}
        \item \Z\Z\Z $\Omega, \Gamma \satisfies e \hastype \code{$l$->$\tau_{rr}$}$ by theorem \ref{theorem:model_typing_intersection_elimination}
        \item \Z\Z\Z $\Omega, \Gamma \satisfies \code{$e$.$l$} \hastype \tau_{rl}$ by theorem \ref{theorem:model_subtyping_record_elimination}
        \item \Z\Z\Z $\Omega, \Gamma \satisfies \code{$e$.$l$} \hastype \tau_{rr}$ by theorem \ref{theorem:model_subtyping_record_elimination}
        \item \Z\Z\Z $\Omega, \Gamma \satisfies \code{$e$.$l$} \hastype \code{$\tau_{rl}$\&$\tau_{rr}$}$ by definition
        \item \Z\Z\Z $\Omega, \Gamma \satisfies e \hastype \code{$l$->($\tau_{rl}$\&$\tau_{rr}$)}$ by theorem \ref{theorem:model_typing_record_introduction} 
      \item \Z\Z $\forall e\ \Gamma .\ \Omega, \Gamma \satisfies e \hastype \tau_l \implies \Omega, \Gamma \satisfies e \hastype 
        \code{$l$->($\tau_{rl}$\&$\tau_{rr}$)}
      $ by implication and generalization
      \item \Z\Z $\Omega \satisfies \tau_l \subtypes \code{$l$->($\tau_{rl}$\&$\tau_{rr}$)}$
      by definition 
      \item \Z\Z $\exists \Omega .\ \Omega \satisfies \tau_l \subtypes \tau_r$ by substitution and witness 
...
\begin{theorem}(Model typing implication elimination)
  ...
  \TODO{Must redo using new implication rule}
  \begin{mathpar}
    \inferrule { 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    $
      ...
      \item \Z\Z $\B{case } 
        (\code{$F$($e_1$)})[\Sigma] \rightsquigarrow e'
        \I
        \Omega, \Gamma \satisfies e' \hastype \tau_r
      $
      \item \Z\Z $\B{wrt } e'$
        \item \Z\Z\Z $(\code{$F$($e_1$)})[\Sigma] = \code{$F[\Sigma]$($e_1[\Sigma]$)}$ by definition
        \item \Z\Z\Z $\code{$F[\Sigma]$($e_1[\Sigma]$)} \rightsquigarrow e'$ by substitution 
        \item \Z\Z\Z $(\code{$F[\Sigma]$\$$p$=>$e[\Sigma]\backslash\text{FV}(p)$)($e_1[\Sigma]$)} \rightsquigarrow e'$ by definition 

        \item \Z\Z\Z $(\code{($F$\$$p$=>$e$)($e_1$)})[\Sigma] = \code{($F[\Sigma]$\$$p$=>$e[\Sigma\backslash\text{FV}(p)]$)($e_1[\Sigma]$)}
        $ by definition
        \item \Z\Z\Z $(\code{($F$\$$p$=>$e$)($e_1$)})[\Sigma] \rightsquigarrow e'$ by substitution 
...
\begin{theorem}(Model typing function progress)
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \exists e' \Sigma .\ \code{$F$($e$)}[\Sigma] \rightsquigarrow e' \land \Omega, \Gamma \satisfies e' \hastype \tau_r
    } 
  \end{mathpar}
  Proof:

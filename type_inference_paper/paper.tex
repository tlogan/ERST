...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \Omega}\ Proof Typing
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      \text{FTV}(\Gamma)\ \vec{\alpha} \entails \vec{\pi} \cong^+ \vec{\tau} 
    } {
    ...
    \inferrule {
      ...
      \\\\
      \forall \vec{\alpha}'\ \Delta' .\ 
        \vec{\alpha}', \llbracket \vec{\alpha}\ \vec{\alpha}' \entails \Delta', \alpha_l \obj{->} \alpha_r \rrbracket^+ \in \vec{\pi}_\nu
      \implies
      \tau \subtypes \alpha_l\obj{->}\alpha_r \given  \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
      \\\\
      ...
      \\
      \text{FTV}(\Gamma)\ \vec{\alpha}\ \alpha_\mu \entails \vec{\pi}_\mu \cong^- \vec{\tau}
    } {
      \Gamma \entails \obj{loop(}e\obj{)} \hastype \obj{ALL[}\alpha_x \obj{]}\alpha_x\obj{->EXI[}\alpha_y \obj{;} \alpha_x \obj{*} \alpha_y
      \obj{<:LFP[} \alpha_\mu \obj{]}\text{union}(\vec{\tau})\obj{]}\alpha_y \given \vec{\alpha}, \Delta 
    }\lab{9}
...
\begin{definition}\boxed{\tau \subtypes \tau \given \Omega}\ Proof Subtyping
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \TODO{confirm that it's safe to remove constraints. update implementation to match}
    } {
      \alpha \subtypes \tau_r
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi} \sim \vec{\eta}}\ Function Lifting
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \vec{\alpha}', \llbracket \vec{\alpha}\ \vec{\alpha}' \entails \Delta', \tau_l \obj{->} \tau_r \rrbracket \in \vec{\pi}'
      \implies
...
\begin{definition}\boxed{\vec{\alpha} \entails \vec{\pi} \cong^\pm \vec{\tau}}\ Multiple Constraint Congruence 
...
}\ Polar Type Interpretation 
\label{def:polar_type_interpretation}
...
\begin{definition}\boxed{\vec{\alpha}, \vec{\alpha}, \Delta \entails \tau \cong^{\pm} \tau} Type Packaging 
  \begin{mathpar}
    \inferrule {

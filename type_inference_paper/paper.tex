...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      \text{factorPair}(\obj{<:LFP[} \alpha_\mu \obj{]}\tau_\mu) = (\tau_l, \tau_r)
    } {
...
% \hfill
% \begin{definition} Proof Subtyping (Implication Rewriting)
%   \\
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      \text{split}^-(\Delta[\alpha_\open\slash\tau_r]^{+} \backslash \Delta) = (\Delta_\closed, \Delta_\open)
      \\
    ...
    \inferrule {
      ...
      \\\\
      \exists \Delta' .\ \llbracket \vec{\alpha}_\closed \entails \Delta, \alpha_\closed \rrbracket^- = (\Delta', \tau_l)
      \\\\
      \exists \Omega .\ 
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \alpha_\open \notin \vec{\alpha}_\closed
      \\\\
      \text{split}^+(\Delta[\alpha_\open\slash\tau_l]^{-} \backslash \Delta) = (\Delta_\closed, \Delta_\open)
      \\
      \Delta_\open \given \vec{\alpha}_\closed', \Delta'
    } {
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \phi \subtypes \tau_{l} \given \Omega 
    } {
      \phi \subtypes \tau_{l}\obj{|}\tau_{r} \given \Omega 
    }

    \inferrule {
      \phi \subtypes \tau_{r} \given \Omega 
    } {
      \phi \subtypes \tau_{l}\obj{|}\tau_{r} \given \Omega 
    }
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \text{pack}^-(\text{FTV}(\Gamma)\ \vec{\alpha}\ \alpha_\mu \entails \vec{\pi}_\mu) = \tau_\mu
      \\
      \exists \tau_r .\ \text{factorPair}(\obj{<:LFP[} \alpha_\mu \obj{]}\tau_\mu) = (\tau_l, \tau_r)
    } {
      \Gamma \entails \obj{loop(}e\obj{)} 
      \hastype 
      \obj{ALL[}\alpha_x \obj{;} \alpha_x \obj{<:} \tau_l \obj{]}
      \alpha_x
      \obj{->} 
      \obj{EXI[}\alpha_i\ \alpha_y 
          \obj{;} \alpha_i \obj{<:} \alpha_x 
          \obj{;} \alpha_i \obj{*} \alpha_y 
...
\begin{definition} 
  ...
  \hfill 
  \boxed{\phi \subtypes \obj{LFP[}\alpha\obj{]}\tau \given \Omega}
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{definition} 
  \label{def:proof_subtyping_variable_elimination}
  Rel Proof Subtyping (Closed Variable Elim)
  \hfill
  \boxed{\alpha \subtypes \tau \given \Omega} \ \textbf{where}\ $\nexists \nu.\ \tau = \nu$
  \\
  \begin{mathpar}
    \inferrule {
      ...
      \nexists \alpha.\ \tau_r = \alpha 
      \\\\
      \exists \Delta' .\ \llbracket \vec{\alpha}_\closed \entails \Delta\ \text{factor}(\Delta, \alpha_\closed), \alpha_\closed \rrbracket^- = (\Delta', \tau_l)
      \\\\
      \exists \Omega .\ 
      (\vec{\alpha}_\closed, \Delta) \preceq \Omega \land
      \tau_l \subtypes \tau_r \given \Omega 
...
\begin{definition} 
  ...
  \\\\
  \boxed{
    \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau \rrbracket^\pm = (\Delta', \tau)
  }
  \\
  \begin{mathpar}
    \inferrule {
      ...
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \alpha \rrbracket^\pm = (\Delta', \tau)
    }

    \inferrule {
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \obj{@} \rrbracket^\pm = (\Delta, \obj{@})
    }

    \inferrule {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau \rrbracket^\pm = (\Delta', \tau')
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \obj{<}l\obj{>}\tau \rrbracket^\pm = (\Delta', \obj{<}l\obj{>}\tau')
    }

    \inferrule {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau \rrbracket^\pm = (\Delta', \tau')
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, l\obj{->}\tau \rrbracket^\pm = (\Delta', l\obj{->}\tau')
    }

    \inferrule {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_r \rrbracket^\pm = (\Delta_r, \tau_r')
      \\\\
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_l \rrbracket^{\text{flip}(\pm)} = (\Delta_l, \tau_l')
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_l\obj{->}\tau_r \rrbracket^\pm = (\Delta_l \cap \Delta_r, \tau_l'\obj{->}\tau_r')
    }

    \inferrule {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_l \rrbracket^\pm = (\Delta_l, \tau_l')
      \\\\
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_r \rrbracket^{\pm} = (\Delta_r, \tau_r')
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_l\obj{|}\tau_r \rrbracket^\pm = (\Delta_l \cap \Delta_r, \tau_l'\obj{|}\tau_r')
    ...
    \inferrule {
      \llbracket \vec{\alpha}_\foreign \cup \vec{\alpha}, \vec{\alpha}_\closed \entails \Delta, \Delta_q \rrbracket^\pm = (\Delta', \Delta_q')
      \\\\
      \llbracket \vec{\alpha}_\foreign \cup \vec{\alpha}, \vec{\alpha}_\closed \entails \Delta, \tau \rrbracket^{\pm} = (\Delta'', \tau')
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \obj{EXI[}\vec{\alpha}\ \Delta_q \obj{]} \tau \rrbracket^\pm 
      = 
    ...
    \inferrule {
      ...
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \obj{LFP[}\alpha \obj{]} \tau \rrbracket^\pm 
      = 
    ...
    \inferrule {
      ...
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \obj{ALL[}\vec{\alpha}\ \Delta_q \obj{]} \tau \rrbracket^\pm 
      = 
    ...
    \inferrule {
      ...
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau\obj{\textbackslash}\eta \rrbracket^\pm = (\Delta', \tau'\obj{\textbackslash}\eta)
    }
  \end{mathpar}
\end{definition}
...
\begin{definition} 
  ...
  \\\\
  \boxed{
    \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \Delta_q \rrbracket^\pm = (\Delta', \Delta_q')
  }
  \\
  \begin{mathpar}
    \inferrule {
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \epsilon \rrbracket^\pm = (\Delta', \epsilon')
    }

    \inferrule {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \Delta_q\rrbracket^\pm = (\Delta', \Delta_q')
      \\\\
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_r \rrbracket^\pm = (\Delta_l, \tau_r')
      \\\\
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \tau_l \rrbracket^{\text{flip}(\pm)} = (\Delta_r \tau_l')
    } {
      \llbracket \vec{\alpha}_\foreign, \vec{\alpha}_\closed \entails \Delta, \Delta_q\ \obj{;} \tau_l \obj{<:} \tau_r \rrbracket^\pm = (\Delta' \cap \Delta_l \cap \Delta_r, \Delta_q'\ \obj{;} \tau_l' \obj{<:} \tau_r')
    }

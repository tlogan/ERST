...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given Z}\ Proof Typing
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \Gamma \entails \obj{@} \hastype \obj{@} \given M, \Delta 
    }

    \inferrule {
      x\obj{:}\tau \in \Gamma
    } {
      \Gamma \entails x \hastype \tau \given M, \Delta 
    }

    \inferrule {
      \Gamma \entails e \hastype \tau \given M, \Delta 
    } {
      \Gamma \entails \obj{<}l\obj{>}e \hastype \obj{<}l\obj{>}\tau \given M, \Delta 
    }

    \inferrule {
      \Gamma \entails R \hastype \tau_0 \given M, \Delta 
      \\
      \Gamma \entails e \hastype \tau_1 \given M\ M', \Delta\ \Delta' 
    } {
      \Gamma \entails R\ \obj{\$}l\obj{=>}e \hastype \tau_0\ \obj{\&}\ l\obj{->}\tau_1 \given M\ M, \Delta\ \Delta' 
    }

    \inferrule {
      M, \Delta, \Gamma \entails F \liftfun \Pi, \Tau_n 
      \\
      \Gamma \entails \vec{\pi} \cong \vec{\tau} 
    } {
      \Gamma \entails F \hastype \text{inter}(\vec{\tau}) \given M, \Delta 
    }

    \inferrule {
      \Gamma \entails e \hastype \tau_0 \given M, \Delta
      \\
      \tau_0 \subtypes l\obj{->}\alpha \given M\ M', \Delta\ \Delta'
    } {
      \Gamma \entails e\obj{.}l \hastype \alpha \given M\ M', \Delta\ \Delta'
    }

    \inferrule {
      \Gamma \entails e_0 \hastype \tau_0 \given M_0, \Delta_0 
      \\
      \Gamma \entails e_1 \hastype \tau_1 \given M_0\ M_1, \Delta_0\ \Delta_1
      \\\\
      \tau_0 \subtypes \tau_1\obj{->}\alpha \given M_0\ M_1\ M_2, \Delta_0\ \Delta_1\ \Delta_2
    } {
      \Gamma \entails e_0\obj{(}e_1 \obj{)} \hastype \alpha \given M_0\ M_1\ M_2, \Delta_0\ \Delta_1\ \Delta_2
    }

    \inferrule {
      \Gamma \entails e \hastype \obj{$\alpha_{h^+}$->$\tau$} \given M, \Delta
      \\\\
      \exists z.\ z \in Z 
      \\\\
      \forall M'\ \Delta' .\ \left< M\ M', \Delta\ \Delta' \right> \in Z \iff \tau \subtypes \obj{$\alpha_l$->$\alpha_r$} \given M\ M', \Delta\ \Delta' 
      \\\\
      \text{FTV}(\Gamma) \entails \alpha_{h^+} \cdot Z \cdot \obj{$\alpha_l$->$\alpha_r$} \fallingdotseq \alpha_{h^-} \cdot \Tau
    } {
      \Gamma \entails \obj{loop($e$)} \hastype \obj{ALL[$\alpha_l'$]$\alpha_l'$->EXI[$\alpha_r'$;$\alpha_l'$*$\alpha_r'$<:LFP[$\alpha_{h^-}$]}\text{union}(\Tau)\obj{]]$\alpha_r'$} \given M, \Delta 
    }
  \end{mathpar}
\end{definition}
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\eta}}\ Function Lifting
  \begin{mathpar}
    ...
    \inferrule {
      ...
      M, \Delta, \Gamma \entails \vec{f}\ \obj{\$}p\obj{=>}e \liftfun 
      \vec{\pi}\ \vec{\pi}', \vec{\eta}\ (\obj{EXI[}\text{range}(\Gamma')\obj{]}\phi)
    }
  \end{mathpar}
\end{definition}

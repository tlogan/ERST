...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \Omega}\ Proof Typing
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      \text{pack}^-(\text{FTV}(\Gamma)\ \vec{\alpha}\ \alpha_\mu \entails \vec{\pi}_\mu) = \tau'
    } {
      \Gamma \entails \obj{loop(}e\obj{)} \hastype \obj{ALL[}\alpha_x \obj{]}\alpha_x\obj{->EXI[}\alpha_y \obj{;} \alpha_x \obj{*} \alpha_y
      \obj{<:LFP[} \alpha_\mu \obj{]}\tau'\obj{]}\alpha_y \given \vec{\alpha}, \Delta 
    }\lab{9}
...
\begin{definition} Proof Subtyping (Basic Preservation)
  \\
...
\begin{definition} Proof Subtyping (Rewriting)
  \\
  \boxed{\tau \subtypes \tau \given \Omega}

  \label{def:proof_subtyping_rewriting}
  \TODO{these should be subsumed by other rules; double check necessity in implementation}
  \begin{mathpar}
  %   \inferrule {
  %     \tau_l \subtypes
...
\begin{definition} Proof Subtyping (Abstraction Elimination)
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \obj{BOT} \subtypes \tau_r \given \Omega 
    }
    ...
    \inferrule {
      ...
      \\\\
      \forall \Omega' .\ \tau_l \subtypes \tau_r \given \Omega' \implies (\vec{\alpha}\ \vec{\alpha}_l, \Delta) \preceq \Omega' \implies \Omega' = \Omega
    } {
...
\begin{definition} Proof Subtyping (Refinement Introduction)
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \tau_l \subtypes \obj{TOP} \given \Omega 
    }

    \inferrule {
      \tau_{l} \subtypes \tau_{rl}
      \given \Omega 
      \\
      \tau_{l} \subtypes \tau_{rr}
      \given \Omega' 
      \\
      \Omega \preceq \Omega'
    } {
    ...
    \inferrule {
      ...
      \\\\
      \forall \Omega' .\ \tau_l \subtypes \tau_r \given \Omega' \implies (\vec{\alpha}\ \vec{\alpha}_r, \Delta) \preceq \Omega' \implies \Omega' = \Omega
    } {
...
\begin{definition} Proof Subtyping (Variable Elimination)
  ...
  \begin{mathpar}
    \inferrule {
      (\vec{\alpha}, \Delta\ \Delta_\sharp) \preceq (\vec{\alpha}', \Delta')
      \\ 
      \alpha \notin \vec{\alpha}
      \\\\
      \text{split}^-(\Delta[\alpha\slash\tau_r]^{+} \backslash \Delta) = (\Delta_\sharp, \Delta_\flat)
      \\
      \Delta_\flat \given \vec{\alpha}', \Delta'
    } {
      \alpha \subtypes \tau \given (\vec{\alpha}', \Delta'\ \obj{;} \alpha\obj{<:}\tau_r) 
    }

    \inferrule {
      \alpha \in \vec{\alpha} 
      \\
      \forall \alpha.\ \tau_r = \alpha \implies \alpha \in \vec{\alpha} 
      \\\\
      \exists \Delta' .\ \llbracket \vec{\alpha} \entails \Delta\ \text{factor}(\Delta, \alpha), \alpha \rrbracket^- = (\Delta', \tau_l)
      \\
      \exists \Omega .\ 
...
\begin{definition} Proof Subtyping (Variable Introduction)
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \alpha \notin \vec{\alpha}
      \\\\
      \text{split}^+(\Delta[\alpha\slash\tau_l]^{-} \backslash \Delta) = (\Delta_\sharp, \Delta_\flat)
      \\
      \Delta_\flat \given \vec{\alpha}', \Delta'
    } {
      \tau_l \subtypes \alpha \given (\vec{\alpha}', \Delta'\ \obj{;} \tau_l\obj{<:}\alpha) 
    }
    ...
    \inferrule {
      ...
      \\\\
      \exists \Delta' .\ \llbracket \vec{\alpha} \entails \Delta, \alpha \rrbracket^+ = (\Delta', \tau_r)
      \\
      \exists \Omega .\ 
      (\vec{\alpha}, \Delta) \preceq \Omega \land \tau_l \subtypes \tau_r \given \Omega 
    } {
...
\begin{definition} Proof Subtyping (Refinement Elimination)
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \tau_{ll} \subtypes \tau_r \given \Omega
    } {
      \tau_{ll}\obj{\&}\tau_{lr}  \subtypes \tau_r \given \Omega 
    }

    \inferrule {
      \tau_{lr} \subtypes \tau_r \given \Omega 
    } {
      \tau_{ll}\obj{\&}\tau_{lr}  \subtypes \tau_r \given \Omega 
    }

    \inferrule {
      \tau_{l} \subtypes \tau_r \obj{|} \eta \given \Omega
    } {

...
\begin{theorem}(Proof typing soundness)
  ...
  $ by theorem \ref{theorem:proof_subtyping_soundness}
  \item \Z\Z $
    \forall e .\ \Omega, \Gamma \satisfies e \hastype \tau' \implies \Omega \Gamma \satisfies e \hastype \code{$\alpha_0$->$\alpha_1$->$\alpha_2$}
  $ by theorem \ref{theorem:model_subtyping_inversion}
  \item \Z\Z $
   \Omega, \Gamma \satisfies e' \hastype \tau' \implies \Omega, \Gamma .\ e' \hastype \code{$\alpha_0$->$\alpha_1$->$\alpha_2$}
  $ by instantiation 
  \item \Z\Z $
   \Omega, \Gamma \satisfies e' \hastype \code{$\alpha_0$->$\alpha_1$->$\alpha_2$}
  $ by application 
  \item \Z\Z $
  ...
  $ by theorem \ref{theorem:model_typing_fix_elimination} \TODO{...}
  \item \Z\Z $
    \Omega, \Gamma \satisfies e \hastype \tau
  $ by substitution 
...
% \begin{theorem}(Model typing fixpoint sketch)
%   \label{theorem:model_typing_fixpoint_sketch}
%   \begin{mathpar}
%     \inferrule { 
%       \Omega \satisfies \code{($\alpha_1$,$\alpha_2$)} \subtypes \code{FIX[$\alpha_3$.|$(\Tau_{rel})$]}
%     } {
%       \Omega \satisfies \code{ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\Tau_{rel})$]]$\alpha_5$} 
%       \subtypes \code{$\alpha_1$->$\alpha_2$} 
%     } 
%   \end{mathpar}
%   Proof: 
%   \item \TODO{...} 
...
% \begin{theorem}(Proof typing fixpoint idea)
%   \label{theorem:model_typing_fixpoint_ideai}
%   \begin{mathpar}
%     \inferrule { 
%       \alpha_0 \subtypes \code{$\alpha_1$->$\alpha_2$} \given Z
%       \\
%       \text{FTV}(\Gamma) \entails \alpha_0 \circlearrowleft Z \cdot \code{$\alpha_1$->$\alpha_2$} \fallingdotseq \alpha_3 \circlearrowright \Tau_{rel}
%     } {
%       \code{($\alpha_1$,$\alpha_2$)} \subtypes \code{FIX[$\alpha_3$.|$(\Tau_{rel})$]} \given Z
%     } 
%   \end{mathpar}
%   Proof: 
%   \item \TODO{...} 
...
% \begin{theorem}(Model typing fixpoint duality)
%   \label{theorem:model_typing_fixpoint_duality}
%   \begin{mathpar}
%     \inferrule { 
%       \left< M, \Delta \right> \in Z
%       \\
%       \left< M, \Delta \right> \leadsto \Omega 
%       \\\\
%       % \Omega \satisfies \alpha_0 \subtypes \code{$\alpha_1$->$\alpha_2$} 
%       % \\
%       \text{FTV}(\Gamma) \entails \alpha_0 \circlearrowleft Z \cdot \code{$\alpha_1$->$\alpha_2$} \fallingdotseq \alpha_3 \circlearrowright \Tau_{rel}
%     } {
%       \Omega \satisfies \code{($\alpha_1$,$\alpha_2$)} \subtypes \code{FIX[$\alpha_3$.|$(\Tau_{rel})$]}
...
\TODO{note that $\alpha_1$ and $\alpha_2$ could have further constraints in terms of $\alpha_0$ ; therefore the constraint here implies type recursion} 

\TODO{interpretation of the $\alpha_0$ seems to be missing from the rule in paper} 

\TODO{can't rely $\alpha_1$->$\alpha_2$; it's overly specialized in particular model typing world} 

\TODO{can't rely $\tau \subtypes \alpha_0$->$\alpha_1$->$\alpha_2$ gets wrapped up; so can't be reduced to a single choice in $Z$; very different from application, i.e. implication elimination} 

% \begin{theorem}(Model typing implication fixing)
%   \label{theorem:model_typing_implication_fixing}
%   \begin{mathpar}
%     \inferrule { 
%       \Omega, \Gamma \satisfies e \hastype \code{$\alpha_0$->$\alpha_1$->$\alpha_2$}
%       (WRONG!!!)
...
%       \Omega \satisfies \code{($\alpha_1$,$\alpha_2$)} \subtypes \code{FIX[$\alpha_3$.|$(\Tau_{rel})$]}
%     } {
%       \Omega, \Gamma \satisfies \code{fix($e$)} \hastype \code{ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\Tau_{rel})$]]$\alpha_5$} 
%     } 
%   \end{mathpar}
%   Proof: 
%   \item \TODO{...} 
...
\begin{theorem}(Proof subtyping strongest post-condition interpolation)
  ...
  \begin{mathpar}
    \inferrule { 
      ...
      \tau \subtypes \code{$\alpha_0$->$\alpha_1$->$\alpha_2$} \given Z_1
      \\\\
      \text{FTV}(\Gamma) \entails \alpha_0 \circlearrowleft Z_1 \cdot \code{$\alpha_1$->$\alpha_2$} \fallingdotseq \alpha_3 \circlearrowright \Tau_{rel}
    } {
      \tau
      \subtypes
      \code{BOT->ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\Tau_{rel})$]]$\alpha_5$} 
      \given Z_0
    } 
  \end{mathpar}
  Proof: 
...
\begin{theorem}(Model typing implication vacuity elimination)
  \label{theorem:model_typing_vacuous_vacuity_elimination}
  \begin{mathpar}
    \inferrule { 
      \Omega, \Gamma \satisfies e \hastype \code{BOT->$\tau$} 
    } {
      \Omega, \Gamma \satisfies \code{fix($e$)} \hastype \tau
    } 
  \end{mathpar}
  Proof: 
  \item \TODO{...} 

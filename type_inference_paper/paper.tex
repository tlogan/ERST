...
\begin{definition} \boxed{\Omega, \Gamma \entails e \hastype \tau \with \Omega}\ Proof Typing
  \label{def:proof_typing}
  \begin{mathpar}
    \inferrule {
    } {
      \Omega, \Gamma \entails \obj{@} \hastype \obj{@} \with \Omega 
    }\lab{1}
    ...
    \inferrule {
      \Omega, \Gamma \entails e \hastype \tau \with \Omega' 
    } {
    ...
    \inferrule {
      ...
      \Omega, \Gamma \entails \vec{r} \hastype \obj{TOP} \with \Omega
    }\lab{4}

    \inferrule {
      \Omega, \Gamma \entails \vec{r} \hastype \tau \with \Omega'
      \\
      \Omega', \Gamma \entails e \hastype \tau' \with \Omega'' 
    } {
      \Omega, \Gamma \entails \vec{r}\ \obj{\$}l\obj{=>}e \hastype \tau\ \obj{\&}\ l\obj{->}\tau' \with \Omega'' 
    }\lab{5}
    ...
    \inferrule {
      ...
      (\vec{\alpha}, \Delta), \Gamma \entails \vec{f} \hastype \text{inter}(\vec{\tau}) \with \vec{\alpha}, \Delta
    }\lab{6}

    \inferrule {
      \Omega, \Gamma \entails e \hastype \tau_0 \with \Omega'
      \\
      \Omega' \entails \tau_0 \subtypes l\obj{->}\alpha \with \Omega'' 
    } {
      \Omega, \Gamma \entails e\obj{.}l \hastype \alpha \with \Omega''
    }\lab{7}

    \inferrule {
      \Omega, \Gamma \entails e_f \hastype \tau_f \with \Omega' 
      \\
      \Omega', \Gamma \entails e_a \hastype \tau_a \with \Omega''
      \\\\
      \Omega'' \entails \tau_f \subtypes \tau_a\obj{->}\alpha \with \Omega''' 
    } {
      \Omega, \Gamma \entails e_f\obj{(}e_a \obj{)} \hastype \alpha \with \Omega'''
    }\lab{8}

    \inferrule {
      (\vec{\alpha}, \Delta), \Gamma \entails e \hastype \alpha^+\obj{->}\tau \with \vec{\alpha}', \Delta' 
      \\\\
      \forall \vec{\alpha}''\ \Delta'' .\ 
       (\vec{\alpha}'', \Delta'') \in \vec{\Omega}
      \implies
      (\vec{\alpha}', \Delta') \entails \tau \subtypes \alpha_l\obj{->}\alpha_r 
      \with \vec{\alpha}'', \Delta'' 
      \\\\
      ...
      (\vec{\alpha}, \Delta), \Gamma \entails \obj{loop(}e\obj{)} \hastype \obj{ALL[}\alpha_x \obj{]}\alpha_x\obj{->EXI[}\alpha_y \obj{;} \alpha_x \obj{*} \alpha_y
      \obj{<:LFP[} \alpha^- \obj{]}\text{union}(\vec{\tau})\obj{]}\alpha_y 
      \with \vec{\alpha}', \Delta '
    }\lab{9}
    ...
    \inferrule {
      \Omega, \Gamma \entails e \hastype \tau \with \Omega' 
      \\
      \Omega' \entails \tau \subtypes \tau_a \with \Omega'' 
      \\\\
      \Omega'', \Gamma\ x:\tau_x \entails e' \hastype \tau' \with \Omega'''
    } {
      \Omega \entails
      \Gamma \entails \obj{let}\ x \obj{:} \tau_a\ \obj{=}\ e\ \obj{in}\ e' \hastype \tau' 
      \with \Omega'''
    }\lab{10}
  \end{mathpar}
\end{definition}
...
% subtyping constraints that satisfy
...
\begin{definition}\boxed{\Omega \entails \tau \subtypes \tau \with \Omega}\ Proof Subtyping
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \Omega \entails \tau_l[\alpha\slash\tau_r] \subtypes \tau_r \with \Omega' 
    } {
    ...
    \inferrule {
      ...
      % \\\\
      \Omega \entails \tau_l \subtypes \tau_r \with \Omega' 
      \\
      \text{FTV}(\eta) \subseteq \epsilon
      \\\\
      \nexists \Omega'' .\ \Omega' \entails \tau_l \subtypes \eta \with \Omega''
    } {
    ...
    \inferrule {
      \Omega \entails \tau_{ll} \subtypes \tau_r
      \with \Omega'
      \\
      \Omega' \entails \tau_{lr} \subtypes \tau_r
      \with \Omega'' 
    } {
    ...
    \inferrule {
      \Omega \entails \Delta \with \vec{\alpha}_w, \Delta_w
      \\
      \vec{\alpha} \notfree \tau_r
      \\\\
      (\vec{\alpha}_w\ \vec{\alpha}, \Delta_w) \entails \tau_l \subtypes \tau_r \with \Omega'
    } {
    ...
    \inferrule {
      ...
      \\\\
      (\vec{\alpha}, \Delta\ \Delta_f) \entails \text{inter}(\vec{\tau}) \subtypes \tau_r 
      \with \vec{\alpha}', \Delta'
    } {
      \vec{\alpha}, \Delta \entails \alpha \subtypes \tau_r
      \with \vec{\alpha}', \Delta'
    }\lab{6}
    ...
    \inferrule {
      ...
      \\\\
      (\vec{\alpha}, \Delta\ \Delta^\sharp) \entails \Delta^\dagger\ \tau_l\obj{<:}\text{inter}(\vec{\tau}) 
      \with \vec{\alpha}', \Delta'
    } {
      \vec{\alpha}, \Delta \entails \tau_l \subtypes \alpha 
      \with (\vec{\alpha}', \Delta'\ \tau_l\obj{<:}\alpha) 
    }\lab{7}

    \inferrule {
      \Omega \entails \tau_l \subtypes \tau_r \with \Omega'
      \\
      \Omega' \entails \Delta \with \Omega'' 
    } {
      \Omega \tau_l \subtypes 
      \obj{EXI[}\vec{\alpha}\ \Delta \obj{]}\tau_r 
      \with \Omega''
    }\lab{8}

    \inferrule {
      \Omega \entails \tau_l \subtypes \tau_{rl} \with \Omega' 
    } {
      \Omega \entails \tau_l \subtypes \tau_{rl}\obj{|}\tau_{rr} \with \Omega'
    }\lab{9}

    \inferrule {
      \Omega \entails \tau_l \subtypes \tau_{rr} \with \Omega' 
    } {
      \Omega \entails \tau_l \subtypes \tau_{rl}\obj{|}\tau_{rr} \with \Omega'
    }\lab{10}

    \inferrule {
      \Omega \entails \tau_l \subtypes \tau_r\obj{|}\eta \with \Omega'
    } {
      \Omega \entails \tau_l\obj{\textbackslash}\eta \subtypes \tau_r \with \Omega'
    }\lab{11}

    \inferrule {
      \Omega \entails \tau_l \circlearrowleft \obj{LFP[} \alpha \obj{]} \tau_r
      \\
      \Omega \entails \tau_l \subtypes \tau_r[\alpha \slash \obj{LFP[} \alpha \obj{]} \tau_r]
      \with \Omega'
    } {
      \Omega \entails \tau_l \subtypes \obj{LFP[} \alpha \obj{]}\tau_r \with \Omega'
    }\lab{12}
    ...
    \inferrule {
      ...
      \\
      (\vec{\alpha}, \Delta) \entails \tau_n \subtypes \obj{LFP[}\alpha \obj{]} \tau' \entails \Omega
    } {
      (\vec{\alpha}, \Delta) \entails
    ...
    \inferrule {
      ...
      \\\\
      \vec{\alpha} \entails \Delta \wr \text{FTV}(\phi) \entails \vec{\delta}
      \\
      (\vec{\alpha}, \Delta) \entails
      ...
      (\vec{\alpha}, \Delta) \entails
      \phi \subtypes \obj{LFP[}\alpha\obj{]}\tau
      \with \vec{\alpha}', \Delta'\ \obj{;} \phi \obj{<:} \obj{LFP[}\alpha\obj{]}\tau
    }\lab{14}
    \\
    ...
    \inferrule {
      ...
      \Omega \entails l \obj{->} \tau_l \subtypes l \obj{->} \tau_r \with \Omega' 
    }\lab{16}

    \inferrule {
      \Omega \entails \tau_{rl} \subtypes \tau_{ll} \entails \Omega'
      \\
      \Omega' \entails \tau_{lr} \subtypes \tau_{rr} \entails \Omega'' 
    } {
      \Omega \entails
      \tau_{ll} \obj{->} \tau_{lr} \subtypes \tau_{rl} \obj{->} \tau_{rr}
      \with \Omega''
    }\lab{17}
  \end{mathpar}
\end{definition}
...
\begin{definition}\boxed{\tau \subtypes \tau \given M, \Delta}\ Continued Proof Subtyping 
  \label{def:continued_proof_subtyping}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition}\boxed{\vec{\alpha}, \vec{\alpha} \entails \vec{\pi} \cong \vec{\tau}}\ Constrained Implication Congruence 
  \label{def:constrained_implication_congruence}
  \begin{mathpar}
    \inferrule {
    } {

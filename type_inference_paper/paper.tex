...
\tau ::= &\C{type}
  ...
  \\
  \I \code{EXI[$\widebar{\alpha}$ $Q$] $\tau$} &\C{indexed union}
  \\
...
\begin{array}[t]{l r}
  ...
  \\
r ::= \code{;$l$=$e$} \sep r\ \code{;$l$=$e$} &\C{record}
  \\
f ::= \code{case $p$=>$e$} \sep f\ \code{case $p$=>$e$} &\C{function}
  \\
  \\
p ::= &\C{pattern}
  \\
  \I \code{$x$} &\C{variable pattern}
  \\
  \I \code{@} &\C{unit pattern}
  \\
  \I \code{\textasciitilde{}$l$ $p$} &\C{tag pattern}
  \\
  \I b &\C{record pattern}
  \\
  \I \code{$p$,$p$} &\C{tuple pattern}
  \\
  \I \code{($p$)} &\C{association pattern}
  \\
b ::= \code{;$l$=$p$} \sep b\ \code{;$l$=$p$} &\C{record pattern}
  \\
  \\
v ::= &\C{value}
  \\
  ...
  \I \code{$v$,$v$} &\C{tuple}
  \\
  \I \mathbb{W} &\C{function}
  \\
  \I \code{($v$)} &\C{association}
  \\
  \\
\vec{\alpha} ::= \epsilon \sep \vec{\alpha}\ \alpha &\C{collection of type variables}
  \\
\mathbb{V} ::= \epsilon \sep \mathbb{V} \cdot v &\C{collection of values}
  \\
\mathbb{W} ::= \epsilon \sep \mathbb{W} \cdot (v,v) &\C{collection of value-value pairs}
  \\
\mathbb{R} ::= \epsilon \sep \mathbb{R} \cdot (l,v) &\C{collection of label-value pairs}
  \\
\mathbb{H} ::= \epsilon \sep \mathbb{H} \cdot x \mapsto v &\C{collection of value bindings}
  \\
\Gamma ::= \epsilon \sep \Gamma \cdot \code{$x$:$\tau$} &\C{collection of typings}
  \\
\Delta ::= \epsilon \sep \Delta \cdot \code{$\tau$<:$\tau$} &\C{collection of subtypings}
  \\
c ::= \code{$x$:$\tau$} \sep \code{$\tau$<:$\tau$} &\C{collectable}
  \\
% \mathbb{C} ::= \mathbb{H} \sep \Gamma &\C{collection}
C ::= \epsilon \sep C \cdot c \sep C\ c &\C{collection of collectables}
%   \\
%   \\
% f ::= &\C{function}
%   \\
...
\begin{mathpar}
  ...
  \inferrule[Tag] {
    ...
    \code{\textasciitilde{}$l$ $e$} \longrightarrow \code{\textasciitilde{}$l$ $e'$}
  }

  \inferrule[RemainderRecord] {
    e \longrightarrow e'
  } {
    \code{;$l$=$v$} :: e \longrightarrow \code{;$l$=$v$} :: e' 
  }

  \inferrule[FirstRecord] {
    e \longrightarrow e'
  } {
    \code{;$l$=$e$} :: e_r \longrightarrow \code{;$l$=$e'$} :: e_r
  }


  \inferrule[FirstProjection] {
    e \longrightarrow e'
  } {
    \code{$e$.$l$} \longrightarrow \code{$e'$.$l$}
  }

  \inferrule[Projection] {
    \lor_i(i = j)
  } {
    \widebar{\code{;$l_i$=$v_i$}}^i\code{.$l_j$} \longrightarrow v_j
  }

  \inferrule[FirstApplication] {
    e \longrightarrow e'
  } {
    \code{($e$)($e_a$)} \longrightarrow \code{($e'$)($e_a$)}
  }


  \inferrule[MatchedApplication] {
    unify(p,v) = \mathbb{H}
  } {
    \code{(case $p$=>$e$ $::$ $e_r$)($v$)} \longrightarrow subst(e, \mathbb{H})
  }

  \inferrule[NextApplication] {
    unify(p, v) \neq \mathbb{H}
    \\
    \code{($cases$)($v$)} \longrightarrow e_b 
  } {
    \code{(case $p$=>$e_a$ $::$ $e_r$)($v$)} \longrightarrow e_b 
  }

  \inferrule[FirstBinding] {
    e \longrightarrow e'
  } {
    \code{let $x$:$\tau$ = $e$ in $e_k$} \longrightarrow \code{let $x$:$\tau$ = $e'$ in $e_k$}
  }

  \inferrule[Binding] {
  } {
    \code{let $x$:$\tau$ = $v$ in $e$} \longrightarrow subst(e, \{x \mapsto v\})
  }

  \inferrule[FirstRecursion] {
    e \longrightarrow e'
  } {
    \code{fix(e)} \longrightarrow \code{fix(e')}
  }

  \inferrule[Recursion] {
  } {
    \code{fix(case $x$ => $e$)} \longrightarrow subst(e, \{x \mapsto \code{fix(case $x$ => $e$)}\})
  }

  \inferrule[Tuple] {
    \code{;left=$e_a$ ;right=$e_b$} \longrightarrow e' 
  } {
    \code{$e_a$,$e_b$} \longrightarrow e' 
  }

  \inferrule[Funneling] {
    \code{($e_b$)($e_a$)} \longrightarrow e' 
  } {
    \code{$e_a$ |> $e_b$} \longrightarrow e' 
  }
...
\begin{array}[t]{@{}l}
\{ \mathbb{R} \ |\ \forall\ v . 
\\ \I (l, v) \in \mathbb{R} \rightarrow v \in \llbracket \tau \rrbracket
\\
\}
\end{array}
& \C{field} 
...
&=& 
\begin{array}[t]{@{}l}
\{ \mathbb{W} \ |\ \forall\ (v_a, v_c) \in \mathbb{W} .
\\ \I v_a \in \llbracket \tau_a \rrbracket \rightarrow v_c \in \llbracket \tau_c \rrbracket
\\
\}
\end{array}
& \C{implication} 
...
& \C{difference}
\\
\llbracket \code{EXI[$\widebar{\alpha_i}^i$ $Q$] $\tau$} \rrbracket 
&=& 
\begin{array}[t]{@{}l}
\{ v \ |\ \exists\ \sigma . 
...
\begin{mathpar}
  ...
  \inferrule[LowerAny] {
    ...
    Q
    \\
    \left< \Delta \cup \Delta', \Omega \cup \{\alpha_i\}_i \cup \Omega' \right>
    \claim 
    \tau_l
    \subtypes 
    \tau_u
    ...
    \left< \Delta \cup \Delta', \Omega \cup \{\alpha_i\}_i \cup \Omega' \right>
    \claim 
    \code{(EXI[$\widebar{\alpha_i}^i$ $Q$] $\tau_l$)}
    \subtypes 
    \tau_u
  }
  ...
  \inferrule[UpperAny] {
    ...
    \tau_l
    \subtypes 
    \code{(EXI[$\widebar{\alpha_i}^i$ $Q$] $\tau_u$)}
  }
  ...
  \inferrule[UpperAll] {
    ...
    \tau_l
    \subtypes 
    \code{(ALL[$\widebar{\alpha_i}^i$ $Q$] $\tau_u$)}
  }
...
\begin{mathpar}
  ...
  \inferrule[LowerUpperLFP] {
    subst(\{\alpha \mapsto \code{LFP[$\alpha$|$\tau_u$]}\}, \tau_l) = \tau_l'
    \\
    \left< \Delta, \Omega \right>
  ...
  \inferrule[UnrollUpperLFP] {
    ...
    \\
    subst(\{\alpha \mapsto \code{LFP[$\alpha$|$\tau_u$]}\}, \tau_u) = \tau_u'
    \\
    \left< \Delta, \Omega \right> 
    \claim 
    \tau_l \subtypes \tau_u'
...
\begin{mathpar}
  ...
  \inferrule[LowerUnion] {
    \left< \Delta, \Omega \right> \claim \tau_{la} \subtypes \tau_u
    \\
    \left< \Delta \cup \Delta', \Omega \cup \Omega' \right> \claim \tau_{lb} \subtypes \tau_u
  } {
    \left< \Delta \cup \Delta', \Omega \cup \Omega' \right> \claim \code{$\tau_{la}$|$\tau_{lb}$} \subtypes \tau_u
  }
  ...
  \inferrule[UpperInter] {
    \left< \Delta, \Omega \right> 
    \claim \tau_l \subtypes \tau_{ua}
    \\
    \left< \Delta \cup \Delta', \Omega \cup \Omega' \right> 
    \claim \tau_l \subtypes \tau_{ub}
  } {
    \left< \Delta \cup \Delta', \Omega \cup \Omega' \right> 
    \claim 
    \tau_l 

...
\begin{figure*}[h]
  ...
  \begin{mathpar}
    ...
    \inferrule[Recursion] {
      ...
      \\\\
      \text{FTV}(\Gamma), \vec{W}_1 \entails \alpha_0 \circlearrowleft \code{$\alpha_1$->$\alpha_2$} \rightsquigarrow \tau_{rel}
    } {
      \Gamma \entails \code{fix($e$)} \hastype \code{ALL[$\alpha_7$]$\alpha_7$->EXI[$\alpha_8$;($\alpha_7$,$\alpha_8$)<:$\tau_{rel}$]$\alpha_1$} \given \vec{W}
    }

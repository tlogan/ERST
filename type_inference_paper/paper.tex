...
\begin{lemma}(Elimination of implication of model typing)
  ...
  \item \Z $\textbf{case }
    ...
    \code{case $p$=>$e$} \in F 
    \I
    \forall e' .\ \code{case $p$=>$e'$} \in F \implies e' = e 
    $
  \item \Z $
  ...
  $
  \item \Z\Z $\textbf{let } \Sigma \textbf{ s.t. } \Omega, \Sigma \satisfies \Gamma $ by lemma \TODO{...} 
  \item \Z\Z $e_1[\Sigma] \textbf{ wellformed}$ by lemma \ref{lemma:model_typing_soundness}
  \item \Z\Z $\textbf{case } e_1[\Sigma] = v$ 
  \item \Z\Z\Z $\textbf{let } e_1' \textbf{ s.t. } F\code{(}v\code{)} \rightsquigarrow e_1'$ by \TODO{How do we know that application takes a step}
  \item \Z\Z\Z \TODO{probably by cases on v}
  \item \Z\Z\Z \TODO{may need lemma connection the case containment to traverses the list of cases}
  \item \Z\Z\Z \TODO{May need lemma inducting on F}
...
% and $\Omega, \Gamma \satisfies e_1 \hastype \tau_l$
  \item \TODO{Cretin shows how elimination forms mirror proof typing rules. see Lemma 91, p. 129}
\end{lemma}
...
\begin{lemma}(Progress of pattern matching)
  ...
  \item \Z $\code{case $p$=>$e'$} \in F \lor \code{case $p$=>$e'$} \notin F$ by decidability 
  ...
  \item \Z\Z\Z $\code{case $p$=>$e'$} \in F \implies e' = e$ by instantiation

  \item \Z\Z\Z $\textbf{case } \code{case $p$=>$e'$} \in F$
  \item \Z\Z\Z\Z $e' = e$ by application 
  \item \Z\Z\Z\Z $\code{case $p$=>$e'$} \notin F'$ by substitution
  \item \Z\Z\Z $\textbf{case } \code{case $p$=>$e'$} \notin F$
  \item \Z\Z\Z\Z $\textbf{assume }\code{case $p$=>$e'$} \in F'$
  \item \Z\Z\Z\Z\Z $\code{case $p$=>$e'$} \in F$
  \item \Z\Z\Z\Z\Z $\bot$ by application
  \item \Z\Z\Z\Z $\code{case $p$=>$e'$} \notin F'$ by summarization
  \item \Z\Z\Z $\code{case $p$=>$e'$} \notin F'$ by cases on $\code{case $p$=>$e'$} \in F \lor \code{case $p$=>$e'$} \notin F$

  \item \Z\Z\Z $\textbf{assume } \code{$F'$($v$)} \rightsquigarrow e'$
  \item \Z\Z\Z\Z $\bot$ by inversion
  \item \Z\Z\Z $\neg \code{$F'$($v$)} \rightsquigarrow e'$ by summarization
  \item \Z\Z $\forall e' .\ \neg \code{$F'$($v$)} \rightsquigarrow e'$ by summarization 
  \item \Z\Z $(F'\code{ case $p$=>$e$})\code{(}v\code{)} \rightsquigarrow e[\Sigma]$
  \item \Z\Z $\textbf{let } e' \textbf{ s.t. } e' = e[\Sigma]$ 
  \item \Z\Z $F\code{(}v\code{)} \rightsquigarrow e'$
  ...
  \item \Z\Z $F'\ f\code{(}v\code{)} \rightsquigarrow e'$
  \item \Z\Z $F\code{(}v\code{)} \rightsquigarrow e'$
  \item \Z $\exists e' .\ F\code{(}v\code{)} \rightsquigarrow e'$ by induction on $\code{case $p$=>$e$} \in F$
  \item $\square$
\end{lemma}

...
\newcommand{\com}[1]{\color{Gray} \rhd\ \emph{#1}}
...
\paragraph{Fixed point forward broadening.} Consider the application \obj{repeat(<succ> <succ> <zero> @)(x)} where \obj{x}
...
\paragraph{Fixed point backwards broadening.} Now suppose we have a function \obj{foo} whose input
...
\emph{Progression} of an expression, as in definition \ref{definition:progression},
...
The system leverages types of the forms in Definition \ref{definition:types}, 
...
\emph{Proof typing}, in Definition \ref{definition:proof_typing}, checks the viability of an expression's
form. Additionally, by extending the forms of expressions with type annotations, 
as in Definition \ref{definition:sugared_expressions},
...
where a world consists of emph{closed} type variables and subtyping constraints. 
...
We formalize ths soundness claim in Section \ref{section:correctness}. 
...
\paragraph{Rule \ref{definition:proof_typing}.1}
For the unit expression \obj{@}, proof typing simply returns the singleton type of the same
form. 

\paragraph{Rule \ref{definition:proof_typing}.2}
For a variable type expression $\alpha$, proof typing looks
for a corresponding typing in the environment, and returns the 
corresponding type if found.

\paragraph{Rule \ref{definition:proof_typing}.3}
For a tag expression $\obj{<}l\obj{>}e$, proof typing recursively constructs the type of the tag's body,
and returns the corresponding tag type.

\paragraph{Rule \ref{definition:proof_typing}.4}
For an empty expression, proof typing constructs the top type \obj{TOP},
which is merely syntactic sugar for $\obj{EXI[}\alpha\obj{]}\alpha$.

\paragraph{Rule \ref{definition:proof_typing}.5}
For a non empty record expression $\vec{r}\obj{\$}l\obj{=>}e$, proof typing 
recursively constructs the type for each entry, and
refines their types against each other via intersection.

\paragraph{Rule \ref{definition:proof_typing}.6}
For a function expression $\vec{f}$, proof typing 
delegates the work to two helper predicates: \emph{function lifting}, as in definition \ref{definition:function_lifting}, and
\emph{constrained implication congruence}, as in definition \ref{definition:constrained_implication_congruence}.
...
\paragraph{Rule \ref{definition:proof_typing}.7}
For a projection expression $e\obj{.}l$, proof typing
...
\paragraph{Rule \ref{definition:proof_typing}.8}
For an application expression$e_f\obj{(}e_a\obj{)}$, proof  typing
...
\paragraph{Rule \ref{definition:proof_typing}.9}
...
Using all these worlds, it delegates to \emph{fixpoint duality}, as in definition \ref{definition:fixpoint_duality},
...
\paragraph{Rule \ref{definition:proof_typing}.10}
For an annotated definition expression $\obj{let}\ x \obj{:} \tau_a\ \obj{=}\ e\ \obj{in}\ e'$, 
...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \Omega}\ Proof Typing
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \Gamma \entails \vec{r}\ \obj{\$}l\obj{=>}e \hastype \tau\ \obj{\&}\ l\obj{->}\tau' \given \Omega' 
    }\lab{5}

    \inferrule {
      \vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\eta} 
      \\
      \Gamma \entails \vec{\pi} \cong \vec{\tau} 
    } {
      \Gamma \entails \vec{f} \hastype \text{inter}(\vec{\tau}) \given \vec{\alpha}, \Delta
    }\lab{6}

    \inferrule {
      \Gamma \entails e \hastype \tau_0 \given \Omega 
      \\
      \tau_0 \subtypes l\obj{->}\alpha \given \Omega' 
      \\
      \Omega \preceq \Omega'
    } {
    ...
    \inferrule {
      \Gamma \entails e \hastype \alpha^+\obj{->}\tau \given  \vec{\alpha}, \Delta 
      \\\\
      \forall \vec{\alpha}'\ \Delta' .\ 
       (\vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta') \in \vec{\Omega}
      \implies
      \tau \subtypes \alpha_l\obj{->}\alpha_r \given  \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
      \\\\
      \exists \Omega.\ \Omega \in \vec{\Omega}
      \\
      \text{FTV}(\Gamma) \entails \nu\alpha^+.\vec{\Omega}.\alpha_l\rightarrow\alpha_r \fallingdotseq \mu\alpha^-.\vec{\tau}
    } {
      \Gamma \entails \obj{loop(}e\obj{)} \hastype \obj{ALL[}\alpha_x \obj{]}\alpha_x\obj{->EXI[}\alpha_y \obj{;} \alpha_x \obj{*} \alpha_y
      \obj{<:LFP[} \alpha^- \obj{]}\text{union}(\vec{\tau})\obj{]}\alpha_y \given \vec{\alpha}, \Delta 
    }\lab{9}
...
\emph{Proof subtyping}, as in definition \ref{definition:proof_subtyping},
checks the viability of one type subtyping another type.

\TODO{explain the overall predicate}

\TODO{explain the algorithmic interpretation}
...
We leave the remaining rules in the appendix, section \ref{section:appendix}, definition \ref{definition:continued_proof_subtyping}. 
...
\paragraph{Rule \ref{definition:proof_subtyping}.1}
For reflexive subtyping, up to alpha renaming, proof subtyping simply holds
without any updates to the world.

\paragraph{Rule \ref{definition:proof_subtyping}.2}
For a lower bound least fixed point type, $\obj{LFP[} \alpha \obj{]} \tau_l$, proof subtyping  
...
\paragraph{Rule \ref{definition:proof_subtyping}.3}
For an upper bound difference type, $\tau_r\obj{\textbackslash}\eta$, proof subtyping,
checks that the lower bound subtypes the positive type $\tau_r$
and does not subtype the subtracted type $\eta$.
...
of completeness precise in section \ref{section:correctness}.

\paragraph{Rule \ref{definition:proof_subtyping}.4}
For a lower bound union type, $\tau_{ll}\obj{|}\tau_{lr}$, proof subtyping
ensures that both parts of the lower bound subtype the upper bound.

The dual of this rule is the one for an upper bound intersection type, in
the continued definition \ref{definition:continued_proof_subtyping}.

\paragraph{Rule \ref{definition:proof_subtyping}.5}
for a lower bound existential type, $\obj{EXI[}\vec{\alpha}\ \Delta\obj{]}\tau_l$, proof subtyping 
first searches subtyping constraints that satisfy
the existential qualifiers $\Delta$, and then
checks the body of the existential against the upper bound,
under the previously learned subtyping constraints,
with the restriction that the existentially quantified variables  
are closed.

This is analogous to a proof in predicate logic, where
the prover declares a variable from an existential statement
and cannot see its particular value. 

The dual of this rule is the one for an upper bound universal type, 
in the continued definition \ref{definition:continued_proof_subtyping}.

The universal dual is analogous to a proof in predicate logic, where
the prover generalized from an arbitrary variable to a universal statement. 

\paragraph{Rule \ref{definition:proof_subtyping}.6}
...
\paragraph{Rule \ref{definition:proof_subtyping}.7}
For a upper bound open variable $\alpha$, proof subtyping
...
\paragraph{Rule \ref{definition:proof_subtyping}.8}
For an upper bound existential type, proof subtyping
solves the existential's target $\tau_r$ against the lower bound,
in order to learn necessary constraints on the bound variables 
for solving the qualifiers. 
It then uses the world generated from that to solve
the existential's qualifiers $\Delta$.

It is analogous to the proof method in predicate logic, 
in which the prover may choose an expression to be a witness for an existential statement. 


The dual of this rule is the one for a lower bound universal, 
in the continued definition \ref{definition:continued_proof_subtyping}.
The universal dual is analogous to the proof method in predicate logic,
in which the prover instantiates a universal statement with an expression
of their choice.  

\paragraph{Rule \ref{definition:proof_subtyping}.9}
For an upper bound union type, proof subtyping
checks that the left part of the union holds against
the lower bound. 

The dual of this rule is one for an upper bound intersection, 
in the continued definition \ref{definition:continued_proof_subtyping}.

\paragraph{Rule \ref{definition:proof_subtyping}.10}
For an upper bound union type, proof subtyping
also checks that the right part of the union holds against
the lower bound. 

The dual of this rule is one for an upper bound intersection, 
in the continued definition \ref{definition:continued_proof_subtyping}.

\paragraph{Rule \ref{definition:proof_subtyping}.11}
For a lower bound difference type, proof subtyping
simply checks that the positive type subtypes
the upper bound union with the subtracted type. 

\paragraph{Rule \ref{definition:proof_subtyping}.12}
For an upper bound least fixed point type,
where subtyping is decomposable, proof subtyping   
unrolls the least fixed point and checks it against the lower bound. 

\paragraph{Rule \ref{definition:proof_subtyping}.13}
For an upper bound least fixed point type,
...
\paragraph{Rule \ref{definition:proof_subtyping}.14}
For an upper bound least fixed point type,
...
\paragraph{Rule \ref{definition:proof_subtyping}.15}
For subtyping tag types, proof subtyping
simply checks that the labels match and subtyping holds
for their contents.

\paragraph{Rule \ref{definition:proof_subtyping}.16}
For subtyping record types, proof subtyping
simply checks that the labels match and subtyping holds
for their contents.

\paragraph{Rule \ref{definition:proof_subtyping}.17}
For subtyping implication types, proof subtyping
simply checks that the space between types of the upper bound
...
\begin{definition}\boxed{\tau \subtypes \tau \given \Omega}\ Proof Subtyping
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \Delta \given \vec{\alpha}_w, \Delta_w
      \\
      \vec{\alpha} \notfree \tau_r
      \\\\
      \tau_l \subtypes \tau_r \given \Omega 
      \\
      (\vec{\alpha}_w\ \vec{\alpha}, \Delta_w) \preceq \Omega 
    } {
    ...
    \inferrule {
      \alpha \in \vec{\alpha}
      \\
      \text{factor}(\Delta, \alpha) = \Delta_f
      \\
      \vec{\alpha}, (\Delta\ \Delta_f) \entails \alpha <: \vec{\tau} 
      \\\\
      \text{inter}(\vec{\tau}) \subtypes \tau_r \given \vec{\alpha}', \Delta'
      \\
      (\vec{\alpha}, \Delta\ \Delta_f) \preceq (\vec{\alpha}', \Delta')
    } {
      \alpha \subtypes \tau_r
    ...
    \inferrule {
      \alpha \notin \vec{\alpha}
      \\\\
      \vec{\alpha}, \Delta \entails \alpha \slash \tau_l \dagger \Delta^\dagger
      \\
      \vec{\alpha}, \Delta \entails \alpha <:^\dagger \vec{\tau} 
      \\
      \vec{\alpha}, \Delta \entails \alpha \slash \tau_l <:^\sharp \Delta^\sharp
      \\\\
      \Delta^\dagger\ \tau_l\obj{<:}\text{inter}(\vec{\tau}) \given \vec{\alpha}', \Delta'
      \\
      (\vec{\alpha}, \Delta\ \Delta^\sharp) \preceq (\vec{\alpha}', \Delta')
    } {
    ...
    \inferrule {
      \tau_l \subtypes \tau_{rl} \given \Omega 
    } {
      \tau_l \subtypes \tau_{rl}\obj{|}\tau_{rr} \given \Omega 
    }\lab{9}

    \inferrule {
      \tau_l \subtypes \tau_{rr} \given \Omega 
    } {
      \tau_l \subtypes \tau_{rl}\obj{|}\tau_{rr} \given \Omega 
    }\lab{10}

    \inferrule {
      \tau_l \subtypes \tau_r\obj{|}\eta \given \Omega
    } {
    ...
    \inferrule {
      \exists \alpha .\ \alpha \in \text{FTV}(\phi) \land \alpha \in \vec{\alpha} 
      \\
      \phi \subtypes \obj{LFP[}\alpha\obj{]}\tau \cong \phi' \subtypes \obj{LFP[}\alpha \obj{]} \tau'
      \\\\
      \Delta \entails \phi' \subtypes \tau_n \sim 
      \\
    ...
    \inferrule {
      \forall \alpha .\ \alpha \in \text{FTV}(\phi) \implies \alpha \notin \vec{\alpha} 
      \\
      \entails \phi \subtypes \obj{LFP[}\alpha\obj{]}\tau \consis 
      \\\\
      \vec{\alpha} \entails \Delta \wr \text{FTV}(\phi) \given \vec{\delta}
      \\
      \phi[\vec{\delta}] \subtypes \obj{LFP[}\alpha\obj{]}\tau
      \given \vec{\alpha}', \Delta'
      \\
...
\begin{definition}\boxed{\vec{\delta}, \Gamma \satisfies e \hastype \tau}\ Model typing
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \inferrule { 
      \vec{\delta}, \Gamma \satisfies e \hastype \obj{$\tau_l$}
      \\
      \neg\ (\vec{\delta}, \Gamma \satisfies e \hastype \obj{$\tau_r$})
    } {
      \vec{\delta}, \Gamma \satisfies e \hastype \obj{$\tau_l$\textbackslash$\tau_r$}
    } 
...
\begin{definition}\boxed{\tau \subtypes \tau \given M, \Delta}\ Continued Proof Subtyping 
  \label{definition:continued_proof_subtyping}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\eta}}\ Function Lifting
  \label{definition:function_lifting}
  \begin{mathpar}
    \inferrule {
    } {
    ...
    \inferrule {
      \vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\eta} 
      \\
      p \hastype \phi \given \Gamma'
      \\
      \text{diff}(\phi, \vec{\eta}) = \tau_l
      \\\\
      \forall \vec{\alpha}'\ \Delta'\ \tau_r .\ (\vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta', \tau_l \obj{->} \tau_r) \in \vec{\pi}' 
      \implies
...
\begin{definition}\boxed{\vec{\alpha}, \vec{\alpha} \entails \vec{\pi} \cong \vec{\tau}}\ Constrained Implication Congruence 
  \label{definition:constrained_implication_congruence}
  \begin{mathpar}
    \inferrule {
    } {
      \vec{\alpha}_f, \vec{\alpha}_m \entails \epsilon \cong \epsilon 
    }

    \inferrule {
      \vec{\alpha}_f, \vec{\alpha}_m \entails \vec{\pi} \cong \vec{\tau} 
      \\
      \text{FTV}(\tau_l) = \vec{\alpha}_l
      \\
      \text{FTV}(\tau_r) = \vec{\alpha}_r
      \\\\
      \Delta \entails \vec{\alpha}_f\ \vec{\alpha}_m\ \vec{\alpha}_l\ \vec{\alpha}_r \pitchfork \Delta' 
      \\
      \vec{\alpha}_f, \vec{\alpha}_m, \Delta' \entails \tau_l\obj{->}\tau_r \cong^{+} \tau
    } {
      \vec{\alpha}_f, \vec{\alpha}_m \entails \vec{\pi}\ \left<M, \Delta, \tau_l \obj{->} \tau_r \right> \cong \vec{\tau}\ \tau 
    }
  \end{mathpar}
\end{definition}

\begin{definition}
\boxed{\vec{\alpha} \entails \nu \alpha.Z.\alpha \rightarrow \alpha \fallingdotseq \mu \alpha.\vec{\tau}}
\ Fixpoint Duality
\label{definition:fixpoint_duality}
...
\TODO{Note the reason for excluding rigids and closeds from quantification is a way to improve precision, but not necessary for soundness. (Right?). Need to conjure up an example to support this idea.}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \vec{\alpha}_f \sqcup M \sqcup \text{FTV}(\Tau_l) \sqcup \text{FTV}(\Tau_r)\ \alpha_{h^-} = \Alpha 
      \\
      \Delta \entails \Alpha \pitchfork \Delta_i  
      \\\\
      \vec{\alpha}_f\ \alpha_{h^-}, M, \Delta_i \sqcup \Delta_h \entails \obj{\&$(\Tau_l)$*|$(\Tau_r)$} \cong^{-} \tau
...
\begin{definition}\boxed{\Alpha, \Alpha, \Delta \entails \tau \cong^{+|-} \tau}
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      (\text{FTV}(\Delta)\ \text{FTV}(\tau))\ \diamond\ \Alpha_z = \Alpha_o
      \\

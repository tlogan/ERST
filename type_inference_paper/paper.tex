...
\begin{definition}
\boxed{N \entails \alpha \circlearrowleft Z \cdot \code{$\alpha$->$\alpha$} \fallingdotseq \alpha \circlearrowright \mathrm{T}}
\ (Fixpoint duality)
  ...
  \begin{mathpar}
    ...
    \inferrule {
      M, \Delta, \Delta \entails \alpha_1 \subtypes \Tau_h
      \\
      \widebar{\code{($\tau_l$,$\tau_r$)<:$\alpha_3$}}^{\forall \tau_l\ \tau_r .\ \tau_l\code{->}\tau_r \in \Tau_h} = \Delta_h
      \\\\
      M, \Delta, \Delta \entails \alpha_1 \subtypes \Tau_l 
      \\
      ...
      \\
      N\ \alpha_3, M, \Delta_i \sqcup \Delta_h \entails \tau_p \equiv^{-} \tau
...
\begin{theorem}(Proof typing soundness)
  ...
  \item \Z\Z $
    \tau' \subtypes 
    \code{BOT->ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\mathrm{T}_{rel})$]]$\alpha_5$} 
    \given Z_0 
  $ by theorem \ref{theorem:proof_subtyping_interpolation}
  ...
  \item \Z\Z $
    \Omega \satisfies 
    \code{ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\mathrm{T}_{rel})$]]$\alpha_5$} 
  $ by theorem \ref{} \TODO{...}
...
\begin{theorem}(Proof subtyping interpolation (strongest post-condition))
  \label{theorem:proof_subtyping_interpolation}
  \begin{mathpar}
    \inferrule { 
      Z_0 \looparrowright Z_1
      \\

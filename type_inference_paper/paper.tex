...
\\
\\
\nonterm{Qualification}{Q} & \is
  \vec{\alpha} \sep 
  \code{$Q$ ; $\tau$<:$\tau$} 
\\
\\
\nonterm{Type\ var.\ seq.}{\vec{\alpha}} & \is \alpha \sep \vec{\alpha}\ \alpha 
\\
\\
\nonterm{Expression}{e} & \is 
  \begin{array}[t]{@{}l}
  \code{$x$} \sep 
  \code{@} \sep
...
\\
\\
\nonterm{Record\ pttn.}{b} & \is 
  \code{;$l$=$p$} \sep b\ \code{;$l$=$p$}
\\
\\
\nonterm{Value\ ctx.}{\Omega} & \is 
  \epsilon \sep \Omega \cdot x \mapsto v
\\  
\\  
\nonterm{Typing\ ctx.}{\Gamma} & \is 
  \epsilon \sep \Gamma \cdot \code{$x$:$\tau$}
\\
\\  
\nonterm{Subtyping\ ctx.}{\Delta} & \is 
  \epsilon \sep \Delta \cdot \code{$\tau$<:$\tau$}
\\
\\
\nonterm{Collectable}{c} & \is 
  \cdot \code{$x$:$\tau$} \sep 
  \cdot \code{$\tau$<:$\tau$} \sep
  \code{;$l$=$e$} \sep
  \code{;$l$=$p$} \sep

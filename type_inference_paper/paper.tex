...
\begin{definition}\boxed{\tau \subtypes \tau \given M, \Delta}\ (Proof subtyping)
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \Delta_0 \sqcup \Delta_m \preceq \Delta_1
      \\
      \Delta_{rel}\ \code{$\tau_l$<:\&$(\Tau)$} 
      % \sqcup \widebar{\code{.$\tau_l$<:$\tau$}}^{\tau \in \Tau}
      \given M_1, \Delta_1
    } {
      \tau_l \subtypes \alpha
      \given M_1, \Delta_1
...
\TODO{Note the reason for excluding rigids and skolems from quantification is a way to improve precision, but not necessary for soundness. (Right?). Need to conjure up an example to support this idea.}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \widebar{\code{$\tau_l$*$\tau_r$<:$\alpha_{h^-}$}}^{\tau_l\code{->}\tau_r \in \Tau_h} = \Delta_h
      \\\\
...
\begin{theorem}(Universe proof typing case soundness)
  ...
  \begin{mathpar}
    \inferrule { 
      ...
      \tau
      \subtypes
      \code{ALL[$\alpha_l$]$\alpha_l$->EXI[$\alpha_r$.$\alpha_l$*$\alpha_r$<:FIX[$\alpha_{h^-}$.$\tau_i$]]$\alpha_r$} 
    } 
  \end{mathpar}
  Proof: 
  \item \TODO{...} 
...
\begin{theorem}(Universe proof typing fixpoint union)
  ...
  \begin{mathpar}
    \inferrule { 
      ...
      \tau
      \subtypes
      \code{ALL[$\alpha_l$]$\alpha_l$->EXI[$\alpha_r$.$\alpha_l$*$\alpha_r$<:FIX[$\alpha_{h^-}$.$\tau$]]$\alpha_r$} 
    } {
      \Omega \satisfies
      \tau
      \subtypes
      \code{ALL[$\alpha_l$]$\alpha_l$->EXI[$\alpha_r$.$\alpha_l$*$\alpha_r$<:FIX[$\alpha_{h^-}$.|($\Tau\ \tau$)]]$\alpha_r$} 
    } 
  \end{mathpar}
  Proof: 
  \item \TODO{...} 
...
\begin{theorem}\ Influential soundness 
  ...
  \begin{mathpar}
    \inferrule { 
      ...
      \\\\
      N, M, \Delta \sqcup \Delta' \given \code{\&$(\Tau_l)$*|$(\Tau_r)$} \equiv^- \tau
      \\
      N, M, \Delta_i \sqcup \Delta' \given \code{\&$(\Tau_l)$*|$(\Tau_r)$} \equiv^- \tau_i
    } {
...
\begin{theorem}(Proof subtyping weak soundness)
  ...
  $
    ...
    $
    \item \Z $\B{hypo } 
      \forall \Omega .\ \Omega \satisfies \Delta_1 \implies \Omega \satisfies \tau_b \subtypes \tau_r 
    $
    \item \Z $\B{mutu } 
    ...
    \item \Z $\B{wrt } \Alpha\ Q\ \tau_b\ M_1\ \Delta_1\ M_0\ \Delta_0$
      \item \Z\Z $\B{for } \Omega \B{ assume } \Omega \satisfies \Delta$
        ...
        \item \Z\Z\Z $\B{for } e \B{ assume } \Omega \satisfies e \hastype \tau_l$ 
          \item \Z\Z\Z\Z $\Omega \satisfies e \hastype \code{EXI[$\Alpha$ $Q$]$\tau_b$}$ by substitution
          \item \Z\Z\Z\Z \TODO{necessary conditions for existential elimination}
          \item \Z\Z\Z\Z $\Omega \satisfies e \hastype \tau_b$ by theorem \ref{theorem:model_subtyping_existential_elimination} 
          \item \Z\Z\Z\Z $\Omega \satisfies e \hastype \tau_r$ by theorem \ref{theorem:model_typing_subsumption} 
    ...
    \item \Z $\B{wrt } \Alpha\ Q\ \tau_b\ M_1\ \Delta_1\ M_0\ \Delta_0$
      \item \Z\Z $\B{for } \Omega \B{ assume } \Omega \satisfies \Delta$
        ...
        \item \Z\Z\Z $\Omega \satisfies Q$ by application 
        \item \Z\Z\Z $\B{for } e \B{ assume } \Omega \satisfies e \hastype \tau_l$ 
          \item \Z\Z\Z\Z $\Omega \satisfies e \hastype \tau_b$ by theorem \ref{theorem:model_typing_subsumption} 
          \item \Z\Z\Z\Z \TODO{necessary conditions for universal introduction}
          \item \Z\Z\Z\Z $\Omega \satisfies e \hastype \code{ALL[$\Alpha$ $Q$]$\tau_b$}$ by theorem \ref{theorem:model_subtyping_universal_introduction} 
...
% e :  ALL[a b . 3 <: a ] (a,b) \implies e : (a,b)
    ...
    \item \Z $\contin
      \Delta_0 \sqcup \Delta_m \preceq \Delta_1
      \I
      \code{|$(\Tau)$} <: \tau_r \given M_1, \Delta_1
    $
    \item \Z $\B{hypo } \forall \Omega .\ \Omega \satisfies \Delta_1 \implies
      \Omega \satisfies \code{|$(\Tau)$} <: \tau_r
    $
    \item \Z $\B{wrt } \alpha\ M_1\ \Delta_1\ M_0\ \Delta_0\ \Delta_m\ \Tau$
      \item \Z\Z $\B{for } \Omega \B{ assume } \Omega \satisfies \Delta$
        \item \Z\Z\Z $\Omega \satisfies \Delta_1 \code{($\alpha$<:$\tau_r$)}$ by substitution
...
\begin{theorem} Model subtyping existential elimination 
  \label{theorem:model_subtyping_existential_elimination}
  \begin{mathpar}
    \inferrule { 
  ...
  \end{mathpar}
  Proof:
  \item \TODO{WRONG!!! need stricter conditions}
% x/1 |- exists x . x < 3 and x = 2, x/1 |- x < 3 ====> x/1 |- x = 2 
\end{theorem}

\begin{theorem} Model subtyping universal introduction 
  \label{theorem:model_subtyping_universal_introduction}
  \begin{mathpar}
    \inferrule { 
      \Omega \satisfies e \hastype \tau
      \\
      ...
      \Omega \satisfies e \hastype \code{ALL[$\Alpha$ $Q$]$\tau$}
    } 
  \end{mathpar}
  Proof:
  \item \TODO{WRONG!!! need stricter conditions}
% x/2 |- x = 2, x/2 |- x < 3 ====> x/2 |- forall x . x < 3 --> x = 2 
\end{theorem}
...
\begin{theorem} Model subtyping sequence last 
  ...
  \begin{mathpar}
    \inferrule { 
      \Omega \satisfies \Delta\ \code{($\tau_l$<:$\tau_r$)}
    } {
      \Omega \satisfies \tau_l \subtypes \tau_r 
    } 
  \end{mathpar}
...
\begin{definition}\boxed{\Omega, \Gamma \satisfies e \hastype \tau}\ Model typing
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      \alpha \slash \tau \notin \Omega
      \\
      \Omega, \Gamma \satisfies e \hastype \code{TOP} 
    } {
      \Omega, \Gamma \satisfies e \hastype \alpha 
    } 

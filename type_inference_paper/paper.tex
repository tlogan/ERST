...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given Z}
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{theorem}(Model typing implication elimination)
  ...
  \item \Z\Z\Z $\Omega, \Gamma \satisfies \code{($F$*$p$=>$e$)}\code{(}e_1\code{)} \hastype \tau_r$ by definition \TODO{ensure the definitional assumptions are proven} 
...
\begin{theorem}(Model typing function progress)
  ...
  \item \Z $\textbf{for } e'$
  \item \Z\Z $\code{$F$($e$)} \rightsquigarrow e' \land \Omega, \Gamma \satisfies e' \hastype \tau_r$ by identity

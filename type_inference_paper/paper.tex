...
\tau ::= &\C{type}
  ...
  \\
  \I \code{EXI [$\widebar{\alpha}$ $Q$] $\tau$} &\C{existential / indexed union}
  \\
  \I \code{ALL [$\widebar{\alpha}$ $Q$] $\tau$} &\C{universal / indexed intersection}
  \\
  \I \code{LFP $\alpha$|$\tau$} &\C{induction}
  \\
...
\begin{mathpar}
  ...
  \inferrule[Extension] {
    \left< \Delta_i \cup \Delta_o, \Omega_i \cup \Omega_o \right>
    \claim 
    Q
    \\\\
    \left< \Delta_i \cup  \Delta_o, \Omega_i \cup \Omega_o \right>
...
\end{mathpar}
\caption{Subtyping (inductive)}
\label{fig:subtyping_inductive}
\end{figure*}

...
\begin{definition} Internals 
  ...
  \[\begin{array}[t]{r@{}l}
    ...
    \\
    \\
    \nonterm{\Theta} & \is \epsilon \sep \Theta\ \left< \vec{\alpha}, \Delta \right>
    \\
    \\
    ...
    \\
    \\
    \nonterm{\Pi} & \is \epsilon \sep \Pi\ \left< M, \Delta, \tau \obj{->} \tau\right>
    \\
    \\
    \nonterm{\Omega} & \is \epsilon \sep \Omega\ \alpha \slash \tau
  \end{array}\]
\end{definition}
...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \vec{\alpha}, \Delta}\ Proof Typing
  \label{definition:proof_typing}
  \begin{mathpar}
    \inferrule {
    } {
      \Gamma \entails \obj{@} \hastype \obj{@} \given \vec{\alpha}, \Delta 
    }

    \inferrule {
      x:\tau \in \Gamma
    } {
      \Gamma \entails x \hastype \tau \given \vec{\alpha}, \Delta 
    }

    \inferrule {
      \Gamma \entails e \hastype \tau \given \vec{\alpha}, \Delta 
    } {
      \Gamma \entails \obj{<}l\obj{>}e \hastype \obj{<}l\obj{>}\tau \given \vec{\alpha}, \Delta 
    }

    \inferrule {
      \Gamma \entails R \hastype \tau_0 \given \vec{\alpha}, \Delta 
      \\
      \Gamma \entails e \hastype \tau_1 \given \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
    } {
      \Gamma \entails R\ \obj{\$}l\obj{=>}e \hastype \tau_0\ \obj{\&}\ l\obj{->}\tau_1 \given \vec{\alpha}\ \vec{\alpha}, \Delta\ \Delta' 
    }

    \inferrule {
      \vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \Pi, \vec{\phi} 
      \\
      \Gamma \entails \Pi \cong \vec{\tau} 
    } {
      \Gamma \entails \vec{f} \hastype \text{inter}(\vec{\tau}) \given \vec{\alpha}, \Delta 
    }

    \inferrule {
      \Gamma \entails e \hastype \tau_0 \given \vec{\alpha}, \Delta
      \\
      \tau_0 \subtypes l\obj{->}\alpha \given \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta'
    } {
      \Gamma \entails e\obj{.}l \hastype \alpha \given \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta'
    }

    \inferrule {
      \Gamma \entails e_0 \hastype \tau_0 \given \vec{\alpha}_0, \Delta_0 
      \\
      \Gamma \entails e_1 \hastype \tau_1 \given \vec{\alpha}_0\ \vec{\alpha}_1, \Delta_0\ \Delta_1
      \\\\
      \tau_0 \subtypes \tau_1\obj{->}\alpha \given \vec{\alpha}_0\ \vec{\alpha}_1\ \vec{\alpha}_2, \Delta_0\ \Delta_1\ \Delta_2
    } {
      \Gamma \entails e_0\obj{(}e_1 \obj{)} \hastype \alpha \given \vec{\alpha}_0\ \vec{\alpha}_1\ \vec{\alpha}_2, \Delta_0\ \Delta_1\ \Delta_2
    }

    \inferrule {
      \Gamma \entails e \hastype \alpha^+\obj{->}\tau \given \vec{\alpha}, \Delta
      \\\\
      \exists \vec{\alpha}\ \Delta.\ \left< \vec{\alpha},\Delta \right> \in \Theta 
      \\\\
      \forall \vec{\alpha}'\ \Delta' .\ \left< \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' \right> \in \Theta 
      \iff 
      \tau \subtypes \alpha_l\obj{->}\alpha_r \given \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
      \\\\
      \text{FTV}(\Gamma) \entails \nu\alpha^+.\Theta.\alpha_l\rightarrow\alpha_r \fallingdotseq \mu\alpha^-.\vec{\tau}
    } {
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \Pi, \vec{\phi}}\ Function Lifting
  \begin{mathpar}
    \inferrule {
    } {
    ...
    \inferrule {
      ...
      \\
      \exists \vec{\alpha}\ \Delta\ \tau.\ \left< \vec{\alpha},\Delta,\tau \right> \in \Pi' 
      \\\\
      \forall \vec{\alpha}'\ \Delta'\ \tau_r .\ \left< \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta', \tau_l \obj{->} \tau_r \right> \in \Pi' 
      \iff
...
\TODO{double check LFP rules against code and try to improve syntax of helper fuctions and helper predicates}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \text{decomposable}(\Delta_0, \tau_l, \obj{LFP[$\alpha$]$\tau_r$})
      \\\\
      \Delta_0 \preceq \Delta_1
      \\

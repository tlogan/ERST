...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \Omega}\ Proof Typing
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      \text{FTV}(\Gamma) \entails \vec{\pi} \cong^+ \vec{\tau} 
    } {
    ...
    %    (\vec{\alpha}', \Delta') \in \vec{\Omega}_\nu
    %   \implies
    %   \tau \subtypes \alpha_l\obj{->}\alpha_r \given  \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
    %   \\\\
    %   \exists \Omega.\ \Omega \in \vec{\Omega}_\nu
    %   \\\\
    %   \text{FTV}(\Gamma) \entails 
    %   \text{GFP}[\alpha_\nu]\vec{\Omega}_\nu \Rightarrow \alpha_l\rightarrow\alpha_r 
    %   \fallingdotseq 
    %   \text{LFP}[\alpha_\mu]\vec{\Omega}_\mu \times \alpha_l * \alpha_r
    %   \\\\
    %   \text{FTV}(\Gamma)\ \alpha_\mu \entails \vec{\Omega}_\mu \times \alpha_l * \alpha_r \cong \vec{\tau}
    % } {
    %   \Gamma \entails \obj{loop(}e\obj{)} \hastype \obj{ALL[}\alpha_x \obj{]}\alpha_x\obj{->EXI[}\alpha_y \obj{;} \alpha_x \obj{*} \alpha_y
    %   \obj{<:LFP[} \alpha_\mu \obj{]}\text{union}(\vec{\tau})\obj{]}\alpha_y \given \vec{\alpha}, \Delta 
    % }\lab{9}
    ...
    \inferrule {
      ...
      \\\\
      \forall \vec{\alpha}'\ \Delta' .\ 
        \llbracket \vec{\alpha}\ \vec{\alpha}', \Delta', \alpha_l \obj{->} \alpha_r \rrbracket \in \vec{\pi}_\nu
      \implies
      \tau \subtypes \alpha_l\obj{->}\alpha_r \given  \vec{\alpha}\ \vec{\alpha}', \Delta\ \Delta' 
      \\\\
      ...
      \\
      \text{FTV}(\Gamma)\ \alpha_\mu \entails \vec{\pi}_\mu \cong^- \vec{\tau}
    } {
      \Gamma \entails \obj{loop(}e\obj{)} \hastype \obj{ALL[}\alpha_x \obj{]}\alpha_x\obj{->EXI[}\alpha_y \obj{;} \alpha_x \obj{*} \alpha_y
      \obj{<:LFP[} \alpha_\mu \obj{]}\text{union}(\vec{\tau})\obj{]}\alpha_y \given \vec{\alpha}, \Delta 
    }\lab{9}
...
\begin{definition}\boxed{\tau \subtypes \tau \given \Omega}\ Proof Subtyping
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \forall \Omega' .\ \tau_l \subtypes \tau_r \given \Omega' \implies (\vec{\alpha}_w\ \vec{\alpha}, \Delta_w) \preceq \Omega' \implies \Omega' = \Omega
    } {
    ...
    \inferrule {
      ...
      \Delta[\alpha\slash\tau_l]^{-} \backslash \Delta = \Delta^\dagger
      \\
      \text{skolem}^+(\Delta^\dagger) = (\Delta^\flat, \Delta^\sharp)
      % \\
      % \vec{\alpha}, \Delta \entails \alpha \slash \tau_l <:^\sharp \Delta^\sharp
      \\\\
      \Delta^\flat \given \vec{\alpha}', \Delta'
      \\
    ...
    \inferrule {
      ...
      \\\\
      \llbracket \Delta\ \Delta_f, \alpha \rrbracket^- = (\Delta', \tau_l)
      \\
      \tau_l \subtypes \tau_r \given \vec{\alpha}', \Delta''
      \\
      (\vec{\alpha}, \Delta') \preceq (\vec{\alpha}', \Delta'')
      \\\\
      \TODO{confirm that it's safe to remove constraints}
    } {
      \alpha \subtypes \tau_r
...
\begin{definition}\boxed{\Delta[\alpha\slash\tau]^{-|+} = \Delta}\ Polar Subtyping Environment Substitution 
...
\begin{definition}\boxed{\tau[\alpha\slash\tau]^{-|+} = \tau}\ Polar Type Substitution 
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\eta}}\ Function Lifting
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \llbracket \vec{\alpha}\ \vec{\alpha}', \Delta', \tau_l \obj{->} \tau_r \rrbracket \in \vec{\pi}'
      \implies
...
\begin{definition}\boxed{\vec{\alpha} \entails \vec{\pi} \cong^{+|-} \vec{\tau}}\ Multiple Constraint Congruence 
  \label{def:constrained_implication_congruence}
  \begin{mathpar}
    \inferrule {
    } {
      \vec{\alpha}_f \entails \epsilon \cong^{+|-} \epsilon 
    }

    \inferrule {
      \vec{\alpha}_f \entails \vec{\pi} \cong^{+|-} \vec{\tau} 
      \\
      \vec{\alpha}_f, \vec{\alpha}_c, \Delta \entails \tau \cong^{+|-} \tau'
    } {
      \vec{\alpha}_f \entails \vec{\pi}\ \left<\vec{\alpha}_c, \Delta, \tau \right> \cong^{+|-} \vec{\tau}\ \tau' 
    }
  \end{mathpar}
\end{definition}
...
\begin{definition}
\boxed{\vec{\alpha} \entails \alpha \downarrow \vec{\pi} \fallingdotseq \alpha \uparrow \vec{\pi}}
\ Fixed Point Duality 
...
\begin{definition}\boxed{
  \llbracket \vec{\alpha}, \Delta, \tau \rrbracket^- = (\vec{\alpha}, \Delta, \tau)
}\ Negative Type Interpretation 
\label{def:negative_type_interpretation}
...
\begin{definition}\boxed{
  \llbracket \vec{\alpha}, \Delta, \tau \rrbracket^+ = (\vec{\alpha}, \Delta, \tau)
}\ Positive Type Interpretation 
\label{def:positive_type_interpretation}
...
\label{def:negative_interpretation}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \vec{\alpha} \entails (\epsilon, \alpha) \subtypes (\epsilon, \obj{TOP})
    }

    \inferrule {
      (\Delta, \alpha) \subtypes (\Delta', \tau)
    } {
    ...
    \inferrule {
      \alpha \neq \tau_l
      \\
      (\Delta, \alpha_l) \subtypes (\Delta', \tau)
    } {
...
\label{def:positive_interpretation}
  ...
  \TODO{soundess: $\llbracket \Delta, \alpha \rrbracket^+ = (\Delta', \tau) \implies \Delta' \satisfies \tau <: \alpha$}

  \TODO{soundess: $\llbracket \Delta, \alpha \rrbracket^+ = (\Delta', \tau) \implies \Delta \satisfies \Delta'$}

  \begin{mathpar}
    \inferrule {
    } {
      \vec{\alpha} \entails (\epsilon, \alpha) \supertypes (\epsilon, \obj{BOT})
    }

    \inferrule {
      (\Delta, \alpha) \supertypes (\Delta', \tau)
    } {
...
\begin{definition}\boxed{[\alpha]\Delta \fallingdotseq [\alpha]\Delta}\ Fixpoint Constraint Duality 
\label{def:pair_duality}
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \alpha_\nu \notin \text{FTV}(\tau_r)
      \\
      [\alpha_\nu] \Delta_\nu \fallingdotseq [\alpha_\mu]\Delta_\mu
    } {
      [\alpha_\nu] \Delta_\nu\ (\tau_l <: \tau_r) \fallingdotseq [\alpha_\mu]\Delta_\mu\ (\tau_l <: \tau_r)
    }

    \inferrule {
    } {
      [\alpha_\nu] \Delta_\nu\ (\alpha_\nu <: \alpha_l \obj{->} \alpha_r) \fallingdotseq [\alpha_\mu]\Delta_\mu\ (\alpha_l \obj{*} \alpha_r <: \alpha_\mu)
    }
  \end{mathpar}
\end{definition}
...
\begin{definition}\boxed{\vec{\alpha}, \vec{\alpha}, \Delta \entails \tau \cong^{+|-} \tau} Constraint 
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      \text{quantify}^{+|-}(\vec{\alpha}_o, \Delta_o, \vec{\alpha}_i, \Delta_i, \tau) = \tau'
    } {
      \vec{\alpha}_f, \vec{\alpha}_c, \Delta \entails \tau \cong^{+|-} \tau'
    }
  \end{mathpar}

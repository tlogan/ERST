...
\begin{definition}\boxed{e \rightsquigarrow e}\ Progression
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \obj{($F$\$$p$=>$e$)($v$)} \rightsquigarrow e[\vec{\sigma}]
    }
...
\begin{definition} \boxed{\Gamma \entails e \hastype \tau \given \Omega}\ Proof Typing
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\phi} 
      \\
      \Gamma \entails \vec{\pi} \cong \vec{\tau} 
    } {
      \Gamma \entails \vec{f} \hastype \text{inter}(\vec{\tau}) \given \left< \vec{\alpha}, \Delta \right>
    }
...
% \begin{definition} Proof Typing \I \boxed{\Gamma \entails e \hastype \tau \given Z}
%   \label{definition:proof_typing}
%   \begin{mathpar}
%     \inferrule {
...
%     \inferrule {
%       \TODO{simplifiy with one world as input; construction of multiple branches from one world}
%       \\\\
...
\begin{definition} \boxed{\vec{\alpha}, \Delta, \Gamma \entails \vec{f} \liftfun \vec{\pi}, \vec{\phi}}\ Function Lifting
  \begin{mathpar}
    \inferrule {
    } {
...
% \begin{definition} \boxed{Z, \Gamma \entails F \liftfun \vec{\pi}, T}
%   \begin{mathpar}

%     \inferrule {
%       Z_0, \Gamma_0 \entails f \liftfun \vec{\pi}, T 
%       \\
...
%       \vec{\pi} \sqcup \widebar{\left< M, \Delta, \tau_l' \obj{->} \tau_r \right>}^{\left<M, \Delta\right> \in Z_1}, T\ \tau_0
%     }
%   \end{mathpar}
% \end{definition}
...
\TODO{double check LFP rules against code and try to improve syntax of helper fuctions and helper predicates}
...
\begin{definition}\boxed{\tau \subtypes \tau \given M, \Delta}\ Proof subtyping annex 
  ...
  \TODO{switch from extension predicate to using concat}
  \begin{mathpar}
    \inferrule {
    } {
    ...
    \inferrule {
      \TODO{create a negation syntax}
      \\
      \TODO{prove subtyping is complete for negation syntax}
      \\
      \text{negsyn}(\obj{$\tau_r$\textbackslash$\tau_n$})
      \\
      \tau_l \subtypes \tau_r
      \given M, \Delta
      \\\\
...
\begin{definition}\boxed{\vec{\delta} \satisfies \tau <: \tau}
...
\begin{definition}\boxed{\vec{\delta} \satisfies Q}

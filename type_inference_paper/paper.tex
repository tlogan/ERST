...
\begin{lemma}(Soundness of model typing)
  ...
  \end{mathpar}
  Proof:
  \item \TODO{the induction hypothesis includes the generalized assumption, e.g. $\forall e' .\ e' < e \implies Q(e')$ if inducting on $e$ or $\forall e' .\ P(e') \implies Q(e')$ if inducting on predicate $P$} 
  \item $\textbf{assume }
        \Omega, \Sigma \satisfies \Gamma 
  ...
  \item \Z\Z $e' \textbf{ sound}$ by induction hypothesis or some lemma akin to subject reduction???
  \item \Z\Z \TODO{does IH allow this if we induction on $\Omega, \Gamma \satisfies e \hastype \tau$ instead of $e$?, as the predicate acts as a guard/ordering in lieu of a decreasing e?}
  \item \Z\Z \TODO{without inducting on the assumed predicate, using induction here might be problematic; this sort of thing would make subject reduction trivial}
  \item \Z\Z $e \textbf{ sound}$

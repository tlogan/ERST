...
\begin{mathpar}
  ...
  \inferrule[Projection] {
    \code{*$l$=>$v$} \in G 
    \\
    \forall\ v' .\ \code{*$l$=>$v'$} \in G \rightarrow v = v'
  } {
  ...
  \inferrule[LetContin] {
    ...
    \code{let $x$:$\tau$ = $v$ in $e$} \rightsquigarrow e[\cdot x \slash v]
  }
  ...
  \inferrule[Recursion] {
    ...
    \code{fix(case $x$ => $e$)} \rightsquigarrow e[\cdot x \slash \code{fix(case $x$ => $e$)}]
  }
...
\begin{figure*}[h]
  \begin{flalign*}
    &\boxed{\Gamma \entails e \hastype \tau \given Z}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Unit] {
    } {
    ...
    \inferrule[Recursion] {
      ...
      \\\\
      \text{FTV}(\Gamma) \entails \alpha_0 \circlearrowleft Z_1 \cdot \code{$\alpha_1$->$\alpha_2$} \fallingdotseq \alpha_3 \circlearrowright \vec{\tau}_{rel}
    } {
      \Gamma \entails \code{fix($e$)} \hastype \code{ALL[$\alpha_4$]$\alpha_4$->EXI[$\alpha_5$.($\alpha_4$,$\alpha_5$)<:FIX[$\alpha_3$.|$(\vec{\tau}_{rel})$]]$\alpha_5$} \given Z_0
    }
  \end{mathpar}
  ...
  \begin{flalign*}
    &\boxed{Z, \Gamma \entails f \liftfun \Pi, T}&
  \end{flalign*}
  \begin{mathpar}
    ...
    \inferrule[Step] {
      ...
      \Pi \sqcup \widebar{\left< M, \Delta, \tau_l', \tau_r \right>}^{\left<M, \Delta\right> \in Z_1}, T\ \tau_0
    }
  \end{mathpar}
  ...
  \begin{flalign*}
    &\boxed{Z \looparrowright Z}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Expand] {
      \forall M_1\ \Delta_1.\ \left<M_1,\Delta_1\right> \in Z_1 \rightarrow 
  ...
  \end{mathpar}
\caption{Proof semantics (typing, etc)}
\label{fig:proof_semantics_part_a}
\end{figure*}


\begin{figure*}[h]
  \begin{flalign*}
    &\boxed{\tau \subtypes \tau \given M, \Delta}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Refl] {
    } {
  ...
  \end{mathpar}
  \caption{Proof semantics (subtyping . 1/2)}
  \label{fig:proof_semantics_part_b}
\end{figure*}
...
\begin{figure*}[h]
  ...
  \begin{mathpar}
    ...
    \inferrule[Flex-L-Fixpoint-R] {
      ...
      \tau_l \subtypes \code{FIX[$\alpha$.$\tau_r$]}
      \given M_1, \Delta_1 \cdot \code{$\tau_l$<:FIX[$\alpha$.$\tau_r$]}
    }
  ...
  \end{mathpar}
  \caption{Proof semantics (subtyping 2/2, collective subtyping, multiverse subtyping)}
  \label{fig:proof_semantics_c}
\end{figure*}
...
\begin{figure*}[h]
  ...
  \begin{flalign*}
    &{\boxed{\widebar{\ c\ }^{c \in C} \circeq C}}&
  \end{flalign*}
  \[\begin{array}{rclr}
    \widebar{\ c'\ }^{c \in C} 
    &\circeq& 
...
\begin{figure*}[h]
  \begin{flalign*}
    &\boxed{M, \Delta, \Delta \entails M \lessdot \alpha}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
  ...
  \begin{flalign*}
    &\boxed{M, \Delta, \Delta \entails \mathrm{T} \ll \alpha}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
  ...
  \begin{flalign*}
    &\boxed{M, \Delta, \Delta \entails \alpha \lessdot M}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
  ...
  \begin{flalign*}
    &\boxed{M, \Delta, \Delta \entails \alpha \ll \mathrm{T}}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
...
\begin{figure*}[h]
  \begin{flalign*}
    &\boxed{M, \Delta, \Delta \entails \alpha \lessdot \Delta}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
  ...
  \begin{flalign*}
    &\boxed{M, \Delta, \Delta \entails \alpha \ll \Delta}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
  ...
  \caption{Proof semantics (transitive relation extraction)}
  \label{fig:proof_semantics_transitive_relation_extraction}
\end{figure*}
...
\begin{figure*}[h]
\begin{flalign*}
  &\boxed{M, \Delta, \mathrm{A} \entails \Omega}&
\end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
  ...
  \end{mathpar}
\caption{Proof semantics (type interpretation)}
\label{fig:proof_semantics_type_interpretation}
\end{figure*}
...
\begin{figure*}[h]
  ...
  \begin{flalign*}
    &\boxed{\Delta \entails N \pitchfork \Delta}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
    ...
    \inferrule[Match] {
      ...
      \Delta \cdot \code{$\tau_l$<:$\tau_r$} \entails N \pitchfork \Delta_i \cdot \code{$\tau_l$<:$\tau_r$}
    }

    \inferrule[Skip] {
      \forall \alpha .\ \alpha \in N \rightarrow \alpha \notin \text{FTV}(\tau_l) \sqcup \text{FTV}(\tau_r)
      \\
      \Delta \entails N \pitchfork \Delta_i
    } {
      \Delta \cdot \code{$\tau_l$<:$\tau_r$} \entails N \pitchfork \Delta_i
    }
  \end{mathpar}
...
\begin{figure*}[h]
  ...
  \begin{flalign*}
    &\boxed{\Omega \satisfies u \intype \tau}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Var] { 
      \cdot \alpha \slash \tau \in \Omega
      \\
    ...
    \inferrule[Record] { 
      ...
      \Omega \satisfies u \intype \tau 
      \\
      \forall u' .\ \code{*$l$=>$u'$} \in H \rightarrow u' = u 
    } {
      \Omega \satisfies H \intype \code{$l$->$\tau$}
    } 

    \inferrule[Implication] { 
      \cdot u_l \triangleright u_r \in W 
      \\
      \Omega \satisfies u_l \intype \tau_l
      \\
      \Omega \satisfies u_r \intype \tau_r
      \\
      \forall u_r' .\ \cdot u_l \triangleright u_r \in W \rightarrow u_r' = u_r 
    } {
    ...
    \inferrule[IndInter] { 
      \forall \Omega' .\ \Omega \sqcup \Omega' \satisfies Q \rightarrow 
      \Omega \sqcup \Omega' \satisfies u \intype \tau
    } {
    ...
    \inferrule[Fixpoint] { 
      \Omega \cdot \alpha \slash \code{FIX[$\alpha$*$\tau$]} \satisfies u \intype \tau
    } {
      \Omega \satisfies u \intype \code{FIX[$\alpha$*$\tau$]}
    } 
  \end{mathpar}
  \begin{flalign*}
    &\boxed{\Omega \satisfies Q}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] { 
    } {
    ...
    \inferrule[Step] { 
      \Omega \satisfies Q 
      \\
      \forall u .\ \Omega \satisfies u \intype \tau_l \rightarrow u \intype \tau_r
    } {
...
\begin{mathpar}
  \inferrule { 
    \entails e \hastype \tau \given \mathbb{W}
    \\
    \mathbb{W} \entails \tau \equiv \tau'
    \\\\
    e \rightsquigarrow^+ v
    \\
    v << u
    % \forall B\ \Delta.\ \left<B, \Delta \right> \in \mathbb{W} \rightarrow 
    % \Delta \entails Q
    % \ \land\ 
    % \Omega 
  } {

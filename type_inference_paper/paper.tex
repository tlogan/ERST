...
\begin{figure*}[h]
  ...
  \begin{mathpar}
    ...
    \inferrule[Step] {
      ...
      \\
      text{neg}(\tau_0, \vec{\eta}) = \tau_2
    } {
      \vec{W}_0, \Gamma_0 \entails f\ \code{case $p$=>$e$} \liftfun 
      \vec{\pi}\ \widebar{\left< W_1,  \tau_2, \tau_1 \right>}^{W_1 \in \vec{W}_1}, \vec{\eta}\ \tau_0
    }
  \end{mathpar}
...
\begin{figure*}[h]
  \begin{flalign*}
    &\boxed{\Gamma \entails \vec{\pi} \Rrightarrow \vec{\tau}}&
  \end{flalign*}
  \begin{mathpar}
    \inferrule[Empty] {
    } {
      \vec{n} \entails \epsilon \Rrightarrow \epsilon 
    }

    \inferrule[Step] {
      \vec{n} \entails \vec{\pi} \Rrightarrow \vec{\tau} 
      \\
      \text{FTV}(\tau_l) = \vec{\alpha}_l
      \\
      \text{FTV}(\tau_r) = \vec{\alpha}_r
      \\\\
      \Delta \entails \vec{n}\ \vec{\alpha}_l\ \vec{\alpha}_r \ \pitchfork \Delta_i  
      \\
      \text{constrain}^+(\epsilon, \vec{z}, \Delta_i, \code{$\tau_l$->$\tau_r$}) = \tau
    } {
      \vec{n} \entails \vec{\pi}\ \left<\vec{z}, \Delta, \tau_l, \tau_r \right> \Rrightarrow \vec{\tau}\ \tau 
    }
  \end{mathpar}
  ...
  \begin{mathpar}
    ...
    \inferrule[Step] {
      ...
      \\
      \Delta \entails \vec{n}\ \vec{\alpha}_p\ \alpha_3 \ \pitchfork \Delta_i  
      \\
      \text{constrain}^{-}(\alpha_3, \vec{z}, \Delta_i, \tau_p) = \tau

...
\begin{figure*}[h]
  ...
  \begin{mathpar}
    \inferrule[Fixpoint-L] {
      % \vec{z}\ \alpha, \Delta \cdot \alpha \code{<:} \tau_r \entails \tau_l \subtypes \tau_r 
      % \\\\
      % \text{VS}
      % \\\\
      \vec{z}, \Delta \entails \tau_l[\alpha\slash\tau_r] \subtypes \tau_r 
    } {
      \vec{z}, \Delta
      \entails \code{FIX[$\alpha$]$\tau_l$} \subtypes \tau_r 
    }
    ...
    %   \entails \tau_l' \subtypes \code{LFP[$\alpha$|$\tau_u$]}
    % } {
    %   \left< \Delta, \Omega \right>
    %   \entails \code{LFP[$\alpha$|$\tau_l$]} \subtypes \code{LFP[$\alpha$|$\tau_u$]}
    % }
    ...
    % } {
    %   \left< \Delta, \Omega \right>
    %   \entails \code{LFP[$\alpha$|$\tau_l$]} \subtypes \tau_u
    % }
    ...
    \inferrule[Skolem-L-Fixpoint-R] {
      ...
      \\
      \vec{z}, \Delta
      \entails \tau \subtypes \code{FIX[$\alpha$]$\tau_r'$}
    } {
      \vec{z}, \Delta
      \entails \tau_l \subtypes \code{FIX[$\alpha$]$\tau_r$}
    }

    \inferrule[Flex-L-Fixpoint-R] {
      \vec{z}, \Delta, \text{FV}(\tau_l) \entails \Omega
      \\
      \vec{z}, \Delta
      \entails \tau_l[\Omega] \subtypes \code{FIX[$\alpha$]$\tau_r$}
      \\
    ...
    \inferrule[Diff-R] {
      ...
      \\
      \vec{z}, \Delta \entails \tau_l \subtypes \tau_r
      \\
      \neg(\vec{z}, \Delta \entails \tau_l \subtypes \tau_n)
    } {
      \vec{z}, \Delta
      \entails 
    ...
    \inferrule[Tag] {
      \vec{z}, \Delta 
      \entails \tau_l \subtypes \tau_r
    } {
      \vec{z}, \Delta 
      \entails \code{\textasciitilde$l$ $\tau_l$} \subtypes \code{\textasciitilde$l$ $\tau_r$}
    }
    ...
    \inferrule[Record] {
      ...
      \vec{z}, \Delta 
      \entails \code{$l$:$\tau_l$} \subtypes \code{$l$:$\tau_r$}
    }

    \inferrule[Imp] {
      \vec{z}_0, \Delta_0 \entails \tau_{rl} \subtypes \tau_{ll}
      \\
      \vec{z}_0 \preceq \vec{z}_1
      \\
      \Delta_0 \preceq \Delta_1
      \\
      \vec{z}_1, \Delta_1 \entails \tau_{lr} \subtypes \tau_{rr}
    } {
      \vec{z}_1, \Delta_1 
      \entails \code{$\tau_{ll}$->$\tau_{lr}$} \subtypes \code{$\tau_{rl}$->$\tau_{rr}$}
    }
  \end{mathpar}
...
\begin{figure*}[h]
  ...
  \begin{mathpar}
    ...
    \inferrule[Recursion] {
      ...
      \Gamma \entails \code{fix($e$)} \hastype \alpha \given \vec{W}_1
    }

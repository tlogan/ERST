...
reflexivity up to alpha renaming.

\paragraph{Rule \ref{definition:proof_subtyping}.2}
left least fixed point.
...
dual of right universal. The right universal rule checks that subtyping holds for all variations of the payload consistent with
...
This is analogous to declaring a variable from an existential statement,
or generalizing from an arbitrary variable to a universal statement.
to choose a particular interpretation. 

\paragraph{Rule \ref{definition:proof_subtyping}.6}
left closed variable.
...
dual of left universal rule checks that subtyping holds for some variation of the payload consistent with
the variable's upper bound. 
...
It is analogous to how variables are used in predicate logic proofs. 
For a universal assumption, or an existential conclusion, the prover is free to choose
an interpretation for a variable. 

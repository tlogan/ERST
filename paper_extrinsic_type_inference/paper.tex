...
\begin{definition} Sequences 
  ...
  \[\begin{array}[t]{r@{}l@{}r}
    ...
    \\
    \nonterm{\Omega} & \is \epsilon \sep \Delta \  \tau \J{<:} \eta
    & \com{restricted subtyping seq}
    \\
    \nonterm{\mathrm{I}} & \is \epsilon \sep \mathrm{I}\ \iota 
    & \com{composite seq}
    \\
    \nonterm{\iota} & \is 
    \alpha
...
\TODO{note how we also want to ensure that the annotations are relevant and not ignored}
...
guarantee that a program has no errors, since the property could be satisfied vacuously by
having it produce an invalid set of subtyping constraints $\Delta'$.
To ensure that the proof typing system ensure that a program $e$ has no errors,
we need Proof Typing Substance \ref{prop:proof_subtyping_substance}, which guarantees that    
the constraints generated by Proof Typing are valid. That is,
Proof Typing is not vacuous, assuming valid inputs.
...
\begin{property}
  \label{prop:packing_correspondence}
...
The property of Proof Subtyping Soundness cannot ensure that a subtyping is valid on its own,
since it could hold vacuously by generating invalid constraints.
To this end, we require Proof Subtyping Substance \ref{prop:proof_subtyping_substance},
which states that the generated constraints are valid if the input constraints are valid.
Thus, Proof Subtyping must be not be vacuous; in other words, it must have substance. 
...
\TODO{describe experiments in comparison to SuperF}

\TODO{describe experiments with overlapping types}


\section{Library}
\label{sec:library}

\TODO{add generic collection syntax}

\TODO{add collection concatentation}

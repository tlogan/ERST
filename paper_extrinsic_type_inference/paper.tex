...
\newcommand{\J}[1]{\small \texttt{\color{RoyalBlue} #1}}
...
In languages suchas as Java or ML, programmers must declare each data constructor along with the weakest space it belongs to. 
...
The type inference an reconstruct types based on how data constructors are used, such that the
type of a data constructor in one context, may be stronger, weaker, or incomparable to the type of 
the same data constructor in a different context. 

Due to the fact that a data constructor can belong to variety of types of varying strengths,
the system leverages subtyping to check, reconstruct, and even define types.
...
However, SuperF's type system and language is purposefully 
restricted and isn't sufficient for the high-degree of code reusability allowed by untyped languages.
...
In section \ref{sec:forms}, we define the forms of expressions, types, and other key objects. 
...
\begin{definition} Type Ancillaries 
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \vec{r}\J{?}l \fieldmap e \rightsquigarrow \vec{r}\J{?}l \fieldmap e' 
    }

    \inferrule {
      \vec{r} \rightsquigarrow \vec{r}'
    } {
      \vec{r} \J{?}l \fieldmap v \rightsquigarrow \vec{r}'\J{?} l \fieldmap v
    }
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      ...
      \vec{\delta} \satisfies \J{<}l\J{>}e \hastype \J{<}l\J{>}\tau
    } 

    \inferrule { 
      \J{?}l \fieldmap v \in \vec{g}
      \\
      \vec{\delta} \satisfies v \hastype \tau 
      \\
      \forall v' .\ \J{?} l \fieldmap v' \in \vec{g} \implies v' = v
    } {
      \vec{\delta} \satisfies \vec{g} \hastype l \fieldmap \tau
    } 
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      l \fieldmap \tau_l \subtypes l \fieldmap \tau_r \given \Omega 
    }

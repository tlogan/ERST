...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \\
    \inferrule {
      \Theta_l \cap \textbf{dom}(\Delta) = \emptyset 
      \\
      \Theta_l \cap \textbf{dom}(\tau_r) = \emptyset
      \\\\
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \\
    \inferrule {
      \Theta_r \cap \textbf{dom}(\Delta)
      \\
      \Theta_r \cap \textbf{dom}(\tau_l)
      \\\\
...
\begin{theorem}
  \label{theorem:typing_environment_extension}
  Typing Environment Extension 
  \\
  \begin{mathpar}
    \inferrule {
      \delta, \sigma \satisfies \Gamma 
      ...
      \exists v \qua \delta, \sigma\ x \slash v \satisfies \Gamma\ x \hastype \tau 
    }
  \end{mathpar}
\end{theorem}
\hfill
...
and Theorem \ref{theorem:typing_interpretation_extension}.
...
and applying Theorem \ref{theorem:proof_typing_skolem_freshness}
...
\pc
For $\Theta, \Delta, \Gamma \entails \J{let } x \J{:} \tau \J{ = } e \J{ in } e' \hastype \tau' \given \Theta''', \Delta'''$,
...
We derive a value $v$, such that $\delta', \sigma\ x \slash v \satisfies \Gamma\ x \hastype \tau$
by Theorem \ref{theorem:typing_environment_extension}.

We derive $\delta' \cup \delta \satisfies e'[\sigma\ x \slash v] \hastype \tau'$,
by instantiating the first applied induction hypothesis with the typing interpretation $\delta' \cup \delta_a \cup \delta_b$,
and the value interpretation $\sigma\ x \slash v$,
and applying
Theorem \ref{theorem:proof_typing_skolem_freshness},
Theorem \ref{theorem:subtyping_environment_interpretation_contraction},
Theorem \ref{theorem:typing_environment_interpretation_contraction},
...
\TODO{need to include Gamma as input to subtyping so that output skolems from subtyping are not already in Gamma from typing}.

\TODO{update implementation to be consistent with paper}.

\TODO{double check the correct theorems are being applied}.

\TODO{last step is to compose the model typings and model subtypings}.

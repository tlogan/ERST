...
\begin{definition}
  ...
  Model Typing
  \hfill
  \boxed{\vec{\delta}, \vec{\sigma} \satisfies e \hastype \tau}
  \\
  \begin{mathpar}
    \inferrule { 
    ...
    \inferrule { 
      ...
      \\\\
      \forall e_l.\ 
        \vec{\delta} \satisfies e_l \hastype \tau_l 
        \implies
        (\forall \tau_l'\ \tau_r' .\ \vec{\delta} \satisfies \vec{f} \hastype \tau_l' \J{->} \tau_r'
        \implies \neg (\vec{\delta} \satisfies e_l \hastype \tau_l'))
    } {
      \vec{\delta} \satisfies \vec{f} \J{?}p \J{=>} e \hastype \tau_l \J{->} \tau_r
    }

...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \TODO{update proof with packing changes}
      \\\\
      \Theta, \Delta, \Gamma \entails e \hastype \tau \given \Theta', \Delta' 
      \\
      \textbf{pack}^+(\Theta', \Delta' \entails \tau) = \tau^\dagger
      \\\\
      \textbf{ftv}(\tau_a) \subseteq \emptyset
      \\
      \exists \Theta'', \Delta'' \qua \Theta', \Delta' \entails \tau^\dagger \subtypes \tau_a \given \Theta'', \Delta''
      \\\\
...
% \TODO{loop detection isn't necessary to pass example G8A}

% \TODO{is loop detection a form of (co)inductive reasoning?}

% \TODO{there is infinitely never a contradictioin}

% \TODO{inputs are satisfiable regardless of context}

% \TODO{inputs are equisatisfiable with context and target constraint}

% \TODO{so inputs are consistent with target constraint}

% \TODO{the satisfiability property ensures there aren't invalid assumptions used to prove by induction}

% \TODO{that is, induction is only valid if the assumptions are consistent with the induction hypothesis}

% \TODO{A more principaled approach would be to remove the original constraints that have been subbed out,
% and then rename the remaning variables to skolems and use as assumptions}

% \TODO{the remaining unsubbed constraints cannot contradict the IH because the polarity is the same}
...
\begin{theorem}
  \label{thm:subtyping_environment_interpretation_independence}
...
\begin{theorem}
  \label{thm:typing_interpretation_independence}
  Typing Interpretation Independence 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  \label{thm:typing_environment_interpretation_independence}
  Typing Environment Interpretation Independence 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  ...
  \item \I\I \N Derive $(\delta' \oplus \delta_0 \satisfies \Delta')$
    [Thm. \ref{thm:proof_subtyping_skolem_freshness}, Thm. \ref{thm:subtyping_environment_interpretation_independence}]
  \item \I\I \N Derive $(
    \delta' \oplus \delta_0 \satisfies \tau_x \subtypes \tau_p
  )$ [P0]
  ...
  %     \delta' \oplus \delta_0 \satisfies \Delta'
  %     \implies
  %     \delta' \oplus \delta_0 \satisfies \Omega
  % )$ [Thm. \ref{thm:proof_subtyping_sequence_soundness}]
  ...
  \item \I\I \N Derive $(\delta' \oplus \delta_0 \oplus \delta_1 \satisfies \Delta')$
    [Thm. \ref{thm:proof_subtyping_containment}, Thm. \ref{thm:subtyping_environment_prefix_preservation}]
  \item \I\I \N Derive $(\delta' \oplus \delta_0 \satisfies \Delta')$
    [Thm. \ref{thm:proof_subtyping_sequence_skolem_freshness}, Thm. \ref{thm:subtyping_environment_interpretation_independence}]
  \item \I\I \N Derive $(
    \delta' \oplus \delta_0 \satisfies \tau_l \subtypes \tau_r
  )$ [P0]
...
and $
\forall \delta', \sigma \qua 
\delta' \oplus \delta_r \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \oplus \delta_r \satisfies r[\sigma] \hastype \tau
$ (P0)
...
and $
\forall \delta', \sigma \qua 
\delta' \oplus \delta_e \satisfies \Delta'' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \oplus \delta_e \satisfies e[\sigma] \hastype \tau'
$ (P1)
by applying the induction hypothesis.

Next, we derive $\textbf{dom}(\delta_r \oplus \delta_e) = \Theta'' \backslash \Theta$
...
and we assume 
$\delta' \oplus \delta_r \oplus \delta_e \satisfies \Delta''$ 
and 
$\delta', \sigma \satisfies \Gamma$.

We derive $\delta' \oplus \delta_r \oplus \delta_e \satisfies \Delta'$ 
by applying
...
We derive $\delta' \oplus \delta_r \satisfies \Delta'$ 
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
and
Theorem \ref{thm:subtyping_environment_interpretation_independence}.
Then, we have $\delta' \oplus \delta_r \satisfies r[\sigma] \hastype \tau$ 
by instantiating (P0) with the $\delta'$ and $\sigma$, and applying the result.  
Thus, we have $\delta' \oplus \delta_r \oplus \delta_e \satisfies r[\sigma] \hastype \tau$ 
by applying 
Theorem \ref{thm:proof_typing_skolem_freshness}
Theorem \ref{thm:proof_typing_containment},
rewriting with set arithmetic,
and applying
Theorem \ref{thm:typing_interpretation_independence}.


We derive $\delta' \oplus \delta_r, \sigma \satisfies \Gamma$ 
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
and 
Theorem \ref{thm:typing_environment_interpretation_independence}.
Then, we have $\delta' \oplus \delta_r \oplus \delta_e \satisfies e[\sigma] \hastype \tau' $
by instantiating (P1) with $\delta' \oplus \delta_r$ and $\sigma$, and applying the result.


We derive $\delta' \oplus \delta_r \oplus \delta_e \satisfies r[\sigma]\ \J{<}l\J{>} e[\sigma] \hastype \tau$
by applying Theorem \ref{thm:record_extension_preservation}
We derive $\delta' \oplus \delta_r \oplus \delta_e \satisfies r[\sigma]\ \J{<}l\J{>} e[\sigma] \hastype \tau'$
by applying Theorem \ref{thm:entry_type_introduction}.
Then, we derive 
$\delta' \oplus \delta \satisfies r[\sigma]\ \J{<}l\J{>} e[\sigma] \hastype \tau \J{\&} \tau'$
by applying Definition \ref{def:model_typing} (Model Typing).
Finally, we get $\delta' \oplus \delta \satisfies (r\ \J{<}l\J{>} e)[\sigma] \hastype \tau \J{\&} \tau'$
...
and we assume $\delta' \oplus \epsilon \satisfies \Delta$ and $\delta', \sigma \satisfies \Gamma$.
With a little rewriting, we have $\delta' \satisfies \Delta'$.
Then, we derive $\delta' \satisfies f[\sigma] \hastype \tau$ by applying Theorem \ref{thm:path_sequence_typing_soundness}, 
Theorem \ref{thm:function_lifting_skolem_freshness}, and Theorem \ref{prop:packaging_correspondence}, along with a little
rewriting.
Thus, we have $\delta' \oplus \delta \satisfies f[\sigma] \hastype \tau$ 
...
and $
\forall \delta', \sigma \qua 
\delta' \oplus \delta_a \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \oplus \delta_a \satisfies r[\sigma] \hastype \tau
$ (P0)
by applying the induction hypothesis.
We derive an interpretation $\delta_b$, such that $\textbf{dom}(\delta_b) = \Theta'' \backslash \Theta'$
and $
\forall \delta', \sigma \qua 
\delta' \oplus \delta_b \satisfies \Delta'' \implies
\delta' \oplus \delta_b \satisfies \tau \hastype \tau
$ (P1)
by applying Theorem \ref{thm:proof_subtyping_soundness}.

Next, we derive $\textbf{dom}(\delta_a \oplus \delta_b) = \Theta'' \backslash \Theta$ by applying
...
and we assume 
$\delta' \oplus \delta_a \oplus \delta_b \satisfies \Delta''$ and $\delta', \sigma \satisfies \Gamma$.

We derive $\delta' \oplus \delta_a \oplus \delta_b \satisfies \Delta'$ 
by applying 
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:subtyping_environment_prefix_preservation},
and rewriting with set arithmetic.
We derive $\delta' \oplus \delta_a \satisfies \Delta'$ 
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness}
and 
Theorem \ref{thm:subtyping_environment_interpretation_independence}.
Then, we have $\delta' \oplus \delta_a \satisfies e[\sigma] \hastype \tau$ 
by instantiating (P0) with the $\delta'$ and $\sigma$, and applying the result.  
Thus, we have $\delta' \oplus \delta_a \oplus \delta_b \satisfies r[\sigma] \hastype \tau$ 
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness}
Theorem \ref{thm:proof_typing_containment},
rewriting with set arithmetic,
...
We derive $\delta' \oplus \delta_a \oplus \delta_b \satisfies \tau \subtypes \J{<}l\J{>}\alpha$ by 
instantiating (P1) with $\delta' \oplus \delta_a$ and $\sigma$.

Next, we have $\delta' \oplus \delta \satisfies e[\sigma] \hastype \J{<}l\J{>}\alpha$ 
by applying Theorem \ref{subsumption} (Subsumption).
Then, we get $\delta' \oplus \delta \satisfies (e[\sigma])\J{.}l \hastype \alpha$ 
by applying Theorem \ref{elimination} (Record Type Elimination),
which simplifies to $\delta' \oplus \delta \satisfies (e\J{.}l)[\sigma] \hastype \alpha$
...
and $(
\forall \delta', \sigma \qua 
\delta' \oplus \delta_f \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \oplus \delta_f \satisfies e_f[\sigma] \hastype \tau_f
)$ (P0)
by applying the induction hypothesis.
...
\delta' \oplus \delta_a \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \oplus \delta_a \satisfies e_a[\sigma] \hastype \tau_a
)$ (P1)
by applying the induction hypothesis.
...
and $(
\forall \delta' \qua 
\delta' \oplus \delta_s \satisfies \Delta' \implies
\delta' \oplus \delta_s \satisfies \tau_f \subtypes \tau_a \J{->} \alpha
)$ (P2)
by applying Theorem \ref{thm:proof_subtyping_soundness}.

Next, we derive $(\text{dom}(\delta_f \oplus \delta_a \oplus \delta_s) = \Theta''' \backslash \Theta)$
by applying
...
we assume 
$(\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies \Delta''')$
and
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \oplus \delta_f \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_subtyping_skolem_freshness},
Theorem \ref{thm:proof_subtyping_containment},
Theorem \ref{thm:proof_typing_skolem_freshness},
...
We derive
$(\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies e_f[\sigma] \hastype \tau_f)$
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness},
Theorem \ref{thm:proof_subtyping_containment},
Theorem \ref{thm:proof_typing_skolem_freshness},
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \oplus \delta_f \oplus \delta_a \satisfies \Delta'')$
by applying
Theorem \ref{thm:proof_subtyping_skolem_freshness},
and
Theorem \ref{thm:subtyping_environment_interpretation_independence}.
Then, we derive
$(\delta' \oplus \delta_f \satisfies \Gamma)$
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
and
Theorem \ref{thm:typing_interpretation_independence}.
Then, we derive
$(\delta' \oplus \delta_f \oplus \delta_a \satisfies e_a[\sigma] \hastype \tau_a)$
by instantiating (P1)
with $\delta' \oplus \delta_f$ and $\sigma$ and applying the result.
We derive
$(\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies e_a[\sigma] \hastype \tau_a)$
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
and
...
We derive
$(\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies \tau_f \subtypes \tau_a \J{->} \alpha)$
by instantiating (P2) with $\delta' \oplus \delta_f \oplus \delta_a$, and applying the result.
Then, we derive $\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies e_f[\sigma] \hastype \tau_a \J{->} \alpha$
by applying 
Theorem \ref{thm:subsumption}.
Thus, we have $\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies e_f[\sigma]\J{(}e_a[\sigma]\J{)} \hastype \alpha$
by Theorem \ref{thm:path_type_elimination},
which simplifies to $\delta' \oplus \delta_f \oplus \delta_a \oplus \delta_s \satisfies e_f\J{(}e_a\J{)}[\sigma] \hastype \alpha$,
...
\delta' \oplus \delta_a \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \oplus \delta_a \satisfies e[\sigma] \hastype \tau
)$ (P0)
by applying the induction hypothesis.
...
\forall \delta', \sigma \qua 
\delta' \oplus \delta_b \satisfies \Delta'' \implies
\delta' \oplus \delta_b \satisfies \tau \hastype \tau_a
)$ (P1)
by applying Theorem \ref{thm:proof_subtyping_soundness}.
...
and $(
\forall \delta', \sigma \qua 
\delta' \oplus \delta_c \satisfies \Delta''' \implies
\delta', \sigma \satisfies \Gamma\ x \hastype \tau_a \implies
\delta' \oplus \delta_c \satisfies e'[\sigma] \hastype \tau'
)$ (P2)
by applying the induction hypothesis.

Next, we derive $(\textbf{dom}(\delta_a \oplus \delta_b \oplus \delta_c) \cong \Theta''' \backslash \Theta)$
by applying
Theorem \ref{thm:domain_extension} 
and rewriting with set arithmetic.
...
and an arbitrary expression interpretation $\sigma$,
and we assume 
$(\delta' \oplus \delta_a \oplus \delta_b \oplus \delta_c \satisfies \Delta''')$
and
$(\delta, \sigma \satisfies \Gamma)$.
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \oplus \delta_a \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:proof_subtyping_skolem_freshness},
...
with $\delta'$ and $\sigma$ and applying the result.
We derive
$(\delta' \oplus \delta_a \oplus \delta_b \oplus \delta_c \satisfies e[\sigma] \hastype \tau)$
by applying 
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:proof_subtyping_skolem_freshness},
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \oplus \delta_a \oplus \delta_b \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_subtyping_skolem_freshness},
and
Theorem \ref{thm:subtyping_environment_interpretation_independence}.
We derive
$(\delta' \oplus \delta_a \oplus \delta_b \satisfies \tau \subtypes \tau_a)$
by instantiating (P1)
with $\delta' \oplus \delta_a$ and applying the result.
We derive
$(\delta' \oplus \delta_a \oplus \delta_b \oplus \delta_c \satisfies \tau \subtypes \tau_a)$
by applying 
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_subtyping_containment},
and
Theorem \ref{thm:subtyping_interpretation_independence}.


We derive 
$(\delta' \oplus \delta_a \oplus \delta_b \satisfies \Gamma)$
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:proof_subtyping_skolem_freshness},
...
and
Theorem \ref{thm:typing_environment_interpretation_independence}.
Then, we derive $\delta' \oplus \delta_a \oplus \delta_b, \sigma \satisfies e[\sigma] \hastype \tau_a$
by applying Theorem \ref{thm:subsumption}.
Then, We derive $\delta' \oplus \delta_a \oplus \delta_b, \sigma\ x \slash e[\sigma] \satisfies \Gamma\ x \hastype \tau$
by applying Definition \ref{def:model_sequence_typing} (Model Sequence Typing).
Then, we derive
$(\delta' \oplus \delta_a \oplus \delta_b \oplus \delta_c \satisfies e'[\sigma\ x \slash e[\sigma]] \hastype \tau')$
by instantiating (P2) with $\delta' \oplus \delta_a \oplus \delta_b$ and $\sigma\ x \slash e[\sigma]$,
applying the result.


We derive $\delta' \oplus \delta_a \oplus \delta_b \oplus \delta_c \satisfies e'[\sigma][x \slash e[\sigma]] \hastype \tau'$,
...
We derive $\delta' \oplus \delta \satisfies e[\sigma] \hastype \tau$

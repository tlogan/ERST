...
\begin{theorem}
  ...
  \begin{mathpar}
    ...
    \TODO{this might work because it the existential deltas can be combined somehow or maybe be empty?}

    \TODO{what happens if closed variables are unchanged; do they still need to be existentially quantified?}

    \TODO{what if we simply choose the inputs to be the same as the outputs?}

    \TODO{this wouldn't actually happen because the rule that adds constraints depends on the input being smaller than the output}

    \TODO{but probably need to play around to get the details right}

...
Second, the solution to eliminating the indexed union's body must be unique. 
...
Refinement introduction is the double dual of expansion elimination, so its rules
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      \alpha \notin \vec{\alpha}
      \\ 
      \text{split}^-(\vec{\alpha} \entails \Delta[\alpha\slash\tau]^{+} \backslash \Delta) = (\Delta_\closed, \Delta_\open)
      \\\\

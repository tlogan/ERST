...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      ...
      \\
      \text{dom}(\delta') \cong \Theta
    } {
      \delta \satisfies e \hastype \J{EXI[}\Theta\ \Delta\J{]}\tau
    } 

    \inferrule { 
      \forall \delta' .\ 
      \text{dom}(\delta') \cong \Theta \implies
      \delta \cup \delta' \satisfies \Delta \implies 
      \delta \cup \delta' \satisfies e \hastype \tau
    } {
      \delta \satisfies e \hastype \J{ALL[}\Theta\ \Delta \J{]} \tau
    } 
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      ...
      \delta \satisfies \Delta \J{;} \tau_l \J{<:} \tau_r
    } 
  \end{mathpar}
\end{definition}
...
\begin{property}
  ...
  \begin{mathpar}
    \inferrule {
      \Gamma \entails e \hastype \tau \given \Theta, \Delta
    } {
      \exists \delta .\ \text{dom}(\delta) \cong \Theta \land
      (
      \forall \delta'\ \sigma .\ 
      \delta' \cup \delta \satisfies \Delta \implies
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \text{pack}^+(\text{FTV}(\Gamma)\ \Theta \entails \Pi) = \tau 
    } {
    ...
    \inferrule {
      ...
      \\
      \text{FTV}(\Gamma) \cup \text{FTV}(\Delta) = \Theta_\ignore
      \\\\
      \forall \Theta'\ \Delta' .\ 
      ...
      \\\\
      \exists \Theta\ \Delta\ \tau.\ (\Theta\ \Delta, \tau) \in \Pi_\nu
      \\
      \alpha_\nu \downarrow \Pi_\nu
      \fallingdotseq 
      \alpha_\mu \uparrow \Pi_\mu
      \\
      \text{pack}^-(\text{FTV}(\Gamma) \cup \Theta\ \alpha_\mu \entails \Pi_\mu) = \tau_\mu
      \\\\
...
$\text{pack}^+(\text{FTV}(\Gamma)\ \Theta \entails \Pi)$
...
By leveraging the negative packaging $\text{pack}^-(... \entails \Pi_\mu) = \tau_\mu$,
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \text{FTV}(\Gamma) \cup \text{FTV}(\Delta) = \Theta_\ignore
      \\\\
      \forall \Theta'\ \Delta'\ \tau_r .\ 
      \Theta', \llbracket \Theta_\ignore \cup \Theta' \entails \Delta', \tau_l \J{->} \tau_r \rrbracket^+ \in \Pi'
      \implies
      \Gamma \cup \Gamma' \entails e \hastype \tau_r \given \Theta \cup \Theta', \Delta \cup \Delta' 
      \\\\
      \exists \Theta\ \Delta\ \tau .\ (\Theta\ \Delta, \tau) \in \Pi' 
      \\
      \text{capture}(\rho) = \eta
    } {
...
\begin{property} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \text{dom}(\delta_\mu) \cong \Theta_\mu \land
...
\begin{property}
  ...
  \begin{mathpar}
    \inferrule { 
      \text{pack}^+(\Theta_\ignore \entails \Pi) = \tau'
      \\
      \text{dom}(\delta_\ignore) \cong \Theta_\ignore
    } {
      \exists \delta .\ 
...
\noindent
Packaging ($\text{pack}^\pm(\Theta_\ignore \entails \Pi) = \tau'$)
...
\begin{definition} 
  ...
  \hfill
  \boxed{\phi \subtypes \tau \given \Theta\ \Delta}
  \\
  \begin{mathpar}
    \inferrule {
      \tau_a \subtypes \tau
      \given \Theta\ \Delta 
      \\
      \Theta\ \Delta \preceq \Theta'\ \Delta'
      \\
      \tau_b \subtypes \tau
      \given \Theta'\ \Delta' 
    } {
    ...
    \inferrule {
      \Theta_l \notfree \tau_r
      \\
      \Delta_l \given \Theta\ \Delta
      \\
      \forall \Theta'\ \Delta' .\ \Delta_l \given \Theta'\ \Delta' \implies (\Theta'\ \Delta') = (\Theta\ \Delta)
      \\\\
      (\Theta \cup \Theta_l, \Delta) \preceq \Theta'\ \Delta' 
      \\
      \tau_l \subtypes \tau_r \given \Theta'\ \Delta' 
    } {
      \J{EXI[}\Theta_l\ \Delta_l \J{]}\tau_l \subtypes \tau_r
      \given \Theta'\ \Delta' 
    }
  \end{mathpar}
\end{definition}
...
\begin{definition} 
  ...
  \hfill
  \boxed{\tau \subtypes \psi \given \Theta\ \Delta}
  \\
  \begin{mathpar}
    \inferrule {
    ...
    \inferrule {
      \Theta_r \notfree \tau_l
      \\
      \Delta_r \given \Theta, \Delta
      \\
      \forall \Theta'\ \Delta' .\ \Delta_r \given \Theta'\ \Delta' \implies (\Theta'\ \Delta') = (\Theta\ \Delta)
      \\\\
      (\Theta \cup \Theta_r, \Delta) \preceq \Theta'\ \Delta' 
      \\
      \tau_l \subtypes \tau_r \given \Theta'\ \Delta' 
    } {
      \tau_l \subtypes \J{ALL[}\Theta_r\ \Delta_r \J{]}\tau_r \given \Theta'\ \Delta' 
    }
  \end{mathpar}
\end{definition}
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      \alpha \notin \Theta
      \\ 
      \Theta \entails \Delta[\alpha\slash\tau]^{+} \backslash \Delta \twoheadrightarrow (\Delta_\closed, \Delta_\open)
      \\\\
      ...
      \alpha \subtypes \tau \given (\Theta', \Delta' \J{;} \alpha\J{<:}\tau) 
    }
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \tau \subtypes \alpha \given (\Theta', \Delta' \J{;} \tau\J{<:}\alpha) 
    }
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \tau_l
      \subtypes 
      \J{EXI[}\Theta_r\ \Delta_r \J{]}\tau_r \given \Omega' 
    }
  \end{mathpar}
\end{definition}
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \lor 
      (\exists \Omega .\ (\Theta, \Delta) \preceq \Omega \land \tau' \subtypes \tau \given \Omega) 
      )
      \\\\
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      (\Theta,\Delta)  \preceq (\Theta \cup \Theta',\Delta \cup \Delta') 
    }
  \end{mathpar}
\end{definition}
...
\begin{definition} 
  ...
  \begin{array}[t]{r@{}c@{}lcl}
      ...
      \llbracket \Theta \entails \Delta, \Delta_q \J{;} \tau_l \J{<:} \tau_r \rrbracket^\pm
      &\ =\ & 
...
\begin{definition} 
  ...
  \begin{array}[t]{r@{}c@{}lcl}
      ...
      &\text{if}& 
      \begin{array}[t]{@{}l}
        (\forall \tau_l\ \tau_r .\ \J{;} \tau_l \J{<:} \tau_r \in \Delta' \iff \Theta, \Theta_\bullet, \Delta, \alpha \looparrowright^\pm \tau_l \subtypes \tau_r)
        \\
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      \alpha' \in \text{FTV}(\tau) \backslash \Theta
      \\
      \J{;}\alpha'\J{<:}\tau' \in \Delta 
    } {
...
\begin{definition}
  ...
  Type Packaging 
  \hfill
  \boxed{\text{pack}^\pm(\Theta_\ignore \entails \Theta, \Delta, \tau) = \tau'} 
  \\
  \[
  \begin{array}[t]{r@{}c@{}lcl}
      \text{pack}^\pm(\Theta_\ignore \entails \Theta, \Delta, \tau) 
      &\ =\ & 
      ...
      \begin{array}[t]{@{}l}
        ...
        \\
        (\text{FTV}(\Delta) \cup \Theta_p) \cap \Theta = \Theta_o 
        \\
        \text{FTV}(\Delta_i) \cup \Theta_p \backslash \Theta \backslash \Theta_\ignore = \Theta_i 
      \end{array}
  \end{array}
  \]
\end{definition}
...
\begin{definition}
  ...
  \hfill
  \boxed{\text{pack}^\pm(\Theta \entails \Pi) = \tau}\ 
  \\
  \[
  \begin{array}[t]{r@{}c@{}lcl}
      \text{pack}^+(\Theta_\ignore \entails \epsilon) 
      &\ =\ & 
      \J{TOP} 

      \\

      \text{pack}^+(\Theta_\ignore \entails \Pi\ (\Theta, \Delta, \tau)) 
      &\ =\ & 
      \tau \J{\&} \tau' 
      &\text{if}& 
      \begin{array}[t]{@{}l}
        \text{pack}^+(\Theta_\ignore \entails \Pi) = \tau 
        \\
        \text{pack}^+(\Theta_\ignore \entails \Theta, \Delta, \tau) = \tau'
      \end{array}
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \exists \alpha .\ \alpha \in (\text{FTV}(\tau_l) \cup \text{FTV}(\tau_r)) \cap \Theta
      \\
      \Theta \entails \Delta \dashrightarrow \Delta'
    } {
      \Theta \entails  \Delta \J{;} \tau_l \J{<:} \tau_r\dashrightarrow \Delta' \J{;} \tau_l \J{<:} \tau_r
    }
    ...
    \inferrule {
      ...
      \Theta \entails  \Delta \J{;} \tau_l \J{<:} \tau_r\dashrightarrow \Delta'
    }
  \end{mathpar}
\end{definition}
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \Theta_\ignore, \Theta \entails \Delta \rightrightarrows \Delta_o , \Delta_i 
      \\\\
      \exists \alpha .\ \alpha \in (\text{FTV}(\tau_l) \cup \text{FTV}(\tau_r)) \backslash \Theta_\ignore \backslash \Theta
    } {
      \Theta_\ignore, \Theta \entails \Delta \J{;} \tau_l \J{<:}\tau_r \rightrightarrows \Delta_o , \Delta_i \J{;} \tau_l \J{<:} \tau_r
    }
    ...
    \inferrule {
      ...
      \exists \alpha .\ \alpha \in (\text{FTV}(\tau_l) \cup \text{FTV}(\tau_r)) \cap \Theta 
      \\
      \text{FTV}(\tau_l) \cup \text{FTV}(\tau_r) \subseteq \Theta_\ignore \cup \Theta
    } {
      \Theta_\ignore, \Theta \entails \Delta \J{;} \tau_l \J{<:}\tau_r \rightrightarrows \Delta_o  \J{;} \tau_l \J{<:} \tau_r , \Delta_i
    }
  \end{mathpar}
...
\begin{theorem}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \Theta_0\ \Delta_0 \preceq \Theta\ \Delta
      \\
      \text{FTV}(\Gamma) \cup \Theta_0 \cup \text{FTV}(\Delta) = \Theta_\ignore
      \\
      \Gamma \entails e \hastype \tau \given \Theta, \Delta
      \\
      \text{dom}(\delta_0) \cong \Theta_\ignore
    } {
      \exists \delta .\ \text{dom}(\delta) \cong \Theta \backslash \Theta_\ignore \land
      (
      \forall \delta'\ \sigma .\ 
      \delta' \cup \delta_0 \cup \delta \satisfies \Delta \implies
      \delta' \cup \delta_0 \cup \delta, \sigma \satisfies \Gamma \implies
      \delta' \cup \delta_0 \cup \delta \satisfies e[\sigma] \hastype \tau
      )
    }

    \TODO{if we distinguis between input and output worlds}

    \TODO{then can we generalize over the common input variables?}

    \TODO{assuming the input closed variables should not affect the local expression?}

    \TODO{seems valid, since an expression is only constrained by closed variables that arise from within}

    \TODO{consider example of function application, where the return variable is constrained by a closed variable}

    \TODO{however, subtyping does need rely on input closed variables}

    \TODO{need to modify implementation and test out this assumption}

    \TODO{and can we union the environments of the mutually independent output variables?}

    \TODO{and can we union the environments of the mutually independent output variables?}


    ***********************************************************************
    \TODO{this might work because the existential deltas can be combined somehow or maybe be empty?}

    \TODO{what happens if closed variables are unchanged; do they still need to be existentially quantified?}

    \TODO{we add constraint between outputs of two consecutive steps}


    \TODO{Q: what's the deal if there is no previoius step?}

    \TODO{A: maybe we can treat any variable in Gamma as pre-existing, and everything else as an fresh output variable}
    \TODO{this is not good enough to disitinguis between input and output variables. input variabes could be in constraint, but not in Gamma}

    \TODO{Q: is it sufficient to simply treat all variables in Gamma as inputs and everything else as outputs}
    \TODO{this is NOT the same as constraining the outputs of consecutive steps}
    \TODO{Actually, maybe the consecutive steps should be independent! and then unioned together}
    \TODO{need to test out this idea}

    \TODO{should the previous output be treated as empty?}

    \TODO{can we simply choose the inputs to be the same as the outputs?}

    \TODO{probably need to play around to get the details right}
...
In the case of $\Gamma \entails \J{@} \hastype \J{@} \given \Theta, \Delta$,
we derive the conclusion using the definition of model typing (Def. \ref{def:model_typing}).
In the case of $\Gamma \entails x \hastype \tau \given \Theta, \Delta$,
...
In the case of $\Gamma \entails \epsilon \hastype \J{TOP} \given \Theta, \Delta$,
we derive the conclusion using the definition of model typing (Def. \ref{def:model_typing}).



In the case of $\Gamma \entails r \J{<}l \J{>} e \hastype (\tau\ \J{\&}\ \J{<} l \J{>} \tau') \given \Theta, \Delta$,
we deduce $\Gamma \entails r \hastype \tau \given \Theta, \Delta$; $\Omega \preceq \Theta, \Delta$; and $\Gamma \entails e \hastype \tau' \given \Theta, \Delta$,
using the definition of proof typing (Def. \ref{def:proof_typing}).
We have 
$\exists \delta.\ 
... \land
\forall \delta'\ \sigma'.\ 
... \implies \delta' \cup \delta \satisfies r[\sigma] \hastype \tau$ 
and
$\exists \delta.\  
... \land
\forall \delta'\ \sigma'.\ 
... \implies \delta \cup \delta' \satisfies e[\sigma] \hastype \tau'$
...
... \land
\forall \delta'\ \sigma'.\ 
... \implies \delta' \cup \delta \satisfies \J{<}l\J{>}e[\sigma] \hastype \J{<}l\J{>}\tau'$
...
\TODO{figure out how to connect parts of a record}

\TODO{figure out how to construct one expression with two different types}

\TODO{maybe just induction on the right part}

\TODO{might need a separate inductive theorem for describing the sequence on the left}

...
\pc
For $\J{@} \hastype \J{@}$,
we derive the conclusion by Definition \ref{def:model_typing} (Model Typing).

\pc
For $ x \hastype \tau$, 
we derive the conclusion by Theorem \ref{theorem:typing_environment_soundness}
(Typing Environment Soundness).
...
we apply the induction hypothesis to the two inductive premises
\newline
$r \hastype \tau$ and $e \hastype \tau'$. For the former, we derive
...
by Theorem \ref{theorem:domain_extension} (Domain Extension),
...
Theorem \ref{theorem:skolem_variable_freshness} 

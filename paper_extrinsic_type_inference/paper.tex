...
\TODO{consider title: Extrinsic Type Inference with Scalar Induction }

\TODO{because removing the intrinisic type on constructor inputs enables reasoninig scalar induction}
...
flow of program in the form of generating and solving subtypings. 
...
It was not until SuperF introduced a technique for solving subtypings
that a principal type could be automatically constructed for functions like $\J{foo}$
Our extrinsic type system relies on similar principles of solving subtypings,
...
\noindent
Although the subtypings are not present in the type's notation,
the intersection of paths is chosen because it represents the weakest type
consistent with the subtypings on a variable representing the parameter's type. 
...
and positively qualified by subtypings \ms{\Delta}, 
...
and negatively qualified by subtypings $\Delta$,
...
\noindent
Model subtyping sequence describes multiple subtypings. 
...
Proof typing generates subtypings approximating the control flow of an expression, 
and then leverages proof subtyping to solve for the variables in the the subtypings,
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \factorsinto \J{LFP[} \alpha_\mu \J{]}\tau_\mu \subtypes \J{left} \mapsto \tau_l
      \\
      \factorsinto \J{LFP[} \alpha_\mu \J{]}\tau_\mu \subtypes \J{right} \mapsto \tau_r
    } {
...
% that inherits skolem variables $\Theta$, subtypings $\Delta$ and typing constraints $\Gamma$,
% takes an expression $e$, and then generates a type $\tau$,
% a new set of skolem variables $\Theta'$, and a new set of subtypings $\Delta'$.


\noindent
For a given context and expression, 
proof typing infers a type and a subtyping sequence from the expression,
such that if the input context is satisfiable,
then there is a solution to the generated subtyping sequence, such that
the typing of the inferred type holds.
The context consists of skolems \ms{\Theta}, subtypings \ms{\Delta}, and typings \ms{\Delta}. 
The contextual subtypings and typings constrain the search space of valid typings, such that the
generated subtypings are consistent with them. In fact, the generated subtypings contain
all the contextual subtypings, as it keeps the definition a bit less cluttered.
Skolems are variables that further constrain the search space. 
The generated constraints must hold for every interpretation.  
...
in that it can be constrained in any way that is consistent with the other subtypings
and the typing. 
...
it rewrites the schema into a type by wrapping the path type in existentials
and universals qualified by the constraints in the schema's subtyping environment. 
...
bindings in existentials or universals.
...
For elimination of an existential $(
... \entails \J{EXI[}\Theta_l \J{]} \Omega \J{:}\tau_l \subtypes \tau_r \given ...
)$, 
Proof Subtyping ensures that the existential's bound variables
are not free in the super type $\tau_r$. 
Then it finds a solution for the qualifiers $\Delta_l$.
Finally it finds a solution for eliminating the existential's body $\tau_l$,
assuming the solution to the qualifiers $\Delta$.

When eliminating the existential's body,
the the bound variables $\Theta_l$ must be treated as skolem variables.
Since the existential is in the elimination position, 
...
the subtyping that it's proving.
...
)$ to ensure that the subtypings still hold with the substitution of
...
\TODO{move implication rewriting after plastic rules}
\begin{definition} 
...
\TODO{Note how plastic constraints can make skolems plastic selectively on one side or the other}

\TODO{note that we can't simply substitute in the plastic variable; as it would cause an infinite loop}
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \alpha \Vvdash \tau_r \supertypes \tau_r'
      \\
      \Theta, \Delta \entails
      \tau_l \subtypes \tau_r' \given \Theta', \Delta'
    } {
...
to approximately judge whether or not the subtyping is decidable. 
...
generate an unsatisfiable set of subtypings.
To ensure that the subtyping holds,
we need Proof Subtyping Substance \ref{prop:proof_subtyping_substance}, 
which requires that the subtypings generated by Proof Subtyping hold. 
The guarded subtypings must be valid with respect to the skolem variables $\Theta$,
...
\noindent
The theorem of Proof Typing Soundness is actually a bit weak on its own. It does not
require that a program has no errors, since the theorem could be satisfied vacuously by
having Proof Typing generate an unsatisfiable set of subtypings $\Delta'$.
To ensure that a program $e$ has no errors,
we need Proof Typing Substance \ref{prop:proof_subtyping_substance}, 
which requires that the subtypings generated by Proof Typing hold. 
The guarded subtypings must be valid with respect to the skolem variables $\Theta$,
and they must be satisfiable with respect to the remaining type variables. 
...
does not directly reference these annotations, so we need an additional theorem to ensure
...
\TODO{only show experiments where our system differs from SuperF or MLStruct}
...
The SuperF type system infers subtypings that represent the data flow represented by
...
of type variables that meet certain subtypings.   
...
universal types (i.e. universals) may or may not be qualified by subtypings.
...
The high level ideas in SuperF of relying on subtypings were very influential in our work.
Like SuperF, our system relies syntax-directed subtyping rules to check constraints and to learn 
new subtypings by traversing previously learned constraints;
...
Like SuperF, it learns the data flow of programs in terms of subtypings.
...
In contrast to MLStruct, our system relies completely on learning data flow in the form of subtypings
...
with expressions and learns subtypings.
...
We have incorporated previous studied ideas of subtypings,
...
\begin{definition}
  ...
  \hfill 
  \small
  \boxed{\alpha \Vvdash \tau_r \supertypes \tau_r'}
  \\
  \begin{mathpar}
    \inferrule {
      \alpha \notin \textbf{ftv}(\tau)
    } {
      \alpha \Vvdash \tau \supertypes \tau 
    }

    \inferrule {
      \alpha \in \textbf{ftv}(\tau)
      \\
      \alpha \Vvdash \tau_l \supertypes \tau_l' 
      \\
      \alpha \Vvdash \tau_r \supertypes \tau_r' 
    } {
      \alpha \Vvdash \tau_l \J{|} \tau_r \supertypes \tau_l' \J{|} \tau_r'
    }
    ...
    \inferrule {
      ...
      \alpha \Vvdash \tau \supertypes \J{BOT} 
    }
  \end{mathpar}
\end{definition}
\hfill
...
In the case where the subtypings are the same,

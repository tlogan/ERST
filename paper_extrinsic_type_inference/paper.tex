...
\begin{property}
  ...
  \begin{mathpar}
    ...
    % \\\\
    % \inferrule { 
    %   \llbracket \vec{\alpha}_\ignore, \vec{\alpha} \entails \Delta, \tau \rrbracket^+ = (\Delta', \tau')
    %   \\
    %   \vec{\alpha}_\ignore \cup \vec{\alpha} \subseteq \text{dom}(\vec{\delta}')       \\
    %   \\
    %   \vec{\delta} \cup \vec{\delta}' \satisfies \Delta
    ...
    %   \vec{\delta}' \satisfies \Delta'
    % } {
    %   \vec{\delta}' \satisfies e \hastype \tau' \implies
    %   \vec{\delta} \cup \vec{\delta}' \satisfies e \hastype \tau
    % } 
    ...
    %   \vec{\delta}' \satisfies \Delta'
    % } {
    %   \vec{\delta} \cup \vec{\delta}' \satisfies e \hastype \tau \implies
    %   \vec{\delta}' \satisfies e \hastype \tau'
    % } 
...
% \TODO{need to rethink; we don't need the strongest; we just need the interpretation to be safe}

\noindent
Type interpretation $\llbracket \vec{\alpha}_\ignore, \vec{\alpha} \entails \Delta, \tau \rrbracket^\pm = (\Delta', \tau')$
looks for interpretations of type variables in the input type $\tau$ that satisfy the subtyping constraints $\Delta$,
such that the positive interpretation $\llbracket ... \rrbracket^+$ tries to construct the strongest interpretation
satisfying the constraints and ignoring variables $\vec{\alpha}_\ignore$ and $\vec{\alpha}$,
and the negative interpretation $\llbracket ... \rrbracket^-$ tries to construct the weakest interpretation
satisfying the constraints ignoring variables $\vec{\alpha}_\ignore$ and $\vec{\alpha}$.
The ignored variables $\vec{\alpha}_\ignore$ and $\vec{\alpha}$ must not be replaced in the type or removed
from the subtyping environment. 
.
...
\begin{property}
  ...
  %     (
  %     \forall \vec{\delta}'.\ 
  %     (\vec{\alpha}, \Delta, \tau) \in \vec{\pi} \implies
  %     \text{dom}(\vec{\delta}') = \vec{\alpha} \implies
  %     \vec{\delta} \cup \vec{\delta}' \cup \vec{\delta}_\ignore \satisfies \Delta \implies
  %     \vec{\delta} \cup \vec{\delta}' \cup \vec{\delta}_\ignore \satisfies e \hastype \tau
  %     )
  %   } 

  %   \inferrule { 
  %     \text{pack}^+(\epsilon \entails (\J{X}, \epsilon, \J{X})) = \J{EXI[X]X} 
  %     \\
  %     \text{dom}(\vec{\delta}_\ignore) \cong \epsilon 
  %     \\
  %     \vec{\delta}_\ignore \satisfies \J{@} \hastype \J{EXI[X]X}
  %   } {
  %     \exists \vec{\delta}.\ 
  %     (
  %     \forall \vec{\delta}'.\ 
  %     (\vec{\alpha}, \Delta, \tau) = (\J{X}, \epsilon, \J{X}) \implies
  %     \text{dom}(\vec{\delta}') = (\J{X}) \implies
  %     \vec{\delta} \cup \vec{\delta}' \satisfies \J{@} \hastype \J{X}
  %     )
  %   } 
  \end{mathpar}
\end{property}
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      \alpha \notin \vec{\alpha}_\ignore
      \\
      \alpha \notin \vec{\alpha}_\closed
      \\
      \llbracket \vec{\alpha}_\closed \entails \Delta, \alpha \rrbracket^\pm = (\Delta', \tau)
    } {
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, \alpha \rrbracket^\pm = (\Delta', \tau)
    }

    \inferrule {
    } {
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, \J{@} \rrbracket^\pm = (\Delta, \J{@})
    }

    \inferrule {
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, \tau \rrbracket^\pm = (\Delta', \tau')
    } {
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, \J{<}l\J{>}\tau \rrbracket^\pm = (\Delta', \J{<}l\J{>}\tau')
    }

    \inferrule {
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, \tau \rrbracket^\pm = (\Delta', \tau')
    } {
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, l\J{->}\tau \rrbracket^\pm = (\Delta', l\J{->}\tau')
    }
    ...
    \inferrule {
      ...
      \llbracket \vec{\alpha}_\ignore, \vec{\alpha}_\closed \entails \Delta, \tau_l\J{->}\tau_r \rrbracket^\pm = (\Delta_l \cap \Delta_r, \tau_l'\J{->}\tau_r')
    }
...
\begin{definition}\boxed{C \cup C = C}\ Concatenation
  \[\begin{array}{rclr}
    C \cup \epsilon 
    &=& 
    C
...
\begin{definition}\boxed{c \in C}\ Collection Membership 
  \begin{mathpar}
    \inferrule {
    } {
      c \in C\ c 
...
\begin{definition}\boxed{C_l \cong C_r}\ Collection Congruence 
  \begin{mathpar}
    \inferrule {

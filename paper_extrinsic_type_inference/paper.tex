...
\begin{theorem}
  \label{theorem:record_type_elimination}
  Record Type Elimination 
  \\
  \begin{mathpar}
    \inferrule {
...
\pc
For $f \hastype \tau$, we construct an empty type interpretation $\delta$,
...
Theorem \ref{theorem:subtyping_environment_interpretation_contraction},
and Theorem \ref{theorem:typing_interpretation_extension}.
Likewise, we derive $\delta' \cup \delta \hastype \tau \subtypes \J{<}l\J{>}\alpha$ by 
instantiating the applied Theorem \ref{theorem:proof_subtyping_soundness} with $\delta' \cup \delta_a$,
and applying \ref{theorem:proof_typing_skolem_freshness},

...
from which, we derive an interpretation $\delta_r$, such that $\textbf{dom}(\delta_r) \cong \Theta' \backslash \Theta$.
and an interpretation $\delta_e$, such that $\textbf{dom}(\delta_e) \cong \Theta'' \backslash \Theta'$.
We combine these two interpretations into $\delta$, such that $\delta = \delta_r \cup \delta_e$.
...
Next, we introduce an arbitrary interpretation $\delta'$, and we assume
$\delta' \cup \delta \satisfies \Delta''$ and $\delta', \sigma \satisfies \Gamma$.
We derive $\delta' \cup \delta \satisfies r[\sigma] \hastype \tau$
by instantiating the induction hypothesis applied to $r \hastype \tau$ with $\delta'$,
and applying 
...
by instantiating the induction hypothesis applied to $e \hastype \tau'$ with $\delta' \cup \delta_r$,
and applying 
Theorem \ref{theorem:proof_typing_skolem_freshness}
and 
Theorem \ref{theorem:typing_environment_interpretation_contraction}. 
...
we construct an empty type interpretation $\delta$,
such that $\textbf{dom}(\delta) = \epsilon$, which is equivalent to $\textbf{dom}(\delta) = \Theta \backslash \Theta$.
Next, we introduce and arbitrary type interpretation $\delta'$,
...
we apply the induction hypothesis to the typing $e \hastype \tau$, 
from which we derive an interpretation $\delta_a$, such that $\textbf{dom}(\delta_a) = \Theta' \backslash \Theta$.
We also apply Theorem \ref{theorem:proof_subtyping_soundness} (Proof Subtyping Soundness) to
the subtyping $\tau \subtypes \J{<}l\J{>}\alpha$, from which 
we derive an interpretation $\delta_b$, such that $\textbf{dom}(\delta_b) = \Theta'' \backslash \Theta'$. 
We combine these two interpretation into $\delta$, such that $\delta = \delta_a \cup \delta_b$.
Thus, we have $\textbf{dom}(\delta) = \Theta'' \backslash \Theta$ by applying
Theorem \ref{theorem:domain_extension} (Domain Extension) and rewriting. 

Next, we introduce an arbitrary interpretation $\delta'$, and we assume
$\delta' \cup \delta \satisfies \Delta''$ and $\delta', \sigma \satisfies \Gamma$.
Under these assumptions, we derive $\delta' \cup \delta \satisfies e[\sigma] \hastype \tau$
by instantiating the applied induction hypothesis with $\delta'$, 
and applying 
Theorem \ref{theorem:subtyping_environment_prefix_preservation},
...
and 
Theorem \ref{theorem:typing_interpretation_extension}.
Likewise, we derive $\delta' \cup \delta \satisfies \tau \subtypes \J{<}l\J{>}\alpha$ by 
instantiating the applied Theorem \ref{theorem:proof_subtyping_soundness} with $\delta' \cup \delta_a$,
and applying 
Theorem \ref{theorem:proof_typing_skolem_freshness}
and
Theorem \ref{theorem:typing_environment_interpretation_contraction}.

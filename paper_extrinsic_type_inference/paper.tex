...
we apply the induction hypothesis to the typing $e \hastype \tau$, resulting in a statement, which we name $P_a$.
We also apply Theorem \ref{theorem:proof_subtyping_soundness} (Proof Subtyping Soundness) to get a statement, which we 
name $P_b$.
From $P_a$, we derive an interpretation $\delta_a$, such that $\textbf{dom}(\delta_a) = \Theta' \backslash \Theta$. 
From $P_b$, we derive an interpretation $\delta_b$, such that $\textbf{dom}(\delta_b) = \Theta'' \backslash \Theta'$. 
We combine these two interpretation into $\delta$, such that $\delta = \delta_a \cup \delta_b$.
Thus, we have $\textbf{dom}(\delta) = (\Theta' \backslash \Theta) \cup (\Theta'' \backslash \Theta')$ by applying
Theorem \ref{theorem:domain_extension} (Domain Extension), 
which simplifies to $\textbf{dom}(\delta) = \Theta'' \backslash \Theta$.
...
by applying Theorem \ref{theorem:subtyping_environment_interpretation_contraction},
Theorem \ref{theorem:typing_interpretation_extension},
and Theorem \ref{theorem:proof_subtyping_skolem_freshness}.
Likewise, we derive $\delta' \cup \delta \hastype \tau \subtypes \J{<}l\J{>}\alpha$ by (TODO: various theorems).
Then, we have $\delta' \cup \delta \hastype e[\sigma] \hastype \J{<}l\J{>}\alpha$ 
by applying Theorem \ref{subsumption} (Subsumption).
Finall, we get $\delta' \cup \delta \hastype (e[\sigma]).\J{<}l\J{>} \hastype \alpha$ 
by applying Theorem \ref{elimination} (Record Type Elimination),
which simplifies to $\delta' \cup \delta \hastype (e.\J{<}l\J{>})[\sigma] \hastype \alpha$
by applying Definition \ref{sub} (Substitution).


\TODO{remove some tedious details and clean up lemmas}.

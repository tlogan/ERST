...
it delegates to duality correspondence $
      \alpha_\nu \downarrow \vec{\pi}_\nu
      \fallingdotseq 
      \alpha_\mu \uparrow \vec{\pi}_\mu
...
By rewriting an implication type into two weaker implication types,
proof subtyping is able to make further progress than it would without the rewriting.
...
The constraints of $\Delta_\closed$ cannot are valid since the single variable on either side
can't contradict the other side. Therefore, proof subtyping simply adds them
to the input assumptions $(\vec{\alpha}, \Delta \cup \Delta_\closed) \preceq (\vec{\alpha}', \Delta')$ 
and solves the remaining constraints $\Delta_\open \given \vec{\alpha}', \Delta'$. 

To eliminate an closed variable $\alpha \in \vec{\alpha}$,
proof subtyping leverages the predicate for closed variable elimination safety
$(\vec{\alpha}, \Delta) \entails \alpha \subtypes \tau_r \safe$,
which finds an interpretation for $\alpha$ that is
weaker than its constraints suggest, for which proof subtyping still holds.
...
\hfill
\begin{definition} 
  \label{def:closed_variable_elimination_safety}
  Closed Variable Elimination Safety
  \hfill
...
\hfill
\begin{definition} 
  \label{def:closed_variable_introduction_safety}
  Closed Variable Introduction Safety
  \hfill

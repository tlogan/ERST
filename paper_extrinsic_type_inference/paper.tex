...
% \begin{definition}
%   \label{def:multi_progression}
%   Multi Progression 
%   \hfill
%   \boxed{e \rightsquigarrow^\star e'}
%   \\
%   \begin{mathpar}
%     \inferrule { 
%     } {
%       e \rightsquigarrow^\star e 
%     } 
...
\begin{definition}
  \label{def:model_subtyping_environment}
  Model Subtyping Environment
  \hfill
...
\begin{property} 
  \label{prop:colony_sequence_duality_soundness}
  Colony Sequence Duality Correspondence Soundness
  \\
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      \delta\ \alpha_\nu \slash \J{BOT} \cup \delta' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
      \delta\ \alpha_\nu \slash \J{BOT} \cup \delta' \cup \delta^\dagger \satisfies e_f \hastype \tau^\dagger
      )
      )
      \\\\
      \iff
      ...
      \\\\
      \delta\ \alpha_\mu \slash \J{TOP} \cup \delta' \cup \delta^\dagger \satisfies \Delta^\dagger \up
      \delta\ \alpha_\mu \slash \J{TOP} \cup \delta' \cup \delta^\dagger \satisfies (e_a \J{,} e_r) \hastype \tau^\dagger 
      )
      )
      \implies
...
\begin{property} 
  \label{prop:subtyping_environment_duality_soundness}
  Subtyping Environment Duality Soudness
  \\
  \begin{mathpar}
    \inferrule {
      \alpha_\nu \downarrow \Delta_\nu \fallingdotseq \alpha_\mu \uparrow \Delta_\mu
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\\\
      \TODO{ensure bound variables are added to constraints with top upper bound}
      \\\\
      \Theta, \Delta \entails \Omega \cup \textbf{taut}(\Theta_l) \given \Theta', \Delta'
    } {
      \Theta, \Delta \entails
...
\begin{theorem}
  \label{thm:entry_type_elimination}
...
\begin{theorem}
  \label{thm:path_type_elimination}
  Path Type Elimination 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  \label{thm:subtyping_environment_union}
  Subtyping Environment Union 
  \\
  \begin{mathpar}
    \inferrule {
      \delta \satisfies \Delta
...
\begin{theorem}
  \label{thm:subtyping_environment_prefix_preservation}
...
\begin{theorem}
  \label{thm:subtyping_environment_interpretation_contraction}
  Subtyping Environment Interpretation Irrelevance 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  \label{thm:subtyping_environment_interpretation_extension}
  Subtyping Environment Interpretation Irrelevance 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  \label{thm:typing_interpretation_extension}
  Typing Interpretation Irrelevance 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  \label{thm:subtyping_interpretation_extension}
  Subtyping Interpretation Irrelevance 
  \\
  \begin{mathpar}
    \inferrule {
      \textbf{dom}(\delta') \cap \textbf{ftv}(\tau) = \emptyset
      ...
      \delta \satisfies \tau \subtypes \tau' 
      \iff
      \delta \cup \delta' \satisfies \tau \subtypes \tau' 
    }
  \end{mathpar}
\end{theorem}
\hfill
...
\begin{theorem}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \delta_0 \cup \delta_1 \cup \delta_2 \cup \delta_3 \satisfies \tau \subtypes \tau' 
      \iff
      \delta_0 \cup \delta_2 \cup \delta_1 \cup \delta_3 \satisfies \tau \subtypes \tau' 
    }
  \end{mathpar}
\end{theorem}
\hfill
...
\begin{theorem}
  \label{thm:subtyping_environment_interpretation_reordering}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \delta_0 \cup \delta_1 \cup \delta_2 \cup \delta_3 \satisfies \Delta 
      \iff
      \delta_0 \cup \delta_2 \cup \delta_1 \cup \delta_3 \satisfies \Delta 
    }
  \end{mathpar}
\end{theorem}
\hfill
...
\begin{theorem}
  \label{thm:typing_environment_interpretation_extension}
  Typing Environment Interpretation Irrelevance 
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  \label{thm:path_type_elimination_loop}
...
\begin{theorem}
  \label{thm:proof_subtyping_environment_skolem_freshness}
  Proof Subtyping Environment Skolem Freshness 
  \\
  \begin{mathpar}
    \inferrule {
      \Theta, \Delta \entails \Delta_q \given \Theta', \Delta'
...
\begin{theorem}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \delta' \cup \delta \satisfies \tau \subtypes \alpha \J{->} \tau^\dagger
      )
      )
    }
  \end{mathpar}
...
\begin{theorem}
  \label{thm:function_lifting_soundness}
...
\begin{theorem} 
  \label{thm:colony_sequence_duality_soundness}
  Colony Sequence Duality Correspondence Soundness
  \\
  \begin{mathpar}
    \inferrule {
...
\begin{theorem}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \delta \cup \delta' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
      \delta \cup \delta' \cup \delta^\dagger \satisfies e \hastype \tau^\dagger
      )
      ) 
      \\\\
      \iff
    ...
    \inferrule {
      ...
      \delta \cup \delta' \cup \delta^\dagger \satisfies \Delta^\dagger \up
      \delta \cup \delta' \cup \delta^\dagger \satisfies e \hastype \tau^\dagger
      )
      ) 
      \\\\
...
\begin{theorem}
  \label{thm:subtyping_factorization_soundness}
  Subtyping Factorization Soundness 
  \\
  \begin{mathpar}
    % \inferrule {
...
\begin{theorem}
  \label{thm:proof_subtyping_environment_soundness}
  Proof Subtyping Environment Soundness 
  \\
  \begin{mathpar}
    \inferrule {
      \Theta, \Delta \entails \Delta_q \given \Theta', \Delta'
      ...
      \forall \delta' \qua 
      \delta' \cup \delta \satisfies \Delta' \implies
      \delta' \cup \delta \satisfies \Delta_q 
      )
    }
  \end{mathpar}
\end{theorem}
...
\begin{theorem}
  \label{thm:proof_decidable_subtyping_environment_uniqueness}
  Proof Decidable Subtyping Environment Uniqueness 
  \\
  \begin{mathpar}
    \inferrule {
      \Theta, \Delta \entails \Omega \given \Theta', \Delta'
...
% \begin{theorem}
%   \label{thm:proof_decidable_subtyping_environment_completeness}
%   Proof Decidable Subtyping Environment Completeness 
%   \\
%   \begin{mathpar}
%     \inferrule {
...
%       \forall \delta' \qua 
%       \delta' \cup \delta \satisfies \Delta' \implies
%       \delta' \cup \delta \satisfies \Omega 
%       )
%       \\
%       \exists \delta \qua \delta \satisfies \Delta
%     } {
%       \Theta, \Delta \entails \Omega \given \Theta', \Delta'
%     }
%   \end{mathpar}
...
\begin{theorem}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \textbf{dom}(\delta^\dagger) \subseteq \Theta^\dagger
      \implies
      \delta \cup \delta^\dagger \satisfies \Omega
      \implies
      \delta \cup \delta^\dagger \satisfies \Delta''
    }
  \end{mathpar}

  \TODO{some irrelevance notion}

  \TODO{since subtyping holds for all input constraints, then an arbitrary interpretation affecting just the input constraints has no effect}

\end{theorem}

\begin{theorem}
  \label{thm:proof_subtyping_input}
  Proof Subtyping Input 
  \\
  \begin{mathpar}
    \inferrule {
      \Theta, \Delta \entails \tau_l \subtypes \tau_r \given \Theta', \Delta'
      ...
      \forall \delta \qua
      \textbf{dom}(\delta) \subseteq \textbf{ftv}(\Delta)
      \implies
      \delta' \cup \delta \satisfies \Delta
      \implies
      \delta' \cup \delta \satisfies \Delta'
...
\begin{theorem}
  ...
  \begin{mathpar}
    ...
    \TODO{rewrite existential condition to be containment}
  \end{mathpar}
  \\\\
  ...
  \item \I \N For $\delta'$, assume $(\delta' \cup \delta \satisfies \Delta)$
  \item \I\I \N Derive $(\forall e \qua \delta' \cup \delta \satisfies e \hastype \tau \implies \delta' \cup \delta \satisfies e \hastype \tau)$
  \item \I\I \N Derive $(\delta' \cup \delta \satisfies \tau \subtypes \tau)$ 
  ...
  \item \I \N For $\delta'$, assume $(\delta' \cup \delta \satisfies \Delta')$
  \item \I\I \N Derive $(\delta' \cup \delta \satisfies \tau_l \subtypes \tau_r)$ [P]
  \item \I\I \N Derive $(\delta' \cup \delta \satisfies \J{<}l\J{>}\tau_l \subtypes \J{<}l\J{>}\tau_r)$ [Thm. \ref{thm:entry_type_preservation}]
  ...
  \item \I \N And P0: $(
    \forall \delta' \qua 
    \delta \cup \delta_0 \satisfies \Delta' 
    \implies 
    \delta \cup \delta_0 \satisfies \tau_x \subtypes \tau_p
  )$ [IH]
  ...
  \item \I \N And P1: $(
    \forall \delta' \qua 
    \delta \cup \delta_1 \satisfies \Delta' 
    \implies 
    \delta \cup \delta_1 \satisfies \tau_q \subtypes \tau_y
  )$ [IH]

  \item \I \N Derive $(
    \textbf{dom}(\delta_0 \cup \delta_1) = \Theta'' \backslash \Theta
  )$
  \item \I \N For $\delta'$, assume $(\delta' \cup \delta_0 \cup \delta_1 \satisfies \Delta'')$

  \item \I\I \N Derive $(\delta' \cup \delta_0 \cup \delta_1 \satisfies \Delta')$
    [Thm. \ref{thm:proof_subtyping_containment}, Thm. \ref{thm:subtyping_environment_union}]
  \item \I\I \N Derive $(\delta' \cup \delta_0 \satisfies \Delta')$
    [Thm. \ref{thm:proof_subtyping_skolem_freshness}, Thm. \ref{thm:subtyping_environment_interpretation_contraction}]
  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \satisfies \tau_x \subtypes \tau_p
  )$ [P0]

  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \tau_x \subtypes \tau_p
  )$ [Thm. \ref{thm:proof_subtyping_containment}, Thm. \ref{thm:subtyping_interpretation_extension}]

  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \tau_q \subtypes \tau_y
  )$ [P1]

  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \tau_p \J{->} \tau_q \subtypes \tau_x \J{->} \tau_y
  ...
  \item \I \N And P: $(
    ...
    \delta \cup \delta \satisfies \Delta' 
    \implies 
    \delta \cup \delta \satisfies \tau_l \subtypes (\tau_a\J{->}\tau_r)\J{\&}(\tau_b\J{->}\tau_r) 
  )$ [IH]
  \item \I \N For $\delta'$, assume $(\delta' \cup \delta \satisfies \Delta')$
  \item \I\I \N Derive $(
    \delta' \cup \delta \satisfies \tau_l \subtypes (\tau_a\J{->}\tau_r)\J{\&}(\tau_b\J{->}\tau_r)
  ...
  \item \I\I \N Derive $(
    \delta' \cup \delta \satisfies \tau_l \subtypes (\tau_a\J{|}\tau_b)\J{->}\tau_r)
  ...
  \item \I \N And P: $(
    ...
    \delta \cup \delta \satisfies \Delta' 
    \implies 
    \delta \cup \delta \satisfies \tau_l \subtypes 
    (\tau_r\J{->}\tau_a) \J{\&} (\tau_r\J{->}\tau_b)
  )$ [IH]
  \item \I \N For $\delta'$, assume $(\delta' \cup \delta \satisfies \Delta')$
  \item \I\I \N Derive $(
  ...
  \item \I\I \N Derive $(
    \delta' \cup \delta \satisfies \tau_l \subtypes \tau_r\J{->}(\tau_a\J{\&}\tau_b))
  ...
  \item \I\I \N Derive $(
    \delta' \cup \delta \satisfies \tau \subtypes \J{<} l \J{>}(\tau_a\J{\&}\tau_b))
  ...
  \item \I \N And P0: $(
    ...
    \delta \cup \delta_0 \satisfies \Delta' 
    \implies 
    \delta \cup \delta_0 \satisfies \tau_a \subtypes \tau
  )$ [IH]
  ...
  \item \I \N And P1: $(
    ...
    \delta \cup \delta_1 \satisfies \Delta' 
    \implies 
    \delta \cup \delta_1 \satisfies \tau_b \subtypes \tau
  )$ [IH]
  ...
  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \tau_a \subtypes \tau
  )$ [Thm. \ref{thm:proof_subtyping_containment}, Thm. \ref{thm:subtyping_interpretation_extension}]

  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \tau_b \subtypes \tau
  )$ [P1]

  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \tau_a \J{|} \tau_b \subtypes \tau
  ...
  \item \I \N Let $\delta_0$ be, such that $(
    \textbf{dom}(\delta) \subseteq \Theta' \backslash \Theta
  )$ [Thm. \ref{thm:proof_subtyping_environment_soundness}]

  

  \item \I \N \TODO{EXTRA?} And P0: $(
      \forall \delta' \qua
      \delta' \cup \delta_0 \satisfies \Delta'
      \implies
      \delta' \cup \delta_0 \satisfies \Omega
  )$ [Thm. \ref{thm:proof_subtyping_environment_soundness}]
  ...
  \item \I \N And P1: $(
    ...
    \delta' \cup \delta_1 \satisfies \Delta''
    \implies
    \delta' \cup \delta_1 \satisfies \tau_l \subtypes \tau_r
  )$ [IH]


  \item \I \N Derive $(
    \Theta_l \subseteq \textbf{ftv}(\Omega \cup \textbf{taut}(\Theta_l))
  )$ [...]
  ...
  \item \I \N Derive $(
    \textbf{dom}(\delta_0 \cup \delta_1) \subseteq \Theta'' \backslash \Theta \backslash \Theta_l
  )$

  \item \I \N Derive $(
    \textbf{dom}(\delta_0 \cup \delta_1) \subseteq \Theta'' \backslash \Theta
  )$

  \item \I \N For $\delta'$, assume $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \Delta''
  )$ 
  ...
  %   \implies
  %   \delta' \cup \delta_l \cup \delta_0 \cup \delta_1 \satisfies \Delta'' 
  %   \implies
  %   \delta' \cup \delta_l \cup \delta_0 \cup \delta_1 \satisfies \tau_l \subtypes \tau_r
  % )$ [P1]
  ...
  %   \textbf{dom}(\delta_l) \subseteq \Theta_l
  %   \implies
  %   \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \Delta'' 
  %   \implies
  %   \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \tau_l \subtypes \tau_r
  % )$ [Thm. \ref{thm:subtyping_interpretation_reordering}, Thm \ref{thm:subtyping_environment_interpretation_reordering}]

  % \item \I\I \N Derive $(
  %   \delta' \cup \delta_l \cup \delta_0 \cup \delta_1 \satisfies \Delta'
  % )$ [Thm. \ref{thm:proof_subtyping_containment}, Thm. \ref{thm:subtyping_environment_union}]
  % \item \I\I \N Derive $(
  %   \delta' \cup \delta_l \cup \delta_0 \satisfies \Delta'
  % )$ [Thm. \ref{thm:proof_subtyping_skolem_freshness}, Thm. \ref{thm:subtyping_environment_interpretation_contraction}]
  % \item \I\I \N Derive $(
  %   \delta' \cup \delta_l \cup \delta_0 \satisfies \Omega 
  % )$ [P0]
  % \item \I\I \N Derive $(
  %   \delta' \cup \delta_l \cup \delta_0 \cup \delta_1 \satisfies \Omega 
  % )$ [Thm. \ref{thm:proof_subtyping_skolem_freshness}, 
  %   Thm. \ref{thm:proof_subtyping_environment_containment}
  %   Thm. \ref{thm:subtyping_environment_interpretation_extension}]
  ...
  \item \I\I \N Derive $(
    ...
    \implies
    \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \Delta''
  )$ [Thm \ref{thm:proof_subtyping_input} (Proof Subtyping Input)]

  \item \I\I \N  \TODO{generalize with uniqueuess}

  \item \I\I \N  \TODO{switch antecedent condition from $\Delta'$ to $\Omega$}

  \item \I\I \N Derive $(
    \forall \delta_l \qua
    \textbf{dom}(\delta_l) \subseteq \Theta_l 
    \implies
    \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \Omega
    \implies
    \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \Delta''
  )$ [Thm. CRUX]
  ...
  \item \I\I \N Derive $(
    ...
    \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \Omega
    \implies
    \delta' \cup \delta_0 \cup \delta_l \cup \delta_1 \satisfies \Delta''
  ...
  \item \I\I \N Derive $(
    ...
    \delta' \cup \delta_0 \cup \delta_1 \cup \delta_l \satisfies \Omega
    \implies
    \delta' \cup \delta_0 \cup \delta_l \cup \delta_1 \satisfies \tau_l \subtypes \tau_r
  )$ [P1] 
  ...
  \item \I\I \N Derive $(
    \delta' \cup \delta_0 \cup \delta_1 \satisfies \J{EXI[} \Theta_l \J{]} \Omega\ \tau_l \subtypes \tau_r
...
\pc
For $\Theta, \Delta, \Gamma \entails \J{@} \hastype \J{@} \given \Theta, \Delta$,
we derive the conclusion by applying Definition \ref{def:model_typing} (Model Typing).

\pc
For $\Theta, \Delta, \Gamma \entails x \hastype \tau \given \Theta, \Delta$, 
we derive the conclusion by applying Theorem \ref{thm:typing_environment_soundness}
(Typing Environment Soundness).

\pc
For $\Theta, \Delta, \Gamma \entails \epsilon \hastype \J{TOP} \given \Theta, \Delta$,
we derive the conclusion using Definition \ref{def:model_typing} (Model Typing).

\pc
For $r\ \J{<}l \J{>} e \hastype (\tau\ \J{\&}\ \J{<} l \J{>} \tau') \given \Theta'', \Delta''$,
we have 
$\Theta, \Delta, \Gamma \entails r \hastype \tau \given \Theta', \Delta'$
and
$\Theta', \Delta', \Gamma \entails e \hastype \tau' \given \Theta'', \Delta''$
by applying Definition \ref{def:proof_typing}.
.
We derive an interpretation $\delta_r$, such that 
$\textbf{dom}(\delta_r) \cong \Theta' \backslash \Theta$
and $
\forall \delta', \sigma \qua 
\delta' \cup \delta_r \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \cup \delta_r \satisfies r[\sigma] \hastype \tau
$ (P0)
by applying the induction hypothesis.
We derive an interpretation $\delta_e$, such that 
$\textbf{dom}(\delta_e) \cong \Theta'' \backslash \Theta'$
and $
\forall \delta', \sigma \qua 
\delta' \cup \delta_e \satisfies \Delta'' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \cup \delta_e \satisfies e[\sigma] \hastype \tau'
$ (P1)
by applying the induction hypothesis.

Next, we derive $\textbf{dom}(\delta_r \cup \delta_e) = \Theta'' \backslash \Theta$
by applying Theorem \ref{thm:domain_extension} and rewriting with set arithmetic.
Then, we introduce an arbitrary typing interpretation $\delta'$
and an arbitrary expression interpretation $\sigma$,
and we assume 
$\delta' \cup \delta_r \cup \delta_e \satisfies \Delta''$ 
and 
$\delta', \sigma \satisfies \Gamma$.

We derive $\delta' \cup \delta_r \cup \delta_e \satisfies \Delta'$ 
by applying
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:subtyping_environment_prefix_preservation},
and rewriting with set arithmetic.
We derive $\delta' \cup \delta_r \satisfies \Delta'$ 
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
and
Theorem \ref{thm:subtyping_environment_interpretation_contraction}.
Then, we have $\delta' \cup \delta_r \satisfies r[\sigma] \hastype \tau$ 
by instantiating (P0) with the $\delta'$ and $\sigma$, and applying the result.  
Thus, we have $\delta' \cup \delta_r \cup \delta_e \satisfies r[\sigma] \hastype \tau$ 
by applying 
Theorem \ref{thm:proof_typing_skolem_freshness}
Theorem \ref{thm:proof_typing_containment},
rewriting with set arithmetic,
and applying
Theorem \ref{thm:typing_interpretation_extension}.


We derive $\delta' \cup \delta_r, \sigma \satisfies \Gamma$ 
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
and 
Theorem \ref{thm:typing_environment_interpretation_extension}.
Then, we have $\delta' \cup \delta_r \cup \delta_e \satisfies e[\sigma] \hastype \tau' $
by instantiating (P1) with $\delta' \cup \delta_r$ and $\sigma$, and applying the result.


We derive $\delta' \cup \delta_r \cup \delta_e \satisfies r[\sigma]\ \J{<}l\J{>} e[\sigma] \hastype \tau$
by applying Theorem \ref{thm:record_extension_preservation}
We derive $\delta' \cup \delta_r \cup \delta_e \satisfies r[\sigma]\ \J{<}l\J{>} e[\sigma] \hastype \tau'$
by applying Theorem \ref{thm:entry_type_introduction}.
Then, we derive 
$\delta' \cup \delta \satisfies r[\sigma]\ \J{<}l\J{>} e[\sigma] \hastype \tau \J{\&} \tau'$
by applying Definition \ref{def:model_typing} (Model Typing).
Finally, we get $\delta' \cup \delta \satisfies (r\ \J{<}l\J{>} e)[\sigma] \hastype \tau \J{\&} \tau'$
by rewriting with Definition \ref{...} (Substitution).


\pc
For $\Theta, \Delta, \Gamma \entails f \hastype \tau \given \Theta, \Delta$, 
we have 
$\exists \Xi \qua \Theta, \Delta, \Gamma \entails f \liftfun \Pi \sim \Xi$,
$\Theta \cup \textbf{ftv}(\Delta) \cup \textbf{ftv}(\Gamma) = \dTheta$,
and 
...
We know that $\textbf{dom}(\epsilon) = \Theta \backslash \Theta$ by rewriting with set arithmetic.
Next, we introduce an arbitrary type interpretation $\delta'$ and an arbitrary expression interpretation $\sigma$,
and we assume $\delta' \cup \epsilon \satisfies \Delta$ and $\delta', \sigma \satisfies \Gamma$.
With a little rewriting, we have $\delta' \satisfies \Delta'$.
Then, we derive $\delta' \satisfies f[\sigma] \hastype \tau$ by applying Theorem \ref{thm:function_lifting_soundness}, 
Theorem \ref{thm:function_lifting_skolem_freshness}, and Theorem \ref{prop:packaging_soundness}, along with a little
rewriting.
Thus, we have $\delta' \cup \delta \satisfies f[\sigma] \hastype \tau$ 
by rewriting the statement to include the empty interpretation.



\pc
For $\Theta, \Delta, \Gamma \entails e \J{.} l \hastype \alpha \given \Theta'', \Delta''$,
we have
$\Theta, \Delta, \Gamma \entails e \hastype \tau \given \Theta', \Delta'$
and
...
We derive an interpretation $\delta_a$, such that $\textbf{dom}(\delta_a) = \Theta' \backslash \Theta$
and $
\forall \delta', \sigma \qua 
\delta' \cup \delta_a \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \cup \delta_a \satisfies r[\sigma] \hastype \tau
$ (P0)
by applying the induction hypothesis.
We derive an interpretation $\delta_b$, such that $\textbf{dom}(\delta_b) = \Theta'' \backslash \Theta'$
and $
\forall \delta', \sigma \qua 
\delta' \cup \delta_b \satisfies \Delta'' \implies
\delta' \cup \delta_b \satisfies \tau \hastype \tau
$ (P1)
by applying Theorem \ref{thm:proof_subtyping_soundness}.

Next, we derive $\textbf{dom}(\delta_a \cup \delta_b) = \Theta'' \backslash \Theta$ by applying
Theorem \ref{thm:domain_extension} and rewriting with set arithmetic. 
Then, we introduce an arbitrary type interpretation $\delta'$ and an arbitrary expression interpretation $\sigma$,
and we assume 
$\delta' \cup \delta_a \cup \delta_b \satisfies \Delta''$ and $\delta', \sigma \satisfies \Gamma$.

We derive $\delta' \cup \delta_a \cup \delta_b \satisfies \Delta'$ 
by applying 
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:subtyping_environment_prefix_preservation},
and rewriting with set arithmetic.
We derive $\delta' \cup \delta_a \satisfies \Delta'$ 
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness}
and 
Theorem \ref{thm:subtyping_environment_interpretation_contraction}.
Then, we have $\delta' \cup \delta_a \satisfies e[\sigma] \hastype \tau$ 
by instantiating (P0) with the $\delta'$ and $\sigma$, and applying the result.  
Thus, we have $\delta' \cup \delta_a \cup \delta_b \satisfies r[\sigma] \hastype \tau$ 
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness}
Theorem \ref{thm:proof_typing_containment},
rewriting with set arithmetic,
...
We derive $\delta' \cup \delta_a \cup \delta_b \satisfies \tau \subtypes \J{<}l\J{>}\alpha$ by 
instantiating (P1) with $\delta' \cup \delta_a$ and $\sigma$.

Next, we have $\delta' \cup \delta \satisfies e[\sigma] \hastype \J{<}l\J{>}\alpha$ 
by applying Theorem \ref{subsumption} (Subsumption).
Then, we get $\delta' \cup \delta \satisfies (e[\sigma])\J{.}l \hastype \alpha$ 
by applying Theorem \ref{elimination} (Record Type Elimination),
which simplifies to $\delta' \cup \delta \satisfies (e\J{.}l)[\sigma] \hastype \alpha$
by applying Definition \ref{sub} (Substitution).



\pc
For $(\Theta, \Delta, \Gamma \entails e_f \J{(} e_a \J{)} \hastype \alpha \given \Theta''', \Delta''')$,
we have 
$(\Theta, \Delta, \Gamma \entails e_f \hastype \tau_f \given \Theta', \Delta')$,
$(\Theta', \Delta', \Gamma \entails e_a \hastype \tau_a \given \Theta'', \Delta'')$,
and
...
We derive a type interpretation $\delta_f$, such that
$(\textbf{dom}(\delta_f) = \Theta' \backslash \Theta)$
and $(
\forall \delta', \sigma \qua 
\delta' \cup \delta_f \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \cup \delta_f \satisfies e_f[\sigma] \hastype \tau_f
)$ (P0)
by applying the induction hypothesis.
We derive a type interpretation $\delta_a$, such that
$(\textbf{dom}(\delta_a) = \Theta'' \backslash \Theta')$
and $(
\forall \delta', \sigma \qua 
\delta' \cup \delta_a \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \cup \delta_a \satisfies e_a[\sigma] \hastype \tau_a
)$ (P1)
by applying the induction hypothesis.
We derive a type interpretation $\delta_s$, such that
$(\textbf{dom}(\delta_s) = \Theta''' \backslash \Theta'')$
and $(
\forall \delta' \qua 
\delta' \cup \delta_s \satisfies \Delta' \implies
\delta' \cup \delta_s \satisfies \tau_f \subtypes \tau_a \J{->} \alpha
)$ (P2)
by applying Theorem \ref{thm:proof_subtyping_soundness}.

Next, we derive $(\text{dom}(\delta_f \cup \delta_a \cup \delta_s) = \Theta''' \backslash \Theta)$
by applying
Theorem \ref{thm:domain_extension}
and
rewriting with set arithmetic.
The, we introduce an arbitrary type interpretation $\delta'$ and 
an arbitrary expression interpretation $\sigma$, and
we assume 
$(\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies \Delta''')$
and
$(\delta' \satisfies \Gamma)$.

We derive 
$(\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_subtyping_containment},
Theorem \ref{thm:proof_typing_containment},
and
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \cup \delta_f \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_subtyping_skolem_freshness},
Theorem \ref{thm:proof_subtyping_containment},
Theorem \ref{thm:proof_typing_skolem_freshness},
...
$(\delta' \cup \delta_f \satisfies e_f[\sigma] \hastype \tau_f)$
by instantiating (P0)
with $\delta'$ and $\sigma$ and applying the result.
We derive
$(\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies e_f[\sigma] \hastype \tau_f)$
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness},
Theorem \ref{thm:proof_subtyping_containment},
Theorem \ref{thm:proof_typing_skolem_freshness},
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \cup \delta_f \cup \delta_a \satisfies \Delta'')$
by applying
Theorem \ref{thm:proof_subtyping_skolem_freshness},
and
Theorem \ref{thm:subtyping_environment_interpretation_contraction}.
Then, we derive
$(\delta' \cup \delta_f \satisfies \Gamma)$
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
and
Theorem \ref{thm:typing_interpretation_extension}.
Then, we derive
$(\delta' \cup \delta_f \cup \delta_a \satisfies e_a[\sigma] \hastype \tau_a)$
by instantiating (P1)
with $\delta' \cup \delta_f$ and $\sigma$ and applying the result.
We derive
$(\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies e_a[\sigma] \hastype \tau_a)$
by applying 
Theorem \ref{thm:proof_subtyping_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
and
...
We derive
$(\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies \tau_f \subtypes \tau_a \J{->} \alpha)$
by instantiating (P2) with $\delta' \cup \delta_f \cup \delta_a$, and applying the result.
Then, we derive $\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies e_f[\sigma] \hastype \tau_a \J{->} \alpha$
by applying 
Theorem \ref{thm:subsumption}.
Thus, we have $\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies e_f[\sigma]\J{(}e_a[\sigma]\J{)} \hastype \alpha$
by Theorem \ref{thm:path_type_elimination},
which simplifies to $\delta' \cup \delta_f \cup \delta_a \cup \delta_s \satisfies e_f\J{(}e_a\J{)}[\sigma] \hastype \alpha$,
by Definition \ref{def:substitution}.

\pc
For $(\Theta, \Delta, \Gamma \entails \J{let } x \J{:} \tau_a \J{ = } e \J{ in } e' \hastype \tau' \given \Theta''', \Delta''')$,
we have
$(\Theta, \Delta, \Gamma \entails e \hastype \tau \given \Theta', \Delta')$,
$(\Theta', \Delta' \entails \tau \subtypes \tau_a \given \Theta'', \Delta'')$,
and
...
We derive a typing interpretation $\delta_a$, such that 
$(\textbf{dom}(\delta_a) = \Theta' \backslash \Theta)$
and $(
\forall \delta', \sigma \qua 
\delta' \cup \delta_a \satisfies \Delta' \implies
\delta', \sigma \satisfies \Gamma \implies
\delta' \cup \delta_a \satisfies e[\sigma] \hastype \tau
)$ (P0)
by applying the induction hypothesis.
We derive a typing interpretation $\delta_b$, such that 
$(\textbf{dom}(\delta_b) = \Theta'' \backslash \Theta')$
and $(
\forall \delta', \sigma \qua 
\delta' \cup \delta_b \satisfies \Delta'' \implies
\delta' \cup \delta_b \satisfies \tau \hastype \tau_a
)$ (P1)
by applying Theorem \ref{thm:proof_subtyping_soundness}.
We derive a typing interpretation $\delta_c$, such that 
$(\textbf{dom}(\delta_c) = \Theta''' \backslash \Theta'')$
and $(
\forall \delta', \sigma \qua 
\delta' \cup \delta_c \satisfies \Delta''' \implies
\delta', \sigma \satisfies \Gamma\ x \hastype \tau_a \implies
\delta' \cup \delta_c \satisfies e'[\sigma] \hastype \tau'
)$ (P2)
by applying the induction hypothesis.

Next, we derive $(\textbf{dom}(\delta_a \cup \delta_b \cup \delta_c) \cong \Theta''' \backslash \Theta)$
by applying
Theorem \ref{thm:domain_extension} 
and rewriting with set arithmetic.
Then, we introduce an arbitrary type interpretation $\delta'$
and an arbitrary expression interpretation $\sigma$,
and we assume 
$(\delta' \cup \delta_a \cup \delta_b \cup \delta_c \satisfies \Delta''')$
and
$(\delta, \sigma \satisfies \Gamma)$.
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \cup \delta_a \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:proof_subtyping_skolem_freshness},
...
with $\delta'$ and $\sigma$ and applying the result.
We derive
$(\delta' \cup \delta_a \cup \delta_b \cup \delta_c \satisfies e[\sigma] \hastype \tau)$
by applying 
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:proof_subtyping_skolem_freshness},
...
Theorem \ref{thm:subtyping_environment_prefix_preservation}.
Then, we derive
$(\delta' \cup \delta_a \cup \delta_b \satisfies \Delta')$
by applying
Theorem \ref{thm:proof_subtyping_skolem_freshness},
and
Theorem \ref{thm:subtyping_environment_interpretation_contraction}.
We derive
$(\delta' \cup \delta_a \cup \delta_b \satisfies \tau \subtypes \tau_a)$
by instantiating (P1)
with $\delta' \cup \delta_a$ and applying the result.
We derive
$(\delta' \cup \delta_a \cup \delta_b \cup \delta_c \satisfies \tau \subtypes \tau_a)$
by applying 
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_subtyping_containment},
and
Theorem \ref{thm:subtyping_interpretation_extension}.


We derive 
$(\delta' \cup \delta_a \cup \delta_b \satisfies \Gamma)$
by applying
Theorem \ref{thm:proof_typing_skolem_freshness},
Theorem \ref{thm:proof_typing_containment},
Theorem \ref{thm:proof_subtyping_skolem_freshness},
...
and
Theorem \ref{thm:typing_environment_interpretation_extension}.
Then, we derive $\delta' \cup \delta_a \cup \delta_b, \sigma \satisfies e[\sigma] \hastype \tau_a$
by applying Theorem \ref{thm:subsumption}.
Then, We derive $\delta' \cup \delta_a \cup \delta_b, \sigma\ x \slash e[\sigma] \satisfies \Gamma\ x \hastype \tau$
by applying Definition \ref{def:model_sequence_typing} (Model Sequence Typing).
Then, we derive
$(\delta' \cup \delta_a \cup \delta_b \cup \delta_c \satisfies e'[\sigma\ x \slash e[\sigma]] \hastype \tau')$
by instantiating (P2) with $\delta' \cup \delta_a \cup \delta_b$ and $\sigma\ x \slash e[\sigma]$,
applying the result.


We derive $\delta' \cup \delta_a \cup \delta_b \cup \delta_c \satisfies e'[\sigma][x \slash e[\sigma]] \hastype \tau'$,
by rewriting with Definition \ref{def:substitution}.
Then, we have 
$\delta' \cup \delta \entails \J{let } x \J{:} \tau_a \J{ = } e[\sigma] \J{ in } e'[\sigma] \hastype \tau'$
by applying Theorem \ref{thm:annotated_binding_introduction},
which simplifies to 
$\delta' \cup \delta \entails (\J{let } x \J{:} \tau_a \J{ = } e \J{ in } e')[\sigma] \hastype \tau'$
by applying Definition \ref{def:substitution}. 
...
We derive a typing interpretation $\delta$, such that 
$(\textbf{dom}(\delta) = \Theta' \backslash \Theta)$
and $(
\forall \delta', \sigma \qua 
\delta' \cup \delta \satisfies \Delta' \implies
...
by applying the induction hypothesis.
Next, we introduce an arbitrary type interpretation $\delta'$
and an arbitrary expression interpretation $\sigma$,
and we assume 
$\delta' \cup \delta \satisfies \Delta'$
and $\delta', \sigma \satisfies \Gamma$.
We derive $\delta' \cup \delta \satisfies e[\sigma] \hastype \tau$
by instantiating (P0) with $\delta'$ and $\sigma$, and
applying the result. 
Additionally, we derive 
$\forall \delta'' \qua \textbf{dom}(\delta'') \cap \textbf{ftv}(\Delta') \implies \delta' \cup \delta \cup \delta'' \satisfies \Delta'$
(P1)
by applying 
Theorem \ref{thm:subtyping_environment_interpretation_contraction}.
We derive 
$\forall \delta'' \qua \textbf{dom}(\delta'') \cap \textbf{ftv}(\Delta') \implies \delta' \cup \delta \cup \delta'' \satisfies e[\sigma] \hastype \tau$
(P2)
by applying
Theorem \ref{thm:proof_typing_containment},
and
...
(
\forall \delta' \qua
\delta' \cup \delta^\dagger \satisfies \Delta' \implies
\delta' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
\delta' \cup \delta^\dagger \satisfies \tau \subtypes \alpha \J{->} \tau^\dagger
)
)
$ (P3)
by applying
Theorem \ref{thm:function_type_explosion_soundness}
and
Theorem \ref{thm:subtyping_environment_union}.
We derive
$(
\forall \Theta^\dagger, \Delta^\dagger, \tau^\dagger \qua 
(\Theta^\dagger, \Delta^\dagger, \tau^\dagger) \in \Pi_\nu \implies
...
(
\forall \delta'' \qua
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies \Delta' \implies
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies \tau \subtypes \alpha \J{->} \tau^\dagger
)
)
$
by rewriting and partially instantiating (P3).
Then, we have
$(
\forall \Theta^\dagger, \Delta^\dagger, \tau^\dagger \qua 
(\Theta^\dagger, \Delta^\dagger, \tau^\dagger) \in \Pi_\nu \implies
...
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies e[\sigma] \hastype \alpha \J{->} \tau^\dagger
)
)
$
by applying (P1), (P2), and Theorem \ref{thm:subsumption}.
...
\forall \delta'' \qua
\textbf{dom}(\delta'') \cap \textbf{ftv}(\Delta') \implies
\delta' \cup \delta\ \alpha \slash \J{BOT} \cup \delta'' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
\delta' \cup \delta\ \alpha \slash \J{BOT} \cup \delta'' \cup \delta^\dagger \satisfies e[\sigma] \hastype \alpha \J{->} \tau^\dagger
)
)
$
by applying 
Theorem \ref{...} (weakest interpretation of alpha).
...
\textbf{dom}(\delta'') \cap \textbf{ftv}(\Delta') \implies
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies \Delta^\dagger \implies
\delta' \cup \delta \cup \delta'' \cup \delta^\dagger \satisfies \J{loop(}e[\sigma]\J{)} \hastype \tau^\dagger
)
)
$
by Theorem \ref{thm:path_type_elimination_loop}.

We derive
$(
\forall e_a, e_r \qua 
\delta' \cup \delta\ \alpha_\mu \slash \J{TOP} \satisfies (e_a\J{,}e_r) \hastype \tau_\mu
\implies 
e_r \cong \J{loop(}e[\sigma]\J{)(} e_a \J{)}
)$
by applying 
Theorem \ref{...} (Duality Soundness)
and
Theorem \ref{thm:packaging_soundness}.

We derive
$(
\forall e_a, e_r \qua 
\delta' \cup \delta \satisfies (e_a\J{,}e_r) \hastype \J{LFP[}\alpha_\mu\J{]}\tau_\mu
\implies 
e_r \cong \J{loop(}e[\sigma]\J{)(} e_a \J{)}
)$
by applying 
Theorem \ref{thm:least_fixed_point_type_elimination}.
...
\delta' \cup \delta \satisfies (e_a\J{,}e_r) \hastype \J{LFP[}\alpha_\mu\J{]}\tau_\mu
\implies 
\delta' \cup \delta \satisfies e_r \hastype \tau_r 
\up
e_r \cong \J{loop(}e[\sigma]\J{)(} e_a \J{)}
)$
by applying 
Theorem \ref{thm:subtyping_factorization_soundness}.
Theorem \ref{thm:subsumption},
Theorem \ref{thm:entry_type_elimination},
and
Theorem \ref{thm:right_congruence}.


We derive
$(
\forall e_a, e_r \qua 
\delta' \cup \delta \satisfies e_a \hastype \tau_l 
\implies 
\delta' \cup \delta \satisfies e_r \hastype \tau_r 
\up
e_r \cong \J{loop(}e[\sigma]\J{)(} e_a \J{)}
...
by applying 
Theorem \ref{thm:left_congruence}
and
Theorem \ref{thm:subtyping_factorization_soundness}.
...
\delta' \cup \delta \satisfies e_a \hastype \tau_l 
\implies 
\delta' \cup \delta \satisfies \J{loop(}e[\sigma]\J{)(} e_a \J{)} \hastype \tau_r 
)$
by applying 
Theorem \ref{thm:expression_congruence_typing}.

We have 
$(
\delta' \cup \delta  \satisfies \J{loop(}e[\sigma]\J{)} \hastype \tau_l \J{->}\tau_r 
)$
by applying 
Theorem \ref{thm:path_type_elimination_loop}

We have 
$(
\delta' \cup \delta  \satisfies \J{loop(}e\J{)}[\sigma] \hastype \tau_l \J{->}\tau_r 
)$
by applying 
Definition \ref{...} (Substitution)

*********************************************************************


\TODO{update proof formatting to make easier to read and edit}.
...
%       $ by theorem \ref{thm:fixpoint_duality_soundness}
...
%   \label{thm:fixpoint_duality_soundness}
%   \begin{mathpar}
%     \inferrule { 
%       \tau \subtypes \J{$\alpha_l$->$\alpha_r$} \given Z_1
%       \\
...
%   \label{thm:fixpoint_duality_soundness_old}
%   \begin{mathpar}
%     \inferrule { 
%       Z_0 \looparrowright Z_1
%       \\

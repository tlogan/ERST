...
\begin{theorem}
  \label{theorem:empty_extension_equivalence}
  Empty Extension Equivalence 
  \\
  \begin{mathpar}
    \inferrule {
    } {
...
\begin{theorem}
  \label{theorem:subtyping_environment_interpretation_elimination}
  Subtyping Environment Interpretation Elimination 
  \\
  \begin{mathpar}
    \inferrule {
      \delta \cup \delta' \cup \delta'' \satisfies \Delta
      \\
      \text{dom}(\delta') \cap \text{FTV}(\Delta) = \epsilon
    } {
      \delta \cup \delta'' \satisfies \Delta
    }
  \end{mathpar}
...
\begin{theorem}
  \label{theorem:typing_interpretation_introduction}
  Typing Interpretation Introduction 
  \\
  \begin{mathpar}
    \inferrule {
      \delta \cup \delta'' \satisfies e \hastype \tau 
      \\
      \text{dom}(\delta') \cap \text{FTV}(\tau) = \epsilon
    } {
      \delta \cup \delta' \cup \delta'' \satisfies e \hastype \tau 
    }
  \end{mathpar}
\end{theorem}
\hfill
...
\begin{theorem}
  ...
  \begin{mathpar}
    ...
    % \TODO{if we distinguish between input and output worlds}

    % \TODO{then can we generalize over the common input variables?}

    % \TODO{notice that input variables are used to detect errors}

    % \TODO{if no errors are detected, then for any interpretation of input variables}

    % \TODO{there is an existential interpretation for the generated variables, such that .... the typing holds}

    % \TODO{then we can choose a single interpretation that saitisfies assumptions of two separate IHs}
    ...
    \inferrule {
      ...
      \text{dom}(\delta) \cong \Theta \cup \text{FTV}(\Delta) \cup \text{FTV}(\Gamma) \implies
      ...
      (
      \forall \delta'' .\ 
      \delta'' \cup \delta' \cup \delta \satisfies \Delta' \implies
      \delta'' \cup \delta' \cup \delta \satisfies e[\sigma] \hastype \tau
      )
    }
  \end{mathpar}
\end{theorem}

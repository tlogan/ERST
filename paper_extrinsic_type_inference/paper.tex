...
\paragraph{Finite Paths Selection.}
Consider the application \J{talky(x)} where \J{x} has the type \J{(<hello> @)|(<thank> @)}. 
...
Now consider a function \J{boo} whose input type is the natural numbers. 
Suppose we have the application \J{boo(n)} where n guaranteed to be an even number.
\[
  \begin{array}[t]{@{} l}
      \J{LFP[R]( BOT} 
      \\
...
In the case of comparing two relations such as \text{natList}(\J{T}) and a corresponding even version, 
...
Consider two functions: \J{uno} of type \J{U->V} and \J{dos} of type \J{D->E}. 
Now suppose these two functions are called on the same variable, e.g. \J{(uno(x),dos(x))}.
Type inference learns that the type of \J{x} can be no weaker than the intersection
of the functions' input types: \J{U\&D}. 

\TODO{note the connection to principal type}

\TODO{consinder using functional example from SuperF: \J{foo f = (f 123, f True)}}



\paragraph{Infinite Path Factorization} 

\TODO{Note how we separate immediately on intrudction of paths, rather than delyaing until elimination rule}
\TODO{this limitation will be rectified in the follow up paper by leveraging reasoning over inductive relational types}
\TODO{In this work, the inductive relational type is syntactically valid but not semantically valid}
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      \alpha \slash \tau \notin \vec{\delta}
      \\
      \vec{\delta}, \Gamma \satisfies e \hastype \J{TOP} 
    } {
      \vec{\delta}, \Gamma \satisfies e \hastype \alpha 
    } 
    ...
    \inferrule { 
      ...
      \vec{\delta}, \Gamma \satisfies \J{@} \hastype \J{@}
    } 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % \inferrule { 
    %   F = \epsilon \lor \vec{\delta}, \Gamma \satisfies F \hastype \J{$\tau_l$->$\tau_r$}
    %   \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % \inferrule { 
    %   \vec{\delta}, \Gamma \satisfies F \hastype \J{$\tau_n$->$\tau_r$}
    %   \\
    %   \vec{\delta}, \Gamma \sqcup \Gamma' \satisfies p \pattype \tau_p
    %   \\
    %   \vec{\delta}, \Gamma \sqcup \Gamma'  \satisfies e \hastype \tau_r
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \inferrule { 
      \vec{\delta}, \Gamma \satisfies e \hastype \J{$\tau_l$}
    } {
      \vec{\delta}, \Gamma \satisfies e \hastype \J{$\tau_l$|$\tau_r$}
    } 

    \inferrule { 
      \vec{\delta}, \Gamma \satisfies e \hastype \J{$\tau_r$}
    } {
      \vec{\delta}, \Gamma \satisfies e \hastype \J{$\tau_l$|$\tau_r$}
    } 
    ...
    \inferrule { 
      ...
      \vec{\delta} \satisfies e \hastype \J{EXI[$\Alpha$ $Q$]$\tau$}
    } 
    ...
    \inferrule { 
      ...
      \vec{\delta}, \Gamma \satisfies e \hastype \J{ALL[$\Alpha$ $Q$]$\tau$}
    } 

    \inferrule { 
      \vec{\delta} \ \alpha \slash \J{LFP[$\alpha$]$\tau$} \satisfies e \hastype \tau
    } {
      \vec{\delta}, \Gamma \satisfies e \hastype \J{LFP[$\alpha$]$\tau$}
    } 
...
type forms. Rather, the progression of expressions is leveraged to eliminate
...
of a sound type proof system that can directly eliminate type forms. 
...
\begin{definition}\boxed{\vec{\delta} \satisfies Q}\ Model Sequence Subtyping
  ...
  \begin{mathpar}
    ...
    \inferrule { 
      ...
      \vec{\delta} \satisfies Q\ \J{.$\tau_l$<:$\tau_r$}
    } 
  \end{mathpar}
\end{definition}
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \text{desugar}(\J{TOP}) = \J{EXI[} \alpha \J{]} \alpha
    }

    \inferrule {
    } {
      \text{desugar}(\J{BOT}) = \J{ALL[} \alpha \J{]} \alpha
    }

    \inferrule {
    } {
      \text{desugar}(\tau_l \J{*} \tau_r) = (\J{left} \fieldmap \tau_l)  \J{*} (\J{right} \fieldmap \tau_r)
    }
  \end{mathpar}
\end{definition} 
\hfill
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \Gamma \entails \vec{r}\ \J{?}l \fieldmap e \hastype \tau\ \J{\&}\ l \fieldmap \tau' \given \Omega' 
    }
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      l \J{->} \tau_l \subtypes l \J{->} \tau_r \given \Omega 
    }
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \tau_{l}\J{\&}\tau_{r}  \subtypes \kappa \given \Omega 
    }
...
\begin{definition} 
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \kappa \subtypes \tau_{l}\J{|}\tau_{r} \given \Omega 
    }
...
\begin{theorem}(Fixpoint duality soundness old)
  ...
  $
  \item \Z $\contin
    N\ \alpha_{h^-}, M, \Delta_i \sqcup \Delta_h \entails \J{\&$(\Tau_l)$*|$(\Tau_r)$} \cong^{-} \tau_i
  $
  \item \Z $\B{hypo }
    N \entails 
    \alpha_{h^+} \cdot Z \cdot \J{$\alpha_l$->$\alpha_r$} 
...
\begin{theorem}\ Influential soundness 
  ...
  \begin{mathpar}
    \inferrule { 
      ...
      \\\\
      N, M, \Delta \sqcup \Delta' \given \J{\&$(\Tau_l)$*|$(\Tau_r)$} \cong^- \tau
      \\
      N, M, \Delta_i \sqcup \Delta' \given \J{\&$(\Tau_l)$*|$(\Tau_r)$} \cong^- \tau_i
    } {
...
\begin{theorem}(Model typing record elimination)
  ...
  $ 
    ...
    $
      ...
      $ by definition 
      \item \Z\Z $
        \J{$G$.$l$}[\vec{\sigma}] \rightsquigarrow v[\vec{\sigma} \sqcup \epsilon] 
      $ by substitution 
      \item \Z\Z $
    ...
    \item \Z $\B{case }
      ...
      \vec{\delta}, \vec{\sigma}' \satisfies \Gamma'
      \I
      \vec{\delta}, \Gamma \sqcup \Gamma' \satisfies e' \hastype \J{$l$->$\tau$}
    $
    \item \Z $\B{hypo }
      \vec{\delta}, \Gamma \sqcup \Gamma' \satisfies e' \hastype \J{$l$->$\tau$}
      \implies
      \vec{\delta}, \Gamma \sqcup \Gamma' \satisfies \J{$e'$.$l$} \hastype \tau
    $
    \item \Z $\B{wrt }
    ...
    $
      ...
      $ by application
      \item \Z\Z $
        \J{$e[\vec{\sigma}]$.$l$} \rightsquigarrow \J{$e'[\vec{\sigma} \sqcup \vec{\sigma}']$.$l$}
      $ by definition 
      \item \Z\Z $
        \J{$e[\vec{\sigma}]$.$l$} = \J{$e$.$l$}[\vec{\sigma}]
      $ by definition 
      \item \Z\Z $
        \J{$e'[\vec{\sigma} \sqcup \vec{\sigma}']$.$l$} = \J{$e'$.$l$}[\vec{\sigma} \sqcup \vec{\sigma}']
      $ by definition 
      \item \Z\Z $
        \J{$e$.$l$}[\vec{\sigma}] \rightsquigarrow \J{$e'$.$l$}[\vec{\sigma} \sqcup \vec{\sigma}']
      $ by substitution 
      \item \Z\Z $
        \vec{\delta}, \Gamma \satisfies \J{$e$.$l$} \hastype \tau 
      $ by definition 
...
% e :  ALL[a b . 3 <: a ] (a,b) \implies e : (a,b)
    ...
    %   \Delta_0 \preceq \Delta_1
    %   \\
    %   \widebar{\J{.$\tau_0[\alpha\slash\tau_l]$<:$\tau_1$}}^{\tau_0<:\tau_1 \in \Delta_{rel}} \sqcup \widebar{\J{.$\tau_l$<:$\tau$}}^{\tau \in \Tau}
    %   \given M_1, \Delta_1
    % } {
    %   \tau_l \subtypes \alpha
    %   \given M_1, \Delta_1
...
\begin{theorem}\ Model typing reduced implication elimination  
  ...
  \item $\B{assume } \VDash e_1[\vec{\sigma}]$
    ...
    \item \Z $\B{wrt}\ e_1' $
      ...
      \item \Z\Z $(\J{($F$?$p$=>$e$)($e_1$)})[\vec{\sigma}] \rightsquigarrow (\J{($F$?$p$=>$e$)($e_1'$)})[\vec{\sigma} \sqcup \epsilon]$ by substitution 
      \item \Z\Z $\Gamma \sqcup \epsilon = \Gamma$ by definition
      \item \Z\Z $\vec{\delta}, \epsilon \satisfies \epsilon$ by definition
      \item \Z\Z $\vec{\delta}, \Gamma \sqcup \epsilon \satisfies \J{($F$?$p$=>$e$)($e_1'$)}$ by substitution 
  ...
  \item $\B{assume}\ F = \epsilon \lor \vec{\delta}, \Gamma \satisfies F \hastype \tau_l \J{->} \tau_r$
    ...
    \item \Z $\B{case } F = \epsilon$ 
      ...
      \item \Z\Z $\J{($F[\vec{\sigma}]$?$p$=>$e[\vec{\sigma}\backslash\B{FV}(p)]$)($v_1$)} \rightsquigarrow e[\vec{\sigma}\backslash\B{dom}(\vec{\sigma}')][\vec{\sigma}]$ 
        by substitution
      \item \Z\Z $\J{($F[\vec{\sigma}]$?$p$=>$e[\vec{\sigma}\backslash\B{FV}(p)]$)($v_1$)} \rightsquigarrow e[\vec{\sigma} \sqcup \vec{\sigma}']$ 
        by substitution 
      \item \Z\Z $\J{($F$?$p$=>$e$)$[\vec{\sigma}]$($v_1$)} \rightsquigarrow e[\vec{\sigma} \sqcup \vec{\sigma}']$ by definition 
      \item \Z\Z $\J{($F$?$p$=>$e$)$[\vec{\sigma}]$($e_1[\vec{\sigma}]$)} \rightsquigarrow e[\vec{\sigma} \sqcup \vec{\sigma}']$ by substitution  

      \item \Z\Z $\J{($F$?$p$=>$e$)$[\vec{\sigma}]$($e_1[\vec{\sigma}]$)} = (\J{($F$?$p$=>$e$)($e_1$)})[\vec{\sigma}]$ by definition  
      \item \Z\Z $(\J{($F$?$p$=>$e$)($e_1$)})[\vec{\sigma}] \rightsquigarrow e[\vec{\sigma} \sqcup \vec{\sigma}']$ by substitution  

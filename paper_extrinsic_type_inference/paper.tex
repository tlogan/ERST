...
\newcommand{\pass}{$\CIRCLE$}
\newcommand{\fail}{$\Circle$}
...
We performed experiments for two broad categories of type inference capabilities: parametric and structural. 
...
\begin{experiment}
  \label{experi:typing_parameters_part_1}
  Typing Parameters - Part 1
  \TODO{this first example is actually generalization, not instantiation. Should we ignore/modify this delineation from SuperF?}
  \\
  \begin{center}
  \scriptsize
  ...
  \begin{tabular}{|lm{35em}||c||c|c|} 
    \hline
    \multicolumn{5}{|c|}{$\approx \exists \tau, \Delta, \Theta \qua \epsilon, \epsilon, \textbf{context} \entails e \hastype \tau \given \Theta, \Delta$ } \\
    \hline
    \hline
    \multicolumn{2}{|c||}{$e$} & \textbf{Extrinsic} & \textbf{SuperF} & \textbf{SuperF w/o D} \\ 
% [0.5ex] 
    \hline\hline
    1. &
    \J{\{x => \{y => y\}\}} 
...
\begin{experiment}
  \label{experi:typing_parameters_part_2}
  Typing Parameters - Part 2 
  \\
  \begin{center}
  \scriptsize
  % \setlength{\arrayrulewidth}{0.5mm}
...
\begin{experiment}
  ...
  \setlength{\tabcolsep}{4pt}
  \renewcommand{\arraystretch}{1.5}
  \begin{tabular}{|lm{35em}||c||c|} 
    \hline
    \multicolumn{4}{|c|}{$\approx \exists \tau, \Delta, \Theta \qua \epsilon, \epsilon, \textbf{context} \entails e \hastype \tau \given \Theta, \Delta$ } \\
    \hline
    \hline
...
\TODO{reformat tables, spacing, etc.}
...
Like SuperF, our system relies syntax-directed subtyping rules check constraints and to learn 
new subtyping constraints by traversing previously learned constraints;
Both systems have instantiation and skolemization in their subtyping rules. 

\TODO{discuss how our system doesn't need extrusion or distributivity}

\TODO{discuss relationship to flow analysis}

\TODO{discuss type inference of structural data}


\TODO{organize by the spectrum of type inference from intrinsic to extrinsic}

\TODO{Note how Extrinsic recombined various paths using the pack function}

\TODO{try to understand what SuperF folks mean by no backtracking in light of rhs union}

\section{Conclusion}
\label{sec:conclusion}

\TODO{recap the achievements. Mention the relational reasoning in future work}

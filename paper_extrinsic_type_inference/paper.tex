...
\begin{property}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      \exists \delta' .\ \text{dom}(\delta') \cong \Theta' \backslash \Theta \land
      (
      \forall \delta'' .\ 
      \delta'' \cup \delta' \cup \delta \satisfies \Delta' \backslash \Delta \implies
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      \Theta \cup \text{FTV}(\Delta) \cup \text{FTV}(\Gamma) = \Theta_\invar
      \\\\
      \forall \Theta''\ \Delta'' .\ 
      ...
      \\
      \text{pack}^-(\Theta_\invar\ \alpha_\mu \entails \Pi_\mu) = \tau_\mu
      \\\\

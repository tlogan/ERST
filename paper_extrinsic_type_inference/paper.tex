...
As for constructions and patterns, consider a Standard ML program that computes the length of a list,
...
means that the strongest assumption allowed for the the sum of the patterns,
is \code{'a list}. 
Thus, it forbids pattern matching with on fewer patterns than those in the datatype.
...
\block{
  \J{fun elen(nil) = zero}
  \\
  \J{\ \ | elen(cons(x,cons(y,zs))) = succ(succ(elen(zs)))}
}

Additionally, the nat datatype prescribes the strongest type
inferrable for constructions, and    

For instance, the datatype mechanism in Standard ML and related languages 

\TODO{define intrisic types as type annotations that restrict the seach space or control flow of a program}

\TODO{note the four kinds of intrinsic types: inputs/outputs of compositional data and input/output of structural data}

\TODO{f(a) is an example of a compositional data}

\TODO{(cons;(x, xs), zero;@) is an example of structural data}
...
Consider the function \mathspan{\J{talky}}, which completes a simple English phrase.

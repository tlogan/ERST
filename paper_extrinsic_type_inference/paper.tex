...
\begin{property}
  ...
  \begin{mathpar}
    \inferrule {
      ...
      \\\\
      (
      \exists \delta \qua \textbf{dom}(\delta) \cong \Theta^\dagger \backslash \Theta \up 
      (
      \forall \delta' \qua 
      \delta' \cup \delta \satisfies \Delta^\dagger \implies
    ...
    \inferrule {
      ...
      \forall \delta' \qua 
      \delta' \cup \delta \satisfies \Delta^\dagger \implies
      \delta' \cup \delta \satisfies \tau^\dagger \subtypes \tau
      )
      )
    }
  \end{mathpar}
...
\begin{theorem}
  \label{theorem:function_soundness}
  Function Soundness 
  \\
  \begin{mathpar}
    \inferrule {
...
and we assume $\delta' \cup \delta \satisfies \Delta'$ and $\delta' \cup \delta, \sigma \satisfies \Gamma$.
With a little rewriting, we have $\delta \satisfies \Delta'$ and $\delta, \sigma \satisfies \Gamma$.

We have $\Pi = \epsilon \down \exists \Theta^\dagger, \Delta^\dagger, \tau^\dagger \qua (\Theta^\dagger, \Delta^\dagger, \tau^\dagger) \in \Pi$
by applying Theorem \ref{theorem:collection_membership_choice} (Collection Membership Choice), along with a little rewriting.
We consider two cases by inverting disjunction.
In the empty case $\Pi = \epsilon$, we derive $\textbf{pack}^+(\dTheta \entails \Pi) = \J{TOP}$ 
by applying Definition \ref{def:multiple_type_packaging} (Sequence Packaging). 
Thus, we trivially derive $\delta' \cup \delta, f[\sigma] \hastype \tau$ by applying Definition \ref{def:model_typing} (Model Typing).
In the membership case, we get $\Theta^\dagger$, $\Delta^\dagger$, and $\tau^\dagger$, such that

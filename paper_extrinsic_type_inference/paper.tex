...
The context consists of skolems \ms{\Theta}, subtypings \ms{\Delta}, and typings \ms{\Gamma}. 
The contextual subtypings and typings constrain the search space of valid typings, 
such that the generated subtypings are consistent with them. 
In fact, the generated subtypings contain
...
The generated subtypings must be satisfiable under every interpretation of
...
to infer as many possible schemas for the source expression \ms{e}.
...
The context consists of skolems \ms{\Theta} and subtypings \ms{\Delta}. 
The contextual subtypings constrain the search space of valid subtypings, 
such that the generated subtypings are consistent with them. 
In fact, the generated subtypings contain all the contextual subtypings.
As with proof typing, skolems constrain the search space,
such that the generated subtypings must be satisfiable
for every interpretation of skolems for which the contextual subtypings are satisfiable.
...
a solution to the subtypings generated by solving the qualifiers.
...
from which the witnesses can be inferred from the generated subtypings.
...
\noindent
Proof subtyping sequence very simply solves a sequence of subtypings
and compounds their results. 
...
\noindent
Proof subtyping soundness states that if proof subtyping generates subtypings \ms{\Delta'} 
for a subtyping goal \ms{\tau_l \subtypes \tau_r},
then any solution for the generated subtypings
...
We prove soundness by induction, leveraging numerous lemmas to derive the semantics
...
Lastly, when pruning subtypings generated by the solver, it is necessary
that the remaining subtyping are as strong or stronger that the removed subtypings
...
generate an unsatisfiable subtypings.
To make the stronger guarantee, we can combine it with another theorem.


\begin{theorem}
  \label{theorem:proof_subtyping_substance}
...
\noindent
Proof Subtyping Substance states that generate constraints are satisfiable
assuming the contextual constraints are satisfiable, with respect
to all interpretations of the contextual skolems.   
The proof is by induction, 
where in most cases the property follows directly from the the induction hypothesis.
The only cases that are actually interesting are the variable rules
where new constraints are generated.
Because the rules check for consistency by searching for constraints on single variables,
it is necessary that the constraints on a single variable are as strong or stronger
than all the constraints with respect to that variable.
It then boils down to checking the bound in the goal against all the bounds on the other side
of the variable, as those are the only opportunities for contradiction.    

...
\[
  \begin{array}[t]{@{} l}
    ...
    \\
    \I \J{\& (}\text{list}(\text{nat}) \J{ -> } \text{list}(\text{nat})
    \\
    \I \J{\& (}\text{list}(\text{list}(\text{nat})) \J{ -> } \text{list}(\text{list}(\text{nat}))
    \\
...
% [0.5ex] 
    ...
    1. &
    \begin{array}[t]{l}
      \J{let isNatList = loop(\{self => } 
      \\
      \I \J{\{nil;@ => true;@\}}
      \\
      \I \J{\{cons;(n, xs) => isNat(n) and self(xs)\}}
      \\
      \I \J{\{ignore => false;@\}}
      \\
      \J{\}) in ...}
...
\TODO{test these examples in implementations}

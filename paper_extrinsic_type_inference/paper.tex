...
\begin{definition} 
  ...
  \\
  \begin{align*}
      \llbracket e_l \J{,} e_r \rrbracket^\square &= \J{?left}\fieldmap e_l \ \J{?right}\fieldmap e_r
      \\
...
For any statement or term containing sugar $s$, we assume that $s$ is implicitly replaced with $\llbracket s \rrbracket^\square$.
...
\begin{definition} 
  ...
  \begin{align*}
    ...
    \\
    \llbracket \tau_l \J{*} \tau_r \rrbracket^\square &= (\J{left} \fieldmap \tau_l)  \J{\&} (\J{right} \fieldmap \tau_r)
  \end{align*}
\end{definition} 
\hfill

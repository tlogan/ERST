...
lemma Zone.pack_negative_right_to_left_soundness {pids t am assums skolems' assums' t'} :
  Zone.pack pids .false ⟨skolems', assums', t'⟩ = t →
  ListSubtyping.free_vars assums ⊆ pids →
  MultiSubtyping.Dynamic am assums →
  ∀ e,
...
:= by
  ...
  =>
    ...
    -- TODO: may need dynamic typing of assums0, generated by substance

    specialize p3 (Iff.mpr List.mem_singleton rfl)

    have myLemmaZ : Subtyping.Dynamic am (Typ.pair (Typ.var idl) r) (.lfp id t') := by
      unfold Subtyping.Dynamic
      intros  e0 temp1

      have temp20 : MultiSubtyping.Dynamic ((id,.bot)::am) assums := by
        -- since id non in assums
        sorry

      have temp22 : MultiSubtyping.Dynamic ((id,.bot)::am) assums0' := by
        -- should follow from inversion, or some sort of substance requirement
        sorry

      -- have temp24 : Typing.Dynamic ((id,.bot)::am) e0 (Typ.var idl) :=
      --   -- follows from temp1
      --   -- since id ∉ FV(.var idl), we have am |= e : idl → (id,BOT)::am |= e : idl
      --   sorry

      have temp26 : Typing.Dynamic ((id,.bot)::am) e0 (Typ.pair (Typ.var idl) r) :=
        -- follows from temp1
        -- since id ∉ FV(.var idl), we have am |= e : idl → (id,BOT)::am |= e : idl
        sorry

      have temp28 := Zone.pack_negative_right_to_left_soundness p6
        (List.subset_cons_of_subset id (List.subset_cons_of_subset idl (fun _ x => x)))
        temp20 e0 temp22 temp26

      have temp30 : Typing.Dynamic am e0 (.lfp id t') := by
        apply Typing.Dynamic.lfp_intro_bot
          (Typ.Monotonic.Static.soundness am p7)
          temp28

      exact temp30

    have myLemmaA : Subtyping.Dynamic am (Typ.var idl) (.lfp id l) := by
      -- follow from myLemmaZ --- Subtyping.Dynamic am (Typ.pair (Typ.var idl) r) (.lfp id t')

      -- start with pack_negative_soundness
      -- since α is uninterpreted,
      -- ∀ e  t, ... (α,t) :: am |= e : (al,r) → (α,t) :: am |= e : t'
      -- we can sepcialized so that  (al,r) <: t'[αlpha ↦ lfp]

      -- convert consequent to typing with lfp
      -- show that factoring the same on both sides preserves subtyping
      sorry
    ...
    apply myLemmaA at p13

    apply myLemmaB


    have ⟨ep,p14,p15⟩ := Typ.factor_expansion_soundness p8 p13
...
mutual
  ...
  | .loop body t0 id zones zones' p0 p1 p2 p3 => by
    ...
    intros tam' p20
    intros eam p30
    apply Subtyping.LoopListZone.Static.soundness p3 p20 (by sorry) -- TODO add requirement to static definition

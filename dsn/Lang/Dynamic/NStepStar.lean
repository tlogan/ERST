...
theorem NStepStar.app_inversion :
  ...
  (∃ m' body p f arg',
    NStepStar (Expr.sub m' body) e ∧
    NStepStar cator (.function ((p,body)::f)) ∧
    NStepStar arg arg' ∧
    Expr.pattern_match arg' p = some m'
  ) ∨
  (∃ f arg' p body,
    NStepStar (.app (.function f) arg') e ∧
    NStepStar cator (.function ((p,body)::f)) ∧
    NStepStar arg arg' ∧
    arg'.is_value ∧
...
--       cases h0 with
--       | inl h1 =>
--         have ⟨m',body,p,f,arg',h2,h3,h4,h5⟩ := h1
--         clear h1
--         have joinable_cator := NStep.semi_confluence step' h3


--         -- have joinable_arg := NStepStar.joinable h4
--         -- have joinable_app := Joinable.app joinable_cator joinable_arg

--       ----------------------------

--         -- have ⟨m,h6⟩ := Expr.pattern_match_subject_star_expansion h4 h5

--         -- have h7 := NStepStar.pattern_match_preservation h4 h6 h5 body
--         -- apply Joinable.subject_star_reduction h7
--         -- exact Joinable.pattern_match h6 joinable_cator
...
--         rw [h2]
--         clear h1 h2 step_star
--         /- TODO: figure out how to use new inversion definition -/
...
/- TODO: define parallel step
  - prove diamond property
  - induct on parallel step
-/
theorem NStepStar.confluence :
  NStepStar e a →
  NStepStar e b →
...
:= by
  intro h0 h1
  apply NStepStar.semi_confluence
  { exact h0 }

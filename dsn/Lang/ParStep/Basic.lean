...
mutual
  ...
  inductive ParFunStep : List (Pattern × Expr) → List (Pattern × Expr) → Prop
  | nil : ParFunStep [] []
  | cons p :
...
:= by
  ...
  | @cons e e' rr rr' l' step_e step_rr =>
    intro fresh_r
    simp [Prod.key_fresh] at fresh_r
    have ⟨h1,h2⟩ := fresh_r
    clear fresh_r
    simp [Prod.key_fresh]
    apply And.intro h1
...
:= by
  ...
  | @cons e e' rr rr' l' step_e step_rr=>
    intro h1
    simp [Prod.keys_unique] at h1
    have ⟨h2,h3⟩ := h1
    simp [Prod.keys_unique]
    apply And.intro
...
mutual
  ...
  | @cons e e' rr rr' l' step_e step_rr =>
    simp [Pattern.match_entry]
    simp [Expr.record_valued] at isval
    have ⟨⟨h0,h1⟩,h2⟩ := isval
    intro h3
    by_cases h4 : l' = l
    {
  ...
  | iso l p' =>
    ...
    | @iso body body' l' step_body =>
      simp [Pattern.match]
      simp [Expr.valued] at isval
      intro h0 h1
      apply And.intro h0
      have ⟨m,ih⟩ := ParStep.pattern_match_expansion isval step_body h1
      exists m
    | _ =>
      simp [Pattern.match] <;> simp [Expr.valued] at isval
  | record ps =>
    cases step with
    | bvar i =>
      intro h0
...
mutual
  ...
  | @skip f f' arg arg' p body step_f step_arg isval nomatching' =>
    ...
    { apply ParFunStep.shift_vars_preservation _ _ step_f }
    { apply ParStep.shift_vars_preservation _ _  step_arg }
    { exact Expr.valued_shift_vars_preservation isval threshold offset }
...
theorem ParGroupStep.concat
  (step0 : ParGroupStep m0 m0')
: ParGroupStep m1 m1' →
  ParGroupStep (m0 ++ m1) (m0' ++ m1')
:= by cases step0 with
| nil =>
  simp
| @cons e e' es es' step_e step_es =>
  intro h0
  apply ParGroupStep.cons step_e
  apply ParGroupStep.concat step_es h0


mutual

  theorem ParGroupStep.pattern_match_entry_preservation
    (step : ParRcdStep r r')
  : Pattern.match_entry l p r = some m →
    Pattern.match_entry l p r' = some m' →
    ParGroupStep m m'
  ...
  | @cons e e' r r' l' step_e step_r =>
    ...
    { simp [h0]
      intro h1 h2
      apply ParGroupStep.pattern_match_preservation step_e h1 h2
    }
    { simp [h0]
      intro h1 h2
      apply ParGroupStep.pattern_match_entry_preservation step_r h1 h2
    }

  theorem ParGroupStep.pattern_match_record_preservation :
    ParRcdStep r r' →
  ...
  | iso l bp =>
    ...
    | @iso body body' l' step_body =>
      simp [Pattern.match]
      intro h0 h1 h2 h3
      apply ParGroupStep.pattern_match_preservation step_body h1 h3
    | _ =>
      simp [Pattern.match]
  | record ps =>
    intro step
    ...
    | @record r r' step_r =>
      simp [Pattern.match]
      intro h0 h1 h2 h3
      apply ParGroupStep.pattern_match_record_preservation step_r h1 h3
    | _ =>
      simp [Pattern.match]
end
  ...
  | bvar i =>
    ...
    { simp [h0]
      ...
      | some e, some e' =>
        ...
        { exact ParGroupStep.pattern_match_preservation step_arg matching matching' }
        { exact h1 }
        { exact h2 }
      | none,none =>
        simp
      ...
      | none,some e' =>
        ...
        have h5 : ¬ i - offset < Pattern.count_vars p := by
...
mutual
      ...
      | @function _ f' step_f =>
        ...
        | @cons _ eeb _ ff _ step_eeb step_ff =>
          ...
          | nil =>
            ...
            exists (Expr.instantiate 0 [Expr.loopi (Expr.function [(Pattern.var x, eec)])] eec)
            apply And.intro
            { exact ParStep.recycle x h2 }
            {
              have h3 : ParStep
                (Expr.loopi (Expr.function [(Pattern.var x, eea)]))
                (Expr.loopi (Expr.function [(Pattern.var x, eec)]))
              := by
                apply ParStep.loopi
                apply ParStep.function
  ...
  | @recycle ee eea x step_eea =>
    ...
    | @recycle _ eeb _ step_eeb =>
      ...
      {
        have h3 : ParStep
          (Expr.loopi (Expr.function [(Pattern.var x, eeb)]))
          (Expr.loopi (Expr.function [(Pattern.var x, eec)]))
        := by
          apply ParStep.loopi
          apply ParStep.function

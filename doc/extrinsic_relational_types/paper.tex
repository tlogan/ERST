...
complex mathematical relations, but require that programs are constructed in rigid ways to be typed.
...
to the artificial restrictions of a verification system. Thus, for an expressive verification system
to be adopted by the masses, it must also be flexible.
...
numbers must first be prescribed with types.
...
This singleton list example is an involves refinement, and this particular kind of rigidity has
...
The least fixed point type enabled describing infinite spaces while the constraint types can
...
Syntax-directed rules would overly restrict the expressivity of types. However, this foundational
...
\begin{itemize}
  ...
  \item Section \ref{sec:future_work}: Future work.  
\end{itemize}
...
recursive types.
Consider applying the function \code{repeat} to some item and the number two, 
...
Unfortunately, because the type system eagerly factored the type of \code{repeat}, it
...
\TODO{
RATIONALE -- SEMANTICS-DIRECTED TYPING:
note that we use semantics-directed typing (i.e. dynamic) rather than syntax-directed typing (i.e. static), in order
to avoid overly restricting the types of constructions like functions or records.
That is, in a syntax-directed semantics, we strive to have functions 
introduce the strongest type possible, which requires combining numerous type forms,
and parsing the structures of expressions.
To ensure the semantics of type forms is clear and intuitive, 
we use sementics-directed typing, which allows
each type form to be specified separately.
In essence, the semantics-directed typing is a denotation
from type syntax to predicate logic.
By contrast, simple type systems are so simple that the syntax-directed typing definitions
are intuitive already, and so there is no need for a separate semantics-directed definition.
}
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \\
      \Theta, \Delta, \Gamma \entails e \hastype \tau_e \given \Theta', \Delta'
      \\
      \Theta', \Delta' \entails \tau_e \subtypes \tau_a \given \Theta'', \Delta''
    } {
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      ...
      \textbf{factor}(\alpha \entails \tau' \cdot \J{right}) = \tau_r'
      \\\\
      \alpha_l \wr^+ \J{LFP[} \alpha \J{]}\tau_r'
      \\
      % founded upper bound
      \alpha \downarrow \tau_l \subtypes \tau_l'
      \\
      (\J{LFP[} \alpha \J{]}\tau_r')[\alpha_l\slash \J{LFP[} \alpha \J{]} \tau_l'] = \tau_r''
    } {
      \Theta^\dagger \entails \alpha \circlearrowleft (\Theta, \Delta, \alpha_l \J{->}\tau_r) \subtypes \alpha_l \J{->} \tau_r''
    }
  \end{mathpar}
\end{definition}
...
\TODO{consider replacing "substance" with some variation on "conditional completemness"}
\begin{theorem}

...
\begin{definition}
  \label{def:subtyping_guarded_list_zone_static}
  Subtyping Guarded List Zone Static 
  \hfill 
  \small
  ...
  \begin{mathpar}
    \inferrule {
      (\forall \Theta', \Delta', \tau' \qua 
      (\Theta' \backslash \Theta, \Delta' \backslash \Delta, \tau') \in \Pi
      \implies
      \Theta, \Delta \entails \tau \subtypes \alpha \J{->} \tau' \given \Theta', \Delta' 
      )
      \\\\
      \textbf{ftv}(\Delta) \entails \Pi \searrow \Pi' % tidy or prune
    } {
...
\begin{definition} 
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \\\\
      \TODO{rewrite with function nested towward the right}
      \\\\
      ...
      % \\
      % ensure \Ga 
      \Delta, \Gamma \entails p :: \rho \given \Delta', \Gamma' %lift pattern to type
      % rhos is the weakest type where every value in rho matches pattern p
      \\
      \textbf{diff}(\rho, \Xi) = \tau_l
      \\\\
      \TODO{add additional universal requirement on zones}
      \\\\
      ...
      \tau_l \J{->} \tau_r \in \Pi'
      \implies
      \Theta, \Delta', \Gamma' \entails e \hastype \tau_r \given \Theta', \Delta'' 
      )
      \\\\
      \textbf{capture}(\rho) = \eta
      \\
...
\begin{definition}
  ...
  \begin{mathpar}
    ...
    \inferrule {
      \Theta \cap (\textbf{ftv}(\tau_l) \cup \textbf{ftv}(\tau_r)) \neq \epsilon
      \\
    ...
    \inferrule {
      \Theta \cap (\textbf{ftv}(\tau_l) \cup \textbf{ftv}(\tau_r)) = \epsilon 
      % \\\\
